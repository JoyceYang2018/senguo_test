...
class TimeFunc:
    ...
    def get_fake_clearing_state(cls, the_date):
        delta_day = (
            ...
            - datetime.datetime.combine(the_date, datetime.time.min)
        ).days
        fake_clearing_state = 0
        for i in range(delta_day):
            temp_day = (the_date + datetime.timedelta(i + 1)).strftime(
                "%Y-%m-%d"
            )
    ...
    @classmethod
    def transfer_range_normal_to_accounting(
        cls, session, shop_id, from_date, to_date
    ):
            ...
            while int_part:
                ...
                tmp = "".join(
                    [
                        ...
                        for x, y in list(
                            zip(small_int_part[::-1], small_int_label)
                        )[::-1]
                    ]
                )
...
class ShopFunc:
    ...
    def get_receipt_config(cls, session, shop_id):
        receipt_config = (
            session.query(models.ReceiptConfig).filter_by(id=shop_id).first()
        )
    ...
    def make_shop_code(cls, session):
        ...
        while 1:
            ...
            shop = (
                session.query(models.Shop.id).filter_by(shop_code=str).scalar()
            )
            if not shop:
                break
        return str
    ...
    def check_cashier_need_print(self, session, shop_id, receipt_config=None):
        ...
        if (
            receipt_config.cashier_accountant_receipt
    ...
    def check_salesman_need_print(self, session, shop_id, receipt_config=None):
        ...
        if not receipt_config:
            receipt_config = ShopFunc.get_receipt_config(session, shop_id)
        if (
            receipt_config.salesman_async_customer_receipt
            or receipt_config.salesman_async_accountant_receipt
            or receipt_config.salesman_async_goods_receipt
        ):
            need_print = True
        return need_print
    ...
    def get_shop_name_through_id(cls, session, shop_id):
        shop_name = (
            session.query(models.Shop.shop_name).filter_by(id=shop_id).scalar()
            or ""
        )
        return shop_name
    ...
    def get_shop_name_dict(cls, session, shop_id_list):
        ...
        shop_name_list = (
            session.query(Shop.id, Shop.shop_name)
    ...
    def get_supplier_ids_in_shop(cls, session, shop_id):
        """ 店鋪內所有供应商id """
        suppliers = (
            session.query(models.ShopSupplier)
    ...
    def is_admin_have_many_shop(cls, session, shop_id):
        ...
        shops_count = 0
        result = (
            session.query(models.Shop.boss_id).filter_by(id=shop_id).first()
        )
        if result:
            shops_count = (
                session.query(models.Shop)
                .filter_by(boss_id=result.boss_id)
                .count()
            )
    ...
    def is_max_staff(cls, session, shop_id, config=None):
        ...
        if not config:
            ...
            result = (
                session.query(
                    Config.max_accountant_count,
                    Config.max_admin_count,
                    Config.max_salesman_count,
                )
        ...
        admin_count = (
            session.query(HireLink.id)
            .filter(HireLink.active_admin == 1, HireLink.shop_id == shop_id)
            .count()
        )
        salesman_count = (
            session.query(HireLink.id)
            .filter(HireLink.active_salesman == 1, HireLink.shop_id == shop_id)
            .count()
        )
        accountant_count = (
            session.query(HireLink.id)
            .filter(
                HireLink.active_accountant == 1, HireLink.shop_id == shop_id
            )
            .count()
        )
        ...
        if (
            max_accountant_count >= 0
            and accountant_count >= max_accountant_count
        ):
            is_max.append(1)
        else:
            is_max.append(0)
...
class StatisticFunc:
    ...
    def get_static_range(cls, type, current_year, current_month):
        ...
        if (
            type == 1
            and int(current_year) == now.year
            and int(current_month) == now.month
        ):
            ...
            if current_month in ("01", "03", "05", "07", "08", "10", "12"):
                ...
                int(current_year) % 4 == 0 and not int(current_year) % 100 == 0
    ...
    def update_statistic_data(
        self, session, statistic_session, shop_id, if_force_update=False
    ):
        ...
        if if_force_update:
            ...
            last_update_timestamp = (
                statistic_session.query(
                    models_statistic.StatisticDataDaily.last_update_timestamp
                )
                .filter_by(
                    shop_id=shop_id, statistic_date=chooseday, statistic_type=1
                )
                .scalar()
            )
    ...
    @classmethod
    def update_accounting_periods_statistic_data(
        self, session, statistic_session, shop_id, if_force_update=False
    ):
    ...
    @classmethod
    def get_funds_info_per_shop_statitic_instance(
        cls, single_fund, data_info, source="accountant"
    ):
        ...
        deposit_net_cent = (
            single_fund.deposit_net_cent or 0 if single_fund else 0
        )
        ...
        online_cent = (
            lc_fund_alibar + lc_fund_aliwap + lc_fund_wxbar + lc_fund_wxwap
        )
    ...
    def get_funds_info_per_accountant_statitic_instance(
        ...
        fund_cash = (
            single_accountant_fund.fund_cash or 0
            if single_accountant_fund
            else 0
        )
        lc_fund_wxbar = (
            single_accountant_fund.lc_fund_wxbar or 0
            if single_accountant_fund
            else 0
        )
        lc_fund_alibar = (
            single_accountant_fund.lc_fund_alibar or 0
            if single_accountant_fund
            else 0
        )
        lc_fund_wxtag = (
            single_accountant_fund.lc_fund_wxtag or 0
            if single_accountant_fund
            else 0
        )
        lc_fund_alitag = (
            single_accountant_fund.lc_fund_alitag or 0
            if single_accountant_fund
            else 0
        )
        fund_pos = (
            single_accountant_fund.fund_pos or 0
            if single_accountant_fund
            else 0
        )
        fund_cheque = (
            single_accountant_fund.fund_cheque or 0
            if single_accountant_fund
            else 0
        )
        fund_remittance = (
            single_accountant_fund.fund_remittance or 0
            if single_accountant_fund
            else 0
        )
    ...
    def get_goods_stockin_data(
        ...
        if data_type:
            StatisticGoodsStockin = (
                models_account_periods_statistic.AccountPeriodsStatisticGoodsStockin
            )
        else:
        ...
        if only_goods:
            ...
            query_base_stockin = statistic_session.query(
                StatisticGoodsStockin.goods_id, *columns_stockin
            ).group_by(StatisticGoodsStockin.goods_id)
        else:
        ...
        if group_id:
            goods_in_group = (
                session.query(models.Goods)
                .filter_by(group_id=group_id, shop_id=shop_id)
                .all()
            )
            filter_params.append(
        ...
        if page == -1:
            ...
            statistic_all = (
                query_base_stockin.offset(page_size * page)
                .limit(page_size)
                .all()
            )
            # 总条目数
        ...
        for statistic in statistic_all:
            ...
            data["quantity_unit_price"] = check_float(
                statistic.total_price / statistic.quantity
...
class StatisticGoodsSalesFunc:
    ...
    @classmethod
    def init_goods_accountant_dicts(
        cls, accountant_id_goods_id_list, map_goods
    ):
        ...
        base_goods_accountant_dict = dict(
            collections.ChainMap(cls._goods_info_dict, cls._sales_kpi_base_dict)
        )
    ...
    @classmethod
    def update_goods_salesman_dict(
        cls, goods_salesman_dict_single, record, gross_cent
    ):
    ...
    def update_daily_dict(cls, daily_dict, record, gross_cent):
        ...
        if record.status in (3, 5):
            ...
            daily_dict["total_cent"] += (
                total_cent if record.record_type == 0 else -total_cent
            )
            daily_dict["sales_cent"] += (
                record.sales_money
                if record.record_type == 0
                else -record.sales_money
            )
            daily_dict["commission_cent"] += (
                record.commission_money
                if record.record_type == 0
                else -record.commission_money
            )
            daily_dict["deposit_cent"] += (
                record.deposit_money
                if record.record_type == 0
                else -record.deposit_money
            )
            daily_dict["gross_cent"] += (
                gross_cent if record.record_type == 0 else -gross_cent
            )
        return None
...
class GoodsFunc:
    ...
    @classmethod
    def get_goods_info(
        cls, goods, source="", multi=False, weight_unit_text="斤"
    ):
        check_float = NumFunc.check_float
        ...
        goods_info["cost_price_weight"] = check_float(
            goods.get_cost_price_weight(weight_unit_text=weight_unit_text) / 100
        )
    ...
    def get_goods_storage_info(cls, goods, stockin_unit, weight_unit_text="斤"):
        ...
        cost_total = (
            check_float(goods.cost_price_quantity * goods.storage)
            if stockin_unit
            else check_float(goods.cost_price_weight * goods.storage_num)
        )
    ...
    def get_supplier_goods(cls, session, shop_id, goods_id, supplier_id):
        goods = (
            session.query(models.Goods)
            .filter_by(
                shop_id=shop_id, id=goods_id, shop_supplier_id=supplier_id
            )
            .first()
        )
        return goods
    ...
    def get_goods_name_through_id_list(cls, session, goods_id_list):
        ...
        if goods_id_list:
            Goods = models.Goods
            goods = (
                session.query(Goods).filter(Goods.id.in_(goods_id_list)).all()
            )
            goods_name_dict = {
        ...
        if goods_id_list:
            ...
            goods_name_dict = {
                g.id: {
                    ...
                    "unit_text": cls.getUnit(
                        g.unit, weight_unit_text=weight_unit_text
                    ),
                    "unit": g.unit,
                }
                for g in goods
    ...
    def get_group_id_dict_with_goods_ids(cls, session, goods_id_list):
        goods_id_group_id = session.query(
            models.Goods.id, models.Goods.group_id
    ...
    def get_group_name_dict_through_id_list(cls, session, group_id_list):
        ...
        if group_id_list:
            GoodsGroup = models.GoodsGroup
            group_id_name_list = (
                session.query(GoodsGroup.id, GoodsGroup.name)
                .filter(GoodsGroup.id.in_(group_id_list))
                .all()
            )
            for group_id, group_name in group_id_name_list:
    ...
    @classmethod
    def get_goods_supplytype_goodstype_through_id_list(
        cls, session, goods_id_list
    ):
    ...
    @classmethod
    def get_goods_info_list_through_id_list(
        cls, session, shop_id, goods_id_list
    ):
        goods_info_dict = {}
        if goods_id_list:
            Goods = models.Goods
    ...
    def get_salesman_goods_limit_list(cls, session, shop_id, account_id):
        ...
        goods_limit = (
            session.query(SalesmanGoodsLimit.limit_goods_list)
    ...
    def get_goods_through_goods_type(cls, session, shop_id, goods_type_list):
        ...
        goods_id_list = (
            session.query(Goods.id)
            .filter(
                Goods.shop_id == shop_id, Goods.goods_type.in_(goods_type_list)
            )
            .all()
        )
        goods_id_list = {x.id for x in goods_id_list}
    ...
    def get_id_goup_by_supply_type(cls, session, shop_id):
        ...
        Goods = models.Goods
        goods_id_list = (
            session.query(Goods.id, Goods.supply_type)
            .filter_by(shop_id=shop_id)
            .all()
        )
    ...
    def get_count_goup_by_supply_type(cls, session, shop_id, active=1):
        ...
        Goods = models.Goods
        goods_id_list = (
            session.query(Goods.supply_type, func.count(1))
    ...
    def get_goods_detail_names(cls, session, goods_id_list, map_suppliers):
        ...
        return {
            g.id: cls.get_goods_detail_name(g, map_suppliers) for g in goods
        }
    ...
    def get_latest_pay_type(cls, session, goods):
        """ 获取最近入库的支付类型 """
        detail = (
            session.query(models.StockInDocsDetail)
            .filter_by(goods_id=goods.id)
            .order_by(models.StockInDocsDetail.id.desc())
            .first()
        )
    ...
    def get_supplier_id_list_through_goods(cls, session, goods_id_list):
        ...
        Goods = models.Goods
        goods_query = (
            session.query(Goods.shop_supplier_id)
            .filter(Goods.id.in_(goods_id_list), Goods.shop_supplier_id > 0)
            .all()
        )
    ...
    def get_id_by_unit(cls, session, shop_id, unit=0):
        ...
        Goods = models.Goods
        goods_id_list = (
            session.query(Goods.id).filter_by(shop_id=shop_id, unit=unit).all()
        )
    ...
    def get_goods_supplier_count(cls, session, shop_id, active=1):
        ...
        Goods = models.Goods
        supplier_count = (
            session.query(func.distinct(Goods.shop_supplier_id))
            .filter_by(shop_id=shop_id, active=active)
    ...
    def get_goods_active_count(cls, session, shop_id, active=1):
        ...
        active_count = (
            session.query(func.count(Goods.id))
            .filter_by(shop_id=shop_id, active=active)
    ...
    def get_date_acitve_goods_ids(cls, session, shop_id, choose_time):
        ...
        """
        query_base = (
            session.query(func.max(models.GoodsHistory.id).label("history_id"))
            .group_by(models.GoodsHistory.goods_id)
            .filter_by(shop_id=shop_id)
        )
        id_querys = query_base.filter(
            models.GoodsHistory.edit_time < choose_time
        ).all()
        history_ids = [q.history_id for q in id_querys]
        goods_historys = (
            session.query(models.GoodsHistory)
            .filter(models.GoodsHistory.id.in_(history_ids))
            .all()
            if history_ids
            else []
        )
    ...
    def get_goods_in_shop(
        ...
        detail_ids = (
            session.query(func.max(StockInDocsDetail.id))
            .join(StockInDocs, StockInDocsDetail.doc_id == StockInDocs.id)
            .filter(
                StockInDocsDetail.goods_id.in_(goods_id_list),
        ...
        details = (
            session.query(StockInDocsDetail)
        ...
        for g in goods:
            ...
            g_dict = dict(
                ...
                storage_num=g.get_storage_num(weight_unit_text=weight_unit_text)
                / 100,
    ...
    @classmethod
    def get_goods_has_supplier_id_list(
        cls, session, shop_id, shop_supplier_id=0
    ):
        Goods = models.Goods
        goods_id_list = session.query(Goods.id).filter_by(shop_id=shop_id)
        if shop_supplier_id:
            goods_id_list = goods_id_list.filter_by(
                shop_supplier_id=shop_supplier_id
            )
        goods_id_list = goods_id_list.filter(
            Goods.shop_supplier_id > 0, Goods.active > 0
        ).all()
        goods_id_list = [x.id for x in goods_id_list]
        return goods_id_list
...
class GoodsBatchFunc:
    ...
    def get_current_batch(cls, session, goods_id):
        ...
        """
        batch = (
            session.query(models.GoodsBatch)
            .filter_by(goods_id=goods_id, consume_status=1, batch_status=1)
            .first()
        )
        if not batch:
            batch = (
                session.query(models.GoodsBatch)
                .filter_by(goods_id=goods_id, consume_status=0, batch_status=1)
    ...
    def get_default_batch(cls, session, goods_id):
        ...
        result = (
            session.query(models.GoodsBatch)
            .filter_by(goods_id=goods_id, consume_status=-1)
            .first()
        )
        return result
    ...
    def get_batch_num_through_id_list(cls, session, batch_ids):
        batchs = (
            session.query(models.GoodsBatch)
            .filter(models.GoodsBatch.id.in_(batch_ids))
            .all()
            if batch_ids
            else []
        )
    ...
    def consume(
        ...
        for consume in consume_list:
            ...
            rate = abs(
                ...
                if consume["quantity"]
                else consume["weight"] / sale_record.sales_num / 100
            )
    ...
    @classmethod
    def _construct_consume_list(
        cls, default_batch, batch_queue, quantity, weight, unit
    ):
    ...
    def _cal_consume(cls, batch, quantity, weight, unit):
        ...
        if unit == 1:
            ...
            consume_weight = (
                weight / quantity * consume_quantity if quantity else 0
            )
        else:
            if weight >= 0:
            ...
            consume_quantity = (
                quantity / weight * consume_weight if weight else 0
            )
    ...
    def get_branch_goods_id_by_batch(cls, session, goods_list):
        ...
        goods_id_query = (
            session.query(models.GoodsBatch.goods_id)
            .filter(
...
class GoodsGroupFunc:
    ...
    def get_groups_under_shop(cls, session, shop_id):
        ...
        """
        groups = (
            session.query(models.GoodsGroup.id, models.GoodsGroup.name)
            .filter_by(shop_id=shop_id)
            .filter(models.GoodsGroup.status.in_([-1, 0]))
            .all()
        )
...
class GoodsMasterBranchFunc:
    ...
    def list_goods_id_master(cls, session, shop_id):
        ...
        goods = (
            query_unself.union(query_self_master)
            .union(query_self_unmaster)
            .all()
        )
        result = {g.id for g in goods}
        return result
    ...
    @staticmethod
    def get_goods_base_query(
        session, shop_id, merge_branch_goods=True, only_id=False
    ):
        """ 货品基本查询，自营货品只取主货品 """
        if only_id:
    ...
    def apply_filter(base_query, shop_id):
        ...
        self_sole_master = base_query.filter(
            models.Goods.supply_type == 0,
            models.Goods.master_goods_id == 0,
            models.Goods.is_master == 0,
        )
    ...
    def list_branch_goods(cls, session, shop_id, master_goods_id):
        ...
        result = (
            session.query(models.Goods)
            .filter_by(
                shop_id=shop_id, master_goods_id=master_goods_id, is_master=0
            )
            .all()
        )
        return result
...
class PubRecordFunc:
    @classmethod
    def get_record_num_from_id_list(
        cls, session, RecordClass, id_list, shop_id=None, goods_id=None
    ):
        """ 根据id列表获取对应的num
        可用于GoodsSalesRecord,
             GoodsStockinRecord,
        ...
        :rtype: dict
        """
        base_query = session.query(RecordClass.id, RecordClass.num).filter(
            RecordClass.id.in_(id_list)
        )
        if shop_id:
    ...
    @classmethod
    def get_docs_id_from_num_list(
        cls, session, DocClass, num_list, shop_id=None
    ):
        """ 根据num列表获取对应的单据id
        ...
        if shop_id:
            query = (
                session.query(DocClass.id, DocClass.num)
                .filter(DocClass.num.in_(num_list), DocClass.shop_id == shop_id)
                .all()
            )
        else:
            query = (
                session.query(DocClass.id, DocClass.num)
                .filter(DocClass.num.in_(num_list))
                .all()
            )
...
class GoodsSaleFunc:
    ...
    def gen_sale_record_num(cls, session, shop_id, base_time=None):
        ...
        if redis.get(key):
            ...
            sale_record = (
                session.query(models.GoodsSalesRecord)
                .filter_by(shop_id=shop_id)
                .filter(
            ...
            sale_record_num = (
                str(int(sale_record.get_num()) + 1)
                if sale_record
                else str(shop_id) + base_time.strftime("%y%m%d") + "00001"
            )
    ...
    def get_sale_record_basic_info(cls, session, sale_record, display_kg):
        ...
        sales_num = (
            sale_record.sales_num / 2
            if display_kg and goods.unit == 0
            else sale_record.sales_num
        )
        fact_price = (
            sale_record.fact_price * 2
            if display_kg and goods.unit == 0
            else sale_record.fact_price
        )
        result = dict(
    ...
    def get_sale_record_info(
        ...
        goods = (
            session.query(models.Goods)
            .filter_by(id=sale_record.goods_id)
            .first()
        )
        supplier = (
            session.query(models.ShopSupplier)
            .filter_by(id=goods.shop_supplier_id)
            .first()
        )
        supplier_account = (
            AccountFunc.get_account_through_id(session, supplier.account_id)
            if supplier
            else None
        )
        sale_record_info = {}
        ...
        sale_record_info["full_bill_time"] = (
            str(sale_record.bill_year)
            + "-"
            + str(sale_record.bill_month).zfill(2)
            + "-"
            + str(sale_record.bill_day).zfill(2)
            + " "
            + str(sale_record.bill_time)
        )
        sale_record_info["num"] = sale_record.num
        sale_record_info["order_record_num"] = (
            order_record_num if order_record_num else sale_record.num
        )
        ...
        shop_name = (
            session.query(models.Shop.shop_name)
            .filter_by(id=sale_record.shop_id)
            .scalar()
        )
        ...
        if courier_record:
            ...
            courier_image_remark = (
                courier_record.image_remark.split(",")
                if courier_record.image_remark
                else []
            )
        ...
        sale_record_info[
            "pay_status_text"
        ] = cls.get_sales_record_pay_status_text(sale_record)
        ...
        sale_record_info["pay_type_text"] = (
            OrderBaseFunc.pay_type_text(sale_record.pay_type)
            if sale_record.order_id
            else "-"
        )

        sale_record_info["avg_price"] = (
            check_float(sale_record.sales_money / sale_record.commission_mul)
            if goods.unit == 0 and sale_record.commission_mul
            else 0
        )
        ...
        if sale_record.shop_customer_id:
            debtor = (
                session.query(models.ShopCustomer)
                .filter_by(id=sale_record.shop_customer_id)
                .first()
            )
        ...
        if sale_record.order_id:
            ...
            sale_record_info["combine_pay_type_text"] = (
                OrderBaseFunc.combine_pay_type_text(
            ...
            sale_record_info["signature_img"] = (
                signature_img if pay_type == 9 else ""
            )
        else:
            sale_record_info["pay_type_text"] = ""
            sale_record_info["combine_pay_type_text"] = ""
        ...
        if sale_record.order_id:
            ...
            if order and order.erase_money > 0:
                ...
                if print_erasement:
                    ...
                    sales_sum_count = (
                        session.query(func.count(models.GoodsSalesRecord.id))
                        .filter_by(order_id=sale_record.order_id)
                        .scalar()
                        or 0
                    )
            ...
            # 原始支付方式（仅修改过支付方式的有该值）
            sale_record_info[
                "origin_pay_type_text"
            ] = PaytypeModifyFunc.get_origin_paytype_text(
                session, sale_record.order_id
            )
            ...
            if (
                repeat_print
                and sale_record.pay_type == 9
                and order.customer_signature
            ):
                sale_record_info[
                    "signature_img"
                ] = CustomerSignature.get_signature_image(sale_record.order_id)
    ...
    def get_sales_record_pay_status_text(cls, record):
        ...
        :rtype: int
        """
        map_pay_status = {-1: "已作废", 1: "未结算", 2: "结算中", 3: "已结算", 4: "已退单"}
    ...
    def get_temp_sale_order_info(
        ...
        if tally_order_id:
            ...
            if tally_order and tally_order.erase_money > 0:
                ...
                sales_sum_count = (
                    session.query(func.count(models.GoodsSalesRecord.id))
                    .filter_by(order_id=tally_order_id)
                    .scalar()
                    or 0
                )
        ...
        if source == "salesman":
            temp_order_info = {
                ...
                "shop_name": shop_name,
                "total_recipt_money": NumFunc.check_float(
            ...
            combine_pay_type_text = (
                OrderBaseFunc.combine_pay_type_text(session, pay_order.id)
                if pay_type == 10
                else ""
            )
            ...
            accountant_time = (
                str(pay_order.pay_year)
                + "-"
                + str(pay_order.pay_month).zfill(2)
                + "-"
                + str(pay_order.pay_day).zfill(2)
                + " "
                + str(pay_order.pay_time)
            )
            ...
            if repeat_print and pay_type == 9 and tally_order_id:
                if tally_order.customer_signature:
                    temp_order_info[
                        "signature_img"
                    ] = CustomerSignature.get_signature_image(tally_order_id)
    ...
    def get_refund_goods_sale_record(cls, session, origin_record_id):
        """
            获取退单关联信息
        """
        refund_goods_sale_record = (
            session.query(models.GoodsSalesRecordRefundInfo)
    ...
    def get_sale_record_through_order(cls, session, order_id):
        ...
        record_list = (
            session.query(GoodsSalesRecord).filter_by(order_id=order_id).all()
        )
        return record_list
    ...
    def query_sales_record_by_billing_record(
        ...
        # 过滤店铺
        query_base = session.query(models.BillingSalesRecord).filter_by(
            shop_id=shop_id
        )
        # 通过收银信息判断小票是否已结算
        ...
        if only_refund or only_unpaid:
            query_base1 = query_base.join(
                GoodsSalesRecord,
                GoodsSalesRecord.id == BillingSalesRecord.record_id,
            ).filter(BillingSalesRecord.type_id == 0)
            if only_refund:
            ...
            query_base2 = query_base.join(
                GoodsSalesRecord,
                GoodsSalesRecord.multi_order_id == BillingSalesRecord.record_id,
            ).filter(BillingSalesRecord.type_id == 1)
            if only_refund:
        ...
        if all_single_record_id_list:
            single_goods_customer = (
                session.query(
                    GoodsSalesRecord.goods_id, GoodsSalesRecord.shop_customer_id
                )
        ...
        if record:
            ...
            if multi_record_id_list:
                ...
                if goods_ids:
                    ...
                    for _multi_order_id in multi_goods_ids_dcit:
                        ...
                        # 判断goods_id交集
                        if list(
                            (set(goods_ids).union(set(temp_goods_ids)))
            ...
            if goods_ids:
                ...
                if record2:
                    record2 = record2.filter(
                        GoodsSalesRecord.goods_id.in_(multi_goods_ids)
                    ).filter(
                        GoodsSalesRecord.multi_order_id.in_(
                            new_multi_record_id_list
                        )
            ...
            if record1 and record2:
                record = (
                    record1.union(record2)
                    .order_by(GoodsSalesRecord.id.desc())
                    .all()
                )
            elif record1:
    ...
    def query_sales_record_by_billing_record_new(
        ...
        if show_type == 1:
            ...
            filter_param_one = (
                GoodsSalesRecord.account_period_date >= start_date
            )
        ...
        first_join = [
            ...
            and_(
                or_(
                    and_(
                        BillingSalesRecord.record_id == GoodsSalesRecord.id,
                        BillingSalesRecord.type_id == 0,
                    ),
                    and_(
                        BillingSalesRecord.record_id
                        == GoodsSalesRecord.multi_order_id,
        ...
        if not (
            ...
            query_base = session.query(models.BillingSalesRecord).filter(
                *base_filter_list
            )
        ...
        # 要支持货品id或者操作人筛选，那么需要join
            ...
            base_query = (
                session.query(BillingSalesRecord, GoodsSalesRecord)
                .join(*first_join)
            ...
            if export:
                all_goods_record = (
                    base_query.filter(*diy_filter_list)
                    .filter(*goods_diy_filter_list)
                    .order_by(GoodsSalesRecord.id.desc())
                    .all()
                )
                ...
                result_query = (
                    session.query(BillingSalesRecord.id.distinct())
                    .join(*first_join)
                    .filter(*base_filter_list)
                    .filter(*diy_filter_list)
                    .filter(*goods_diy_filter_list)
                )
                page_sum = math.ceil(result_query.count() / page_size)
                all_goods_record = (
                    result_query.order_by(GoodsSalesRecord.id.desc())
        ...
        if account_sales_id_list:
            account_name_dict = AccountFunc.get_account_name_through_id_list(
                session, account_sales_id_list
            )
        else:
        ...
        if multi_record_order_id_list:
            order_num_multi_order_id_map = OrderBaseFunc.get_order_num_through_id_list(
                session, multi_record_order_id_list
            )
        ...
        if return_billing_record_id:
            ...
            for _id in return_billing_record_id:
                ...
                billing_record = billing_record_dict.get(
                    _id
                )  # type: models.BillingSalesRecord
                is_multi = billing_record.type_id
                accounting = (
                    1
                    if billing_record.account_period_date
                    and billing_record.account_period_date
                    > datetime.date.today()
                    else 0
                )
                # 单品
                if is_multi == 0:
                    ...
                    data_dict = {
                        "accountant_date_time": TimeFunc.time_to_str(
                            first_goods_record.order_time, "date"
                        )
                        + " "
                        ...
                        "accountant_name": account_name_dict.get(
                            billing_record.accountant_id, ""
                        ),
                        "accountant_time": TimeFunc.time_to_str(
                            first_goods_record.order_time, "time"
                        ),
                        # "accountant_wximgurl": "-",
                        "accounting": accounting,
                        "batch_num": "-",
                        "bill_time": TimeFunc.time_to_str(
                            billing_record.bill_time, "time"
                        ),
                        "bill_time_full": (
                            TimeFunc.time_to_str(
                                billing_record.bill_date, "date"
                            )
                            + " "
                            + TimeFunc.time_to_str(
                                billing_record.bill_time, "time"
                        ...
                        "commission_mul": check_float(
                            first_goods_record.get_commission_mul() / 100
                        ),
                        "commission_sumup": check_float(
                            first_goods_record.get_commission_money() / 100
                        ),
                        "customer_name": customer_name_dict.get(
                            first_goods_record.shop_customer_id
                        ),
                        "deposit_total": check_float(
                            first_goods_record.get_deposit_money() / 100
                        ),
                        ...
                        "goods_sumup": check_float(
                            first_goods_record.get_sales_money() / 100
                        ),
                        "goods_supply_type": goods_supply_type_dict.get(
                            first_goods_record.goods_id
                        ),
                        "goods_supply_type_text": goods_supply_type_text_dict.get(
                            first_goods_record.goods_id
                        ),
                        "goods_unit_text": goods_unit_text_dict.get(
                            first_goods_record.goods_id
                        ),
                        ...
                        "pay_type_text": first_goods_record.pay_type_dict.get(
                            first_goods_record.pay_type
                        ),
                        "receipt_money": check_float(
                            first_goods_record.get_receipt_money() / 100
                        ),
                        "record_type": first_goods_record.record_type,
                        "remark": first_goods_record.remark,
                        "sales_num": check_float(
                            first_goods_record.get_sales_num(
                                goods_unit, weight_unit_text=weight_unit_text
                            )
                        )
                        if goods_unit == 0
                        else "-",
                        "salesman_name": account_name_dict.get(
                            billing_record.salesman_id, ""
                        ),
                        "salesman_id": billing_record.salesman_id,
                        "supplier_name": "-",
                        "unit_price": check_float(
                            first_goods_record.get_fact_price(
                                goods_unit, weight_unit_text=weight_unit_text
                            )
                            / 100
                        ),
                ...
                # 多品
                    ...
                    for item in goods_records:
                        ...
                        temp_dict = {
                            ...
                            "commission_mul": check_float(
                                item.get_commission_mul() / 100
                            ),
                            "commission_sumup": check_float(
                                item.get_commission_money() / 100
                            ),
                            "customer_name": customer_name_dict.get(
                                item.shop_customer_id
                            ),
                            "deposit_total": check_float(
                                item.get_deposit_money() / 100
                            ),
                            ...
                            "pay_type": billing_record.pay_type,
                            "pay_type_text": item.pay_type_dict.get(
                                item.pay_type
                            ),
                            "receipt_money": check_float(
                                item.get_receipt_money() / 100
                            ),
                    ...
                    data_dict = {
                        ...
                        "full_num": order_num_multi_order_id_map.get(
                            first_goods_record.multi_order_id, "-"
                        ),
                        "goods_count": count,
                        "goods_name": goods_name_dict.get(
                            first_goods_record.goods_id, ""
    ...
    def count_unpaid_billing_record_today(cls, session, shop_id):
        ...
        count_single = (
            session.query(func.count(models.BillingSalesRecord.id))
            .join(
                models.GoodsSalesRecord,
                models.GoodsSalesRecord.id
                == models.BillingSalesRecord.record_id,
            )
        ...
        count_multi = (
            session.query(
                func.count(func.distinct(models.BillingSalesRecord.record_id))
            )
            .join(
                models.GoodsSalesRecord,
                models.GoodsSalesRecord.multi_order_id
                == models.BillingSalesRecord.record_id,
            )
        ...
        for r in single_record_list:
            ...
            if not simplify:
                data = dict(
                    ...
                    accounting=1
                    if r.account_period_date
                    and r.account_period_date > current_date
                    else 0,
                    # 冗余一下，因为接口名字被改了
                    is_after_accounting=1
                    if r.account_period_date
                    ...
                    remark=r.remark,
                    bill_time_full=(
                        TimeFunc.time_to_str(r.bill_date, "date")
                        + " "
            ...
            if r.accountant_id:
                ...
                data["accountant_name"] = (
                    accountant.realname or accountant.nickname
                    if accountant
                    else ""
                )
                data["accountant_wximgurl"] = (
                    accountant.head_imgurl_small
                    if accountant.wx_unionid
                    else ""
                )
                ...
                data["accountant_date_time"] = (
                    TimeFunc.time_to_str(r.order_time, "date")
                    + " "
            ...
            data["salesman_wximgurl"] = (
                salesman.head_imgurl_small if salesman.wx_unionid else ""
            )
    ...
    def _parse_multi_records(
        ...
        for record in multi_record_list:
            map_multi_records.setdefault(record.multi_order_id, []).append(
                record
            )
    ...
    def _merge_multi_data(
        ...
        is_after_accounting = (
            ...
            and record.account_period_date > datetime.date.today()
            else 0
        )
        ...
        multi_data = dict(
            ...
            commission_mul=NumFunc.check_float(
                record.get_commission_mul() / 100
            ...
            is_multi=1,
            unit_price="-",
            goods_sumup=NumFunc.check_float(
                record.get_sales_money() / 100
        ...
        if not multi_data.get("salesman_name"):
            ...
            multi_data["salesman_headimg"] = (
                salesman.head_imgurl if salesman else ""
            )
            multi_data["salesman_wximgurl"] = (
                salesman.head_imgurl_small if salesman.wx_unionid else ""
            )
            if record.accountant_id:
                ...
                multi_data["accountant_name"] = (
                    accountant_object.realname or accountant_object.nickname
                    if accountant_object
                    else ""
                )
                multi_data["accountant_wximgurl"] = (
                    accountant_object.head_imgurl_small
                    if accountant_object.wx_unionid
                    else ""
                )
    ...
    def list_sales_each_goods_salesman(
        ...
        if data_type == 1:
            sTable = (
                models_account_periods_statistic.AccountPeriodsStatisticSalesGoodsSalesman
            )
        else:
        ...
        query_base = (
            statistic_session.query(
                ...
                func.sum(
                    sTable.sales_pay_count + 2 * sTable.refund_count
                ).label("count"),
                *columns
            )
            .filter_by(
                shop_id=shop_id, statistic_type=1, salesman_id=salesman_id
            )
            .filter(sTable.goods_id.in_(master_goods_id_set))
            .having(
                and_(
        ...
        query_result = (
            query_base.group_by(sTable.goods_id)
            .order_by(sTable.goods_id)
        ...
        for q in query_result:
            ...
            if goods.unit == 0:
                ...
                avg_price = (
                    q.total_goods_cent / q.commission_mul
                    if q.commission_mul
                    else 0
                )
            data = {
    ...
    def get_sum_query_base(cls, session):
        ...
        result = session.query(
            ...
            ).label("unpayback_money"),
            func.sum(
                func.IF(  # 单位为斤或者Kg时才统计，否则为0
                    GoodsSalesRecord.record_type == 0,
        ...
        if sub_multi:
            ...
            for record in multi_record_list:
                ...
                multi_single_result = GoodsSaleFunc._parse_single_records(
                    [record], goods_info_dict, session, simplify=False
                )[0]
                for multi in multi_result:
    ...
    def nullify_order(cls, session, order_id, operator_id, shop_id):
        ...
        order = (
            session.query(models.Order)
            .filter_by(id=order_id, shop_id=shop_id)
            .first()
        )
        if not order:
            return False, "订单不存在"
        ...
        records = (
            session.query(models.GoodsSalesRecord)
            .filter_by(shop_id=shop_id, multi_order_id=order.id)
            .all()
        )
        record_id_list = []
        for r in records:
    ...
    def nullify_sales_record(
        ...
        if not is_multi:
            record = (
                session.query(models.GoodsSalesRecord)
                .filter_by(id=record_ids[0], shop_id=shop_id)
                .first()
            )
            if not record:
        ...
        session.query(models.GoodsSalesRecord).filter(
            models.GoodsSalesRecord.id.in_(record_ids)
    ...
    def query_and_parse_refunds(cls, session, sales_record_id_list):
        ...
        refund_infos = (
            session.query(models.GoodsSalesRecordRefundInfo)
            .filter(
                models.GoodsSalesRecordRefundInfo.goods_sale_record_id.in_(
                    sales_record_id_list
                )
            )
            .all()
            if sales_record_id_list
            else []
        )
    ...
    @classmethod
    def get_record_shown_status_print(
        cls, record, refund_record_ids, origin_record_ids
    ):
    ...
    @classmethod
    def get_allstatus_salerecord_list(
        cls,
        session,
        shop_id,
        choose_date,
        ...
        if is_branch:
            query_base = session.query(GoodsSalesRecord).filter_by(
                shop_id=shop_id
            )
        else:
            query_base = (
                session.query(GoodsSalesRecord)
                .filter_by(shop_id=shop_id)
        ...
        if debt_id:
            # 通过debt_id反查还款id
            pb_order_id = (
                session.query(models.DebtHistory.fk_id)
            ...
            # 通过还款记录反查流水id
            all_goods_sale_record = (
                session.query(
                    models.GoodsSalesRecordPaybackRecord.goods_sale_record_id
                )
...
class MultiOrderFunc:
    ...
    def get_multi_goods_sale_record_order_id(cls, session, order_id):
        ...
        Order = models.Order
        multi_order_list = (
            session.query(Order).filter_by(tally_order_id=order_id).all()
        )
    ...
    def get_id_dict_through_sales_record(cls, session, record_id_list):
        GoodsSalesRecord = models.GoodsSalesRecord
        multi_id_list = (
            session.query(GoodsSalesRecord.id, GoodsSalesRecord.multi_order_id)
            .filter(
...
class DebtFunc:
    ...
    def update_debt(
        ...
        debtor = (
            session.query(models.ShopCustomer).filter_by(id=debtor_id).first()
        )
        shop = (
            session.query(models.Shop)
            .filter_by(id=shop_id)
...
class AccoutantFunc:
    ...
    def get_current_work_record(cls, session, accountant_id, shop_id):
        work_record = (
            session.query(models.AccountantWorkRecord)
        ...
        if not work_record or (work_record and work_record.offduty_time):
            ...
            try:
                hr = (
                    session.query(models.HireLink)
                    .filter_by(account_id=accountant_id, shop_id=shop_id)
                    .first()
                )
                session.commit()
    ...
    @classmethod
    def get_timebefore_newest_work_record(
        cls, session, accountant_id, shop_id, off_datetime
    ):
        work_record = (
            session.query(models.AccountantWorkRecord)
            .filter_by(accountant_id=accountant_id, shop_id=shop_id)
    ...
    def update_work_record(
        ...
        if pay_type == "pb_combine":
            ...
            pb_total_pay_cent = 0
            cash_cent = (
                order_money.cash_cent
                if not is_reset
                else -order_money.cash_cent
            )
            pos_cent = (
                order_money.pos_cent if not is_reset else -order_money.pos_cent
            )
            tag_ali_cent = (
                order_money.tag_ali_cent
                if not is_reset
                else -order_money.tag_ali_cent
            )
            tag_wx_cent = (
                order_money.tag_wx_cent
                if not is_reset
                else -order_money.tag_wx_cent
            )
            cheque_cent = (
                order_money.cheque_cent
                if not is_reset
                else -order_money.cheque_cent
            )
            remittance_cent = (
                order_money.remittance_cent
                if not is_reset
                else -order_money.remittance_cent
            )
            if cash_cent:
        ...
        if pay_type in [
            "deposit_refund_cash",
            "deposit_refund_pos",
            "deposit_refund_alipay",
            "deposit_refund_wx",
            "deposit_refund_cheque",
            "deposit_refund_remittance",
        ]:
        ...
        if pay_type in [
            ...
            work_record.payout_count += count
        if pay_type in [
            "supplier_borrow_cash",
            "supplier_borrow_pos",
            "supplier_borrow_alipay",
            "supplier_borrow_wx",
            "supplier_borrow_cheque",
            "supplier_borrow_remittance",
        ]:
        ...
        if pay_type in [
            "waste_selling_income",
            "waste_selling_income_cash",
            "waste_selling_income_pos",
            "waste_selling_income_alipay",
            "waste_selling_income_wx",
            "waste_selling_income_cheque",
    ...
    def get_word_record_data(cls, work_record):
        if work_record.offduty_time:
            delta_day = (
                work_record.offduty_time - work_record.onduty_time
            ).days
            delta_seconds = (
                work_record.offduty_time - work_record.onduty_time
            ).seconds
            ...
            delta_seconds = (
                datetime.datetime.now() - work_record.onduty_time
            ).seconds
        ...
        data["offduty_time"] = (
            work_record.offduty_time.strftime("%Y-%m-%d %H:%M:%S")
            if work_record.offduty_time
            else "--"
        )
        ...
        data["order_cash_count"] = (
            work_record.order_cash_count + work_record.order_combine_cash_count
        )
        data["order_pos_money"] = check_float(
            work_record.order_pos_cent / 100
            + work_record.order_combine_pos_cent / 100
        )
        data["order_pos_count"] = (
            work_record.order_pos_count + work_record.order_combine_pos_count
        )
        data["order_ali_money"] = check_float(
            work_record.order_ali_cent / 100
            + work_record.order_combine_ali_cent / 100
        )
        data["order_ali_count"] = (
            work_record.order_ali_count + work_record.order_combine_ali_count
        )
        ...
        data["order_wx_count"] = (
            work_record.order_wx_count + work_record.order_combine_wx_count
        )
        data["order_cheque_money"] = check_float(
            work_record.order_cheque_cent / 100
            + work_record.order_combine_cheque_cent / 100
        )
        data["order_cheque_count"] = (
            work_record.order_cheque_count
        ...
        data["order_remittance_count"] = (
            work_record.order_remittance_count
        ...
        data["order_bank_count"] = (
            data["order_pos_count"] + data["order_remittance_count"]
        )
        ...
        data["pb_cash_count"] = (
            work_record.pb_cash_count + work_record.pb_combine_cash_count
        )
        ...
        data["pb_pos_count"] = (
            work_record.pb_pos_count + work_record.pb_combine_pos_count
        )
        ...
        data["pb_ali_count"] = (
            work_record.pb_ali_count + work_record.pb_combine_ali_count
        )
        data["pb_wx_money"] = check_float(
            work_record.pb_wx_cent / 100 + work_record.pb_combine_wx_cent / 100
        )
        data["pb_wx_count"] = (
            work_record.pb_wx_count + work_record.pb_combine_wx_count
        )
        ...
        data["pb_cheque_count"] = (
            work_record.pb_cheque_count + work_record.pb_combine_cheque_count
        )
        ...
        data["pb_remittance_count"] = (
            work_record.pb_remittance_count
        ...
        data["pb_bank_count"] = (
            data["pb_pos_count"] + data["pb_remittance_count"]
        )
        ...
        data["refund_bank_count"] = (
            data["refund_pos_count"] + data["refund_remittance_count"]
        )
        ...
        data["payout_total_count"] = (
            work_record.deposit_refund_count
            + work_record.payout_count
            + work_record.supplier_borrow_count
        )
        ...
        data["payout_cash_count"] = (
            work_record.deposit_refund_cash_count
            + work_record.payout_cash_count
            + work_record.supplier_borrow_cash_count
        )
        ...
        data["payout_pos_count"] = (
            work_record.deposit_refund_pos_count
            + work_record.payout_pos_count
            + work_record.supplier_borrow_pos_count
        )
        ...
        data["payout_ali_count"] = (
            work_record.deposit_refund_ali_count
            + work_record.payout_ali_count
            + work_record.supplier_borrow_ali_count
        )
        ...
        data["payout_wx_count"] = (
            work_record.deposit_refund_wx_count
            + work_record.payout_wx_count
            + work_record.supplier_borrow_wx_count
        )
        ...
        data["payout_cheque_count"] = (
            work_record.deposit_refund_cheque_count
            + work_record.payout_cheque_count
            + work_record.supplier_borrow_cheque_count
        )
        ...
        data["payout_remittance_count"] = (
            work_record.deposit_refund_remittance_count
        ...
        data["payout_bank_count"] = (
            data["payout_pos_count"] + data["payout_remittance_count"]
        )
        ...
        data["income_total_count"] = (
            work_record.other_income_count
        ...
        data["income_cash_count"] = (
            work_record.other_income_cash_count
        ...
        data["income_pos_count"] = (
            work_record.other_income_pos_count
        ...
        data["income_ali_count"] = (
            work_record.other_income_ali_count
        ...
        data["income_wx_count"] = (
            work_record.other_income_wx_count
        ...
        data["income_cheque_count"] = (
            work_record.other_income_cheque_count
            + work_record.waste_selling_income_cheque_count
        )
        ...
        data["income_remittance_count"] = (
            work_record.other_income_remittance_count
            + work_record.waste_selling_income_remittance_count
        )
        data["income_bank_money"] = check_float(
            data["income_pos_money"] + data["income_remittance_money"]
        )
        data["income_bank_count"] = (
            data["income_pos_count"] + data["income_remittance_count"]
        )

        # 现金清点金额
        cash_expect_money = check_float(
            (
                work_record.order_cash_cent
                ...
                - work_record.supplier_borrow_cash_cent
            )
            / 100
        )
        ...
        data["erase_bank_count"] = (
            data["erase_pos_count"] + data["erase_remittance_count"]
        )
        ...
        ] = work_record.order_fee_remittance_count
        data["order_fee_bank_money"] = check_float(
            data["order_fee_pos_money"] + data["order_fee_remittance_money"]
        )
        data["order_fee_bank_count"] = (
            data["order_fee_pos_count"] + data["order_fee_remittance_count"]
        )

        # 实收资金
        data["total_money"] = check_float(
            (
                work_record.order_total_cent
                ...
                - work_record.supplier_borrow_cent
            )
            / 100
        )
        data["total_count"] = (
        ...
        data["total_pos_money"] = check_float(
            (
                ...
                - work_record.supplier_borrow_pos_cent
            )
            / 100
        )
        data["total_pos_count"] = (
            data["order_pos_count"]
            + data["pb_pos_count"]
            + data["income_pos_count"]
            + data["payout_pos_count"]
        ...
        data["total_ali_money"] = check_float(
            (
                ...
                - work_record.supplier_borrow_ali_cent
            )
            / 100
        )
        data["total_ali_count"] = (
            data["order_ali_count"]
            + data["pb_ali_count"]
            + data["income_ali_count"]
            + data["payout_ali_count"]
        ...
        data["total_wx_money"] = check_float(
            (
                ...
                - work_record.supplier_borrow_wx_cent
            )
            / 100
        )
        data["total_wx_count"] = (
        ...
        data["total_cheque_money"] = check_float(
            (
                ...
                - work_record.supplier_borrow_cheque_cent
            )
            / 100
        )
        data["total_cheque_count"] = (
            data["order_cheque_count"]
            + data["pb_cheque_count"]
            + data["income_cheque_count"]
            + data["payout_cheque_count"]
        ...
        data["total_remittance_money"] = check_float(
            (
                work_record.order_remittance_cent
                ...
                - work_record.supplier_borrow_remittance_cent
            )
            / 100
        )
        ...
        data["total_bank_count"] = (
            data["total_pos_count"] + data["total_remittance_count"]
        )
        return data
...
class OrderBaseFunc:
    ...
    def gen_order_num(cls, session, shop_id, source="order"):
        ...
        if redis.get(key):
            ...
            if (source == "multi_order" and len(order_num) != 9) or (
                source != "multi_order" and len(order_num) != 11
            ):
                redis.delete(key)
                return None
            # TODO END
            ...
            latest_order_num = (
                session.query(models_order.num)
            ...
            if source == "multi_order":
                max_order_num = (
                    session.query(models_order.num, models_order.multi_goods)
                    .filter_by(shop_id=shop_id)
            ...
            # 初始化单号
            init_order_num = "{prefix}{shop_id}000001".format(
                prefix=prefix, shop_id=shop_id
            )
            if latest_order_num:
    ...
    def gen_short_num(cls, session, shop_id, source="payout"):
        ...
        if redis.get(key):
            ...
            db_model = (
                session.query(models.PayoutShortNum.num)
                .filter_by(shop_id=shop_id)
                .order_by(desc(models.PayoutShortNum.id))
                .first()
            )
            num = "10001"
    ...
    @classmethod
    def redis_save_order_lock(
        cls, shop_id, front_end_pay_id, last_update_timestamp, if_pborder=False
    ):
        """防止并发生成订单
        """
        if if_pborder:
            order_lock_redis_key = (
                "pb_order_frontid_updatetime_lock:%d:%s:%d"
    ...
    def finish_order(cls, session, order, total_fee=0, if_pborder=False):
        ...
        order_finish_redis_tag_twice_verify = (
            "finish_order_twice_verify:%s" % order.num
        )
        # print(order_finish_redis_tag)
        if if_pborder:
            order_finish_redis_tag = "pb_" + order_finish_redis_tag
            order_finish_redis_tag_twice_verify = (
                "pb_" + order_finish_redis_tag_twice_verify
            )
        ...
        if if_pborder:
            ...
            session.query(GoodsSalesRecordPaybackRecord).filter_by(
                pb_order_id=order_id
            ).update({"status": 1})

            # 处理小票剩余未还款金额
            goods_sale_record_id_list = (
                session.query(
                    GoodsSalesRecordPaybackRecord.goods_sale_record_id,
            ...
            debt_record_id_list = (
                session.query(
                    HistoryPaybackRecord.debt_history_id,
            ...
            billing_query = session.query(models.BillingSalesRecord).filter(
                models.BillingSalesRecord.shop_id == shop_id
            )
            ...
            if multi_order_id_list:
                session.query(models.Order).filter(
                    models.Order.id.in_(multi_order_id_list)
                ).update(
                    {
                        "tally_order_id": order.id,
                ...
                ).filter(models.BillingSalesRecord.type_id == 1).update(
                    {
                        "accountant_id": order.accountant_id,
                        "account_period_date": account_period_date,
            ...
            if single_order_id_list:
                billing_query.filter(
                    models.BillingSalesRecord.record_id.in_(
                        single_order_id_list
                    )
                ).filter(models.BillingSalesRecord.type_id == 0).update(
                    {
                        "accountant_id": order.accountant_id,
                        "account_period_date": account_period_date,
                        "pay_type": order.pay_type,
        ...
        if order.erase_money:
            order_erase = (
                session.query(models.OrderErase)
                .filter_by(order_id=order.id, order_type=order_type)
                .first()
            )
            if order_erase:
        ...
        if if_pborder or order.pay_type != 9:
            fund_type, fund_value, accountant_id = (
                order.pay_type,
                total_fee,
                accountant_id,
            )
        ...
        try:
            front_num_redis_key = "front_num:{shop_id}:{front_num}:{fact_total_price}".format(
                shop_id=shop_id,
                front_num=order.frontend_num,
                fact_total_price=order.fact_total_price,
            )
    ...
    def update_sales_record(
        ...
        record.order_time = (
            order_time if order_time else datetime.datetime.now()
        )
    ...
    # 收银台结算完成之后发送短信
    @classmethod
    def send_message_after_settlement(
        cls, session, order, if_pborder, is_refund=0
    ):
        customer_id = order.debtor_id
        shop_id = order.shop_id
        accountant_id = order.accountant_id
        salersman_settlement_send_message_key = (
            "salersman_settlement_send_message_key:%d" % (order.id)
        )
        ...
        if customer_id:
            ...
            accountant_name = (
                session.query(
                    models.Accountinfo.realname, models.Accountinfo.nickname
                )
                .filter_by(id=accountant_id)
                .first()
            )
            shop_name = (
                session.query(models.Shop.shop_name)
            ...
            point_display = (
                session.query(models.Config.point_display)
                .filter_by(id=shop_id)
                .scalar()
                or 0
            )
            ...
            if redis.get(salersman_settlement_send_message_key):
                # 给客户发送短信
                from libs.yunpian import (
                    send_yunpian_accountant_settlement_payback,
    ...
    def get_order_money(cls, session, order_id, source_type=1):
        ...
        order_money = session.query(models_name).filter_by(id=order_id).first()
        return order_money

    @classmethod
    def get_order_money_through_id_list(
        cls, session, order_id_list, source_type="order"
    ):
        if order_id_list:
    ...
    # 获取订单条码支付类型
    @classmethod
    def get_order_online_type(
        self, session, order, if_front=False, if_pborder=False
    ):
        pay_type = ""
        ...
        if order.pay_type == 3:
            ...
            order_money = (
                session.query(OrderMoney).filter_by(id=order_id).first()
            )
            if order_money.barcode_wx_cent:
                pay_type = pay_type_wx
            if order_money.barcode_ali_cent:
    ...
    @classmethod
    def barpay_lcsw_order(
        cls, session, shop_id, auth_code, order, if_pborder=False
    ):
        ret_dict = {}
    ...
    def wappay_url_lcsw(cls, session, shop_id, shop_name, total_fee, wap_order):
        ...
        headers = {"content-type": "application/json"}
        parameters = LcswPay.getWapPayParas(
            lc_merchant_no,
            lc_terminal_id,
            terminal_trace,
            total_fee,
    ...
    @classmethod
    def multi_combine_pay_type_text(
        cls, session, order_id_list, source_type="order"
    ):
        """ 批量获取组合支付类型
    ...
    # 批量获取组合支付详情
    @classmethod
    def combine_pay_type_detail_through_id_list(
        cls, session, order_id_list, return_type="text", source_type="order"
    ):
    ...
    def get_order_num_throuth_id(cls, session, order_id):
        ...
        """
        order_num = (
            session.query(models.Order.num).filter_by(id=order_id).scalar()
            or ""
        )
        return order_num
    ...
    def get_order_num_through_id_list(cls, session, order_id_list):
        ...
        if order_id_list:
            Order = models.Order
            order_num_list = (
                session.query(Order.id, Order.num)
    ...
    def get_signature_image(cls, order_id):
        ...
        try:
            filename = (
                "/home/monk/pf_signatures/OrderPaidWithCredit/%s.png" % order_id
            )
    ...
    def get_order_bank_name(cls, session, order_id, source="order"):
        ...
        bank_no = (
            session.query(models_name.bank_no)
            .filter_by(id=order_id, pay_type=12)
            .scalar()
            or 0
        )
    ...
    def check_pb_order_reset(cls, session, order_id_list):
        query_data = (
            session.query(models.PbOrder, models.PbOrderMoney)
            .outerjoin(
                models.PbOrderMoney, models.PbOrder.id == models.PbOrderMoney.id
            )
        ...
        for pborder, pborder_money in query_data:
            ...
            if (
                ...
                and pborder_money
                and (
                    pborder_money.barcode_wx_cent > 0
                    or pborder_money.barcode_ali_cent > 0
                )
            ):
                can_be_reset = 0
...
class OrderSalesRecordFunc:
    ...
    def batch_format_order_record_info(
        ...
        if attach_refund:
            ...
            refund_record_ids, origin_record_ids, map_refund_id_to_origin_id = GoodsSaleFunc.query_and_parse_refunds(
                session, sales_record_id_list
            )
        ...
        if not get_wximgurl:
            ...
            staff_name_dict, staff_wximgurl_dict = AccountFunc.get_account_name_wximg_through_id_list(
                session, account_id_list
            )
        customer_name_dict = ShopCustomerFunc.get_shop_customer_name_through_id_list(
            session, customer_id_list
        ...
        for record in sale_records:
            ...
            sales_time = (
                str(record.bill_year)
                + "-"
                + str(record.bill_month).zfill(2)
                + "-"
                + str(record.bill_day).zfill(2)
                + " "
                + str(record.bill_time)
            )
            ...
            shop_supplier_name = (
                record.shop_supplier_name
                if goods_info and goods_info["supply_type"] > 0
                else ""
            )
            ...
            if (
                pay_type == 9
                and unpayback_money >= 0
                and record.status in [3, 5]
            ):
            ...
            if record.account_period_date:
                ...
                if (
                    assign_accounting_date
                    and (
                        record.account_period_date - assign_accounting_date
                    ).days
                    == 1
                ):
                    accounting = 1
            ...
            record_info["sales_num"] = (
                "-" if goods_info["unit"] == 1 else sales_num
            )  # 已售重量
            record_info["gross_weight"] = check_float(
                record.get_gross_weight(weight_unit_text=weight_unit_text)
            )  # 毛重
            record_info["tare_weight"] = check_float(
                record.get_tare_weight(weight_unit_text=weight_unit_text)
            )  # 皮重
            record_info["storage"] = check_float(record.storage / 100)  # 剩余件数
            record_info["storage_num"] = check_float(
                record.get_storage_num(weight_unit_text=weight_unit_text) / 100
            )  # 剩余重量
            ...
            record_info["batch_num"] = (
                goods_batch_num_dict.get(record.goods_batch_id, "")
                if record.goods_batch_id
                else ""
            )  # 批次号
            ...
            if attach_refund:
                record_info[
                    "shown_status_print"
                ] = GoodsSaleFunc.get_record_shown_status_print(
...
class StorageRecordFunc:
    @classmethod
    def batch_format_storage_record_info(
        cls,
        session,
        record_list,
        shop_id,
    ...
    @classmethod
    def format_export_storage_record_body(
        cls, session, record_list, shop_id, goods_id, weight_unit_text
    ):
        """构造某货品库存变更记录的导出excel格式"""
        data_content = []
        data_dict = StorageRecordFunc.batch_format_storage_record_info(
            session,
            record_list,
            shop_id,
            goods_id,
        ...
        for storage_record in record_list:
            ...
            if record_info["type_id"] in (1, 4):
                foreign_record_num_info = (
                    "入库单号：" + record_info["foreign_record_num"]
                )
            elif record_info["type_id"] in (6, 7):
                foreign_record_num_info = (
                    "报损单号：" + record_info["foreign_record_num"]
                )
            elif record_info["type_id"] in (8, 9, 10):
                foreign_record_num_info = (
                    "调货单号：" + record_info["foreign_record_num"]
                )
            elif record_info["type_id"] in (11,):
                foreign_record_num_info = (
                    "盘点单号：" + record_info["foreign_record_num"]
                )
            else:
            ...
            data_content.append(
                [
                    ...
                    + str(record_info["num"])
                    + weight_unit_text,
                    str(record_info["now_storage"])
                    + "件/"
                    + str(record_info["now_storage_num"])
                    + weight_unit_text,
                    foreign_record_num_info,
                ]
            )
    ...
    def get_storage_data(
        cls, args, session, statistic_session, shop_id, master_goods_ids=None
    ):
        date = args["date"]
        end_date = args.get("end_date", "")
        data_type = args.get("data_type", 0)
        ...
        # 区分自然日/扎账日
        if data_type == 1:
            StatisticGoodsStorage = (
                models_account_periods_statistic.AccountPeriodsStatisticGoodsStorage
            )
        else:
        ...
        if statistic_type == 1:
            ...
            storage_data_ret = query_base.filter(
        ...
        # 暂时只考虑年统计
            ...
            query_base = (
                statistic_session.query(
                    StatisticGoodsStorage.goods_id, *columns
                )
                .group_by(StatisticGoodsStorage.goods_id)
                .filter_by(shop_id=shop_id, statistic_type=filter_type)
            )
        ...
        storage_data_ret = storage_data_ret.filter(
            or_(
                StatisticGoodsStorage.stockin_record != 0,
        ...
        if group_ids:
            goods_in_group = (
                session.query(models.Goods)
                .filter(
                    models.Goods.group_id.in_(group_ids),
                    models.Goods.shop_id == shop_id,
                )
                .all()
            )
            storage_data_ret = storage_data_ret.filter(
                StatisticGoodsStorage.goods_id.in_(
                    [x.id for x in goods_in_group]
                )
            )
        # 过滤供应商ID
        if supplier_ids:
            ...
            storage_data_ret = storage_data_ret.filter(
                StatisticGoodsStorage.goods_id.in_(supplier_goods_id_list)
            )
        # 过滤销售类型
        if supply_types:
            supply_goods_id_list = GoodsFunc.get_goods_through_supply_type(
            ...
            storage_data_ret = storage_data_ret.filter(
                StatisticGoodsStorage.goods_id.in_(supply_goods_id_list)
            )
        # 过滤货品类型
        if goods_types:
            type_goods_id_list = GoodsFunc.get_goods_through_goods_type(
            ...
            storage_data_ret = storage_data_ret.filter(
                StatisticGoodsStorage.goods_id.in_(type_goods_id_list)
            )
    ...
    def get_begin_end_storage(cls, args, session, shop_id, goods_id_list):
        ...
        end_stoarge_id_querys = (
            ...
            .filter(
                GoodsStorageRecord.shop_id == shop_id,
        ...
        end_storage_list = (
            ...
            .filter(GoodsStorageRecord.id.in_(end_id_list))
            .all()
            if end_id_list
            else []
        )
        begin_storage_list = (
            ...
            .filter(GoodsStorageRecord.id.in_(begin_id_list))
            .all()
            if begin_id_list
            else []
        )
    ...
    @classmethod
    def get_extra_storage_data(
        cls, args, session, shop_id, storage_data, data_sales_ret
    ):
    ...
    def get_storage_allocate_end_time_by_storage_allocate_id(
        ...
        result = (
            session.query(GoodsStorageRecord.create_time)
            .filter(*filter_list)
            .order_by(GoodsStorageRecord.create_time.desc())
            .first()
        )
        if result:
...
class SupplierFlowFunc:
    @classmethod
    def batch_format_supplier_flow_info(
        cls, session, shop_id, flow_list, return_type="list", data_type=0
    ):
        """
            批量处理贷款结算记录的数据
            :param shop_id: 店铺id
        ...
        operater_name_dict, operater_wximgurl_dict = (
            operater_name_wximgurl_list[0],
            operater_name_wximgurl_list[1],
        )
        # 查询供货商信息
        ...
        accounting_time = (
            AccountingPeriodFunc.get_accounting_time(session, shop_id)
            if data_type == 1
            else None
        )
        # 查询货款结算手续费
        ...
        for flow in flow_list:
            ...
            data = {
                ...
                "operater_wximgurl": operater_wximgurl_dict.get(
                    flow.operater_id, ""
                ),
                "flow_id": flow_id,
                ...
                "is_after_accounting": 1
                if accounting_time and flow.create_date > accounting_time
                else 0,
                "procedure_fee": procedure_dict.get(flow_id)
                if flow.flow_type == 0
                else 0,
            }
            if return_type == "dict":
    ...
    def _get_multi_clearing_goods(cls, session, clearing_id_list):
        ...
        clearing_id_list = (
            session.query(
                ShopSupplierClearingGoods.clearing_id,
                ShopSupplierClearingGoods.goods_id,
            )
            .filter(ShopSupplierClearingGoods.clearing_id.in_(clearing_id_list))
            .all()
        )
    ...
    def _get_clearing_procedure_fee(cls, session, clearing_id_list):
        ...
        procedure_dict = {
            x.id: check_float(x.procedure_cent / 100) for x in procedure_list
        }
    ...
    def _get_sum_clearing_procedure_fee(cls, session, clearing_id_list):
        ...
        if clearing_id_list:
            procedure_sum = (
                session.query(
                    func.sum(models.ShopSupplierClearing.procedure_cent)
                )
                .filter(models.ShopSupplierClearing.id.in_(clearing_id_list))
                .scalar()
                or 0
    ...
    @classmethod
    def batch_format_borrowing_flow_info(
        cls, session, shop_id, flows_borrowing_records, data_type=0
    ):
        """ 构造预支流水的返回字典 """

        # 查询操作员信息
        operater_id_list = {
            flow.operater_id for flow, _ in flows_borrowing_records
        }
        operator_name_dict, operator_wximgurl_dict = AccountFunc.get_account_name_wximg_through_id_list(
            session, operater_id_list
        )
        # 查询供货商信息
        supplier_id_list = {
            flow.shop_supplier_id for flow, _ in flows_borrowing_records
        }
        supplier_info_dict = SupplierFunc.get_shop_supplier_base_info(
            session, shop_id, supplier_id_list
        )
        ...
        for flow, record in flows_borrowing_records:
            ...
            cancelable = (
                record.create_date.date() == datetime.date.today()
                and record.status == 0
            )
            data_list.append(
                {
                    ...
                    "operater_name": operator_name_dict.get(
                        record.operater_id, ""
                    ),
                    "operator_wximgurl": operator_wximgurl_dict.get(
                        record.operater_id, ""
                    ),
...
class StockinHistoryFunc:
    @classmethod
    def batch_format_stockin_history_info(
        cls,
        session,
        flows,
        weight_unit_text,
        ...
        map_account_name, map_account_wximgurl = AccountFunc.get_account_name_wximg_through_id_list(
            session, account_ids
        )
    ...
    def format_export_stockin_history_body(
        ...
        data_content = []
        data_dict = StockinHistoryFunc.batch_format_stockin_history_info(
            session,
            flows,
            weight_unit_text,
            map_groups_id_name,
...
class BreakageHistoryFunc:
    @classmethod
    def batch_format_breakage_history_info(
        cls, flows, weight_unit_text, return_type="list"
    ):
        """
            批量处理报损记录的数据
        ...
        for flow in flows:
            ...
            if hasattr(flow, "wx_unionid"):
                data["operator_wximgurl"] = (
                    models.AddImgDomain.add_domain_headimgsmall(flow.headimgurl)
                    if flow.wx_unionid
                    else ""
                )
            if return_type == "dict":
    ...
    def format_export_breakage_history_bodt(cls, flows, weight_unit_text):
        """构造某货品报损记录的导出excel格式"""
        data_content = []
        data_dict = BreakageHistoryFunc.batch_format_breakage_history_info(
            flows, weight_unit_text, return_type="dict"
        )
        data_content.append(
            [
                "序号",
...
class SalesRecordExportFunc:
    ...
    @classmethod
    def format_export_goods_sale_history(
        cls, session, shop_id, salerecord_list, export_item_list
    ):
        """货品流水自定义导出指定字段"""
        # 店铺是否开启去皮功能
        config = ShopFunc.get_config(session, shop_id)
    ...
    def format_export_body_custom(
        ...
        if have_headline:
            ...
            filter_config_content.append(
                "票据金额：%s到%s"
                % (
                    filter_config.get("receipt_min_money", 0),
            ...
            filter_config_content.append(
                "收银时间：%s到%s"
                % (
                    filter_config.get("cash_time_start", "无"),
            ...
            filter_config_content.append(
                "开票时间：%s到%s"
                % (
                    filter_config.get("ticket_time_start", "无"),
...
class OrderPrintBaseFunc:
    @classmethod
    def celery_order_print(
        cls, session, shop_id, order_id, user_id=0, receipt_config=None
    ):
    ...
    def order_print(
        ...
        if need_print:
            ...
            if (
                cls._check_order_records_single(session, order_id)
                or source == "salesman_async"
            ):
                origin_merge_print_accountant = (
                    receipt_config.merge_print_accountant
                )
    ...
    def _check_order_if_temp_order(cls, session, order_id):
        ...
        order = (
            session.query(Order)
            .filter_by(tally_order_id=order_id, multi_goods=1)
            .first()
        )
        if order:
            return True
    ...
    def _check_salesman_settlement(cls, order_id):
        salersman_settlement_print_key = (
            "salersman_settlement_print:%d" % order_id
        )
    ...
    def single_sale_record_print(
        ...
        record_counts = (
            session.query(
                models.Order.id, func.count(models.GoodsSalesRecord.id)
            )
            .join(
                models.GoodsSalesRecord,
        ...
        for valid_sale_record in record_list:
            # 结算中只有一个流水的才打印抹零信息
            print_erasement = (
                record_count_dict.get(valid_sale_record.order_id, 0) == 1
            )
        ...
        if not local_print == 1:
            for print_body in print_body_list:
                ...
                if (
                    record_index % slice_count != 0
                    and record_index != print_body_len
                ):
                ...
                if (
                    record_index % slice_count == 0
                    or record_index == print_body_len
                ):
    ...
    @classmethod
    def _merge_sale_record_print(
        cls, session, order, receipt_config, print_dict, repeat_print=False
    ):
        """ 收银台流水合并打印 """
        source = "accountant"
        wireless_print_type = int(print_dict["wireless_print_type"])
    ...
    def get_print_times(self, session, sales_record_id):
        """获取单品小票打印次数"""
        print_times = (
            session.query(func.count(models.GoodsSalesRecordPrintLog.id))
    ...
    def get_first_record_id(cls, session, order_id):
        """获取订单的第一条流水ID"""
        first_record_id = (
            session.query(models.GoodsSalesRecord.id)
            .filter_by(order_id=order_id)
            .first()
        )
    ...
    def get_order_print_times(self, session, order_id):
        """获取订单打印次数"""
        print_times = (
            session.query(func.count(models.RecordPrintLog.id))
...
class OrderPrintContentFunc:
    ...
    def order_print_body(
        ...
        if not if_pborder:
            ...
            # 如果订单内只有一张小票，强制走原单打印模式, 并在结束后恢复控制打印模式的状态
            if (
                OrderPrintBaseFunc._check_order_records_single(
                    session, order_id
                )
                or source == "salesman_async"
            ):
            ...
            if not (source == "salesman_async" and multi_order_dict):
                ...
                if single_record_list:
                    ...
                    record_count_dict = {
                        data[0]: data[1] for data in record_counts
                    }
                    for valid_sale_record in single_record_list:
                        # 结算中只有一个流水的才打印抹零信息
                        index += 1
                        print_erasement = (
                            record_count_dict.get(valid_sale_record.order_id, 0)
                            == 1
                        )
                        sale_record_info = GoodsSaleFunc.get_sale_record_info(
                            session,
            ...
            if print_body_list_content and wireless_print_type != 4:
                ...
                for print_body_content in print_body_list_content:
                    add_cut = (
                        ""
                        if count_index == len(print_body_list_content)
                        else wpp.cut()
                    )
...
class WirelessPrintFunc:
    def __init__(self, print_type, _paper_width=32, shop=None):
        ...
        if shop and shop.is_trial:
            self.support_text = self.line_break(
                "技术支持：森果  服务热线：400-027-0135"
            ) + self.float_middle(
                "系统未缴全款"
            )  # 底部技术支持文字
        else:
    ...
    def get_hirelink_printer(cls, session, shop_id, account_id, scene=1):
        ...
        if printer:
            ...
            wireless_print_type, wireless_print_num, wireless_print_key, printer_remark = (
                "0",
                "",
                "",
                "",
            ...
            if scene == 1:
                printer_id = (
                    session.query(models.HireLink.printer_id)
                    .filter_by(shop_id=shop_id, account_id=account_id)
                    .scalar()
                )
            elif scene == 2:
                printer_id = (
                    session.query(models.HireLink.accountant_printer_id)
                    .filter_by(shop_id=shop_id, account_id=account_id)
                    .scalar()
                )
            elif scene == 3:
                printer_id = (
                    session.query(models.HireLink.admin_printer_id)
                    .filter_by(shop_id=shop_id, account_id=account_id)
                    .scalar()
                )
            if printer_id:
                # 检查打印机的状态和打印机被分配的状态
                print_data = (
                    session.query(models.Printer)
                    .join(
                        models.PrinterAssignLink,
                        models.PrinterAssignLink.printer_id
                        == models.Printer.id,
                    )
                    .filter(
    ...
    def fill_box_to_float_right(self, left_content, box_len, right_content):
        ...
        """
        left_len = len(left_content) + CharFunc.get_contain_chinese_number(
            left_content
        )
        right_len = len(right_content) + CharFunc.get_contain_chinese_number(
            right_content
        )
    ...
    def color_invert(self, str_data, cancel=False):
        ...
        if self._type == "smposprint":
            ...
            _, num = re.search("style=", str_data).span()
            str_data = (
                "<p style='background-color:#000;-webkit-print-color-adjust: exact;color:#fff;"
                + str_data[num + 1 :]
            )
            return str_data
        else:
    ...
    def inline_block(self, str_data):
        ...
        _, num = re.search("style=", str_data).span()
        str_data = (
            "<p style='display:inline-block;min-width: 40%;box-sizing:border-box;"
            + str_data[num + 1 :]
        )
        return str_data
    ...
    def _fequery(cls, machine_code):
        ...
        if code == 200:
            ...
            if ret_dict["ret"] != 0:
                err_txt = (
                    ret_dict["data"] if ret_dict["data"] else ret_dict["msg"]
                )
    ...
    def send_print_request(
        self, content_body, wireless_print_num, wireless_print_key=""
    ):
        """向各个打印平台发送打印请求
        """
    ...
    def get_sale_record_print_body(
        ...
        if source == "salesman":
            ...
            if receipt_config.salesman_customer_receipt:
                ...
                salesman_customer_title = (
                    "[客户联]" + self._salesman_customer_space + "提货撕毁"
                )
                ...
                if (
                    receipt_config.salesman_accountant_receipt
                    or receipt_config.salesman_goods_receipt
                ):
                    content_body += cut()
            ...
            if receipt_config.cashier_accountant_receipt and (
                receipt_config.merge_print_accountant == 0
                and (
                    receipt_config.multi_sale_print_accountant == 1
                    or force_accountant
                )
                or is_hand_print
            ):
            ...
            if receipt_config.cashier_goods_receipt and (
                receipt_config.merge_print_goods == 0
                and (receipt_config.multi_sale_print_goods == 1 or force_goods)
                or is_hand_print
            ):
                ...
                if shop_id in [116, 105]:
                    ...
                    cashier_goods_title = (
                        "[客户提货联]" + self._cashier_goods_space + "提货撕毁"
                    )
                content_body_goods = float_middle(
                    zoom_in(accounting_title, default_size)
                )
    ...
    def format_receipt_temp(
        ...
        if goods_unit == 0:
            if tare_weight:
                sales_num_text = "净重：{sales_num}{goods_unit_text} 皮重：{tare_weight}{goods_unit_text} 毛重：{gross_weight}{goods_unit_text}".format(
                    sales_num=sales_num,
            ...
            if last_newline:
                ...
                if _type == "80localprint":
                    ...
                    if last_string:
                        content_len = len(
                            origin_text
                        ) + CharFunc.get_contain_chinese_number(origin_text)
                        last_content_len = len(
                            last_string
                        ) + CharFunc.get_contain_chinese_number(last_string)
                        content_box_len = (
                            self._paper_width - last_content_len - 28
                        )
    ...
    def get_dot_body(self, sale_record_num):
        ...
        if _type in ["feprint", "sgprint"]:
            ...
            for i in range(len(first_chinese_array)):
                tmp_content1 = (
                    ""
                    + first_chinese_array[i]
                    + second_chinese_array[i]
    ...
    def get_multi_sale_record_print_body(
        ...
        if source == "salesman":
            ...
            if receipt_config.salesman_accountant_receipt:
                ...
                content_body_data += line_break(
                    zoom_in(
                        color_invert("[会计联]" + self._salesman_accountant_space),
                        default_size,
                    )
                )
                content_body_data += content_main_body
            ...
            if (
                receipt_config.cashier_accountant_receipt
                and receipt_config.multi_sale_print_accountant == 0
                and receipt_config.merge_print_accountant == 0
            ):
                # 会计联
            ...
            if (
                receipt_config.cashier_goods_receipt
                and receipt_config.multi_sale_print_goods == 0
                and receipt_config.merge_print_goods == 0
            ):
                # 提货联
    ...
    def get_merge_sale_record_print_body(
        ...
        if (
            ...
            content_body_data += line_break(
                zoom_in(
                    "[客户提货联]" + self._cashier_goods_space + "提货撕毁", default_size
                )
            )
            content_body_data += content_main_body
            content_body += content_body_data
    ...
    def format_multi_receipt_temp(
        ...
        for sale_record_info in sale_record_info_list:
            ...
            for receipt_goods_dict in receipt_goods_list:
                ...
                if (
                    last_goods_content_body == goods_content_body
                    and goods_last_print
                    and not goods_last_newline
                ):
    ...
    def get_work_record_print_body(
        ...
        order_cash_money = "现金：%s元  %s笔" % (
            record_data["order_cash_money"],
            record_data["order_cash_count"],
        )
        order_bank_money = "银行卡：%s元  %s笔" % (
            record_data["order_bank_money"],
        ...
        erase_cash_money = "现金：%s元  %s笔" % (
            record_data["erase_cash_money"],
            record_data["erase_cash_count"],
        )
        erase_bank_money = "银行卡：%s元  %s笔" % (
            record_data["erase_bank_money"],
        ...
        erase_wx_money = "微信：%s元  %s笔" % (
            record_data["erase_wx_money"],
            record_data["erase_wx_count"],
        )
        erase_ali_money = "支付宝：%s元  %s笔" % (
            record_data["erase_ali_money"],
        ...
        total_cash_money = "现金：%s元  %s笔" % (
            record_data["total_cash_money"],
            record_data["total_cash_count"],
        )
        total_bank_money = "银行卡：%s元  %s笔" % (
            record_data["total_bank_money"],
        ...
        total_wx_money = "微信：%s元  %s笔" % (
            record_data["total_wx_money"],
            record_data["total_wx_count"],
        )
        total_ali_money = "支付宝：%s元  %s笔" % (
            record_data["total_ali_money"],
    ...
    def get_clearing_print_body(self, record_data):
        ...
        rate_type_text = (
            "费率：%s" % rate_type_text if rate_type_text != "-" else ""
        )
    ...
    def get_allocate_print_body(
        self, record_data, goods_list, weight_unit_text="斤"
    ):
        """库存调拨记录打印"""
        _paper_width = self._paper_width
        line_break = self.line_break
...
class WirelessPrintInfoFunc:
    ...
    def get_salersman_printer_info(cls, session, user_id, shop_id):
        printer_id = (
            session.query(models.HireLink.printer_id)
            .filter_by(account_id=user_id, shop_id=shop_id)
            .scalar()
        )
    ...
    def get_printer_info(cls, session, printer_id):
        ...
        if printer_id:
            printer = (
                session.query(models.Printer).filter_by(id=printer_id).first()
            )
            printer_info["id"] = printer.id
            printer_info["remark"] = printer.remark
            printer_info["brand"] = (
                "易联云"
                if printer.receipt_type == 1
                else "森果1型"
                if printer.receipt_type == 2
                else "森果2型"
                if printer.receipt_type == 3
                else "未知品牌"
            )
...
class AccountFunc:
    ...
    @classmethod
    def get_account_wx_head_imgurl_through_id_list(
        cls, session, account_id_list
    ):
        if account_id_list:
            Accountinfo = models.Accountinfo
            operator_list = (
    ...
    def get_account_wx_head_imgurl_through_id(cls, session, account_id):
        ...
        if account:
            account_img = (
                account.head_imgurl_small if account.wx_unionid else ""
            )
        else:
            account_img = ""
        return account_img
        ...
        if account_id_list:
            ...
            account = (
                session.query(
                    ...
                    Accountinfo.headimgurl,
                    Accountinfo.wx_unionid,
                )
                .filter(Accountinfo.id.in_(account_id_list))
                .all()
            )
    ...
    # 根据ID列表获取用户是否是店铺的货主
    @classmethod
    def get_account_goods_owner_role_through_id_list(
        cls, session, shop_id, account_id_list
    ):
        result = {}
        if account_id_list:
            hirelinks = (
                session.query(
                    models.HireLink.active_goods_owner, models.Accountinfo
                )
                .join(
                    models.Accountinfo,
                    models.Accountinfo.id == models.HireLink.account_id,
                )
                .filter(
                    models.Accountinfo.id.in_(account_id_list),
                    models.HireLink.shop_id == shop_id,
                )
                .all()
            )
            suppliers = (
    ...
    def get_account_through_uuid(cls, session, account_uuid):
        Accountinfo = models.Accountinfo
        account = (
            session.query(Accountinfo).filter_by(uuid=account_uuid).first()
        )
        return account
    ...
    def get_account_realname(cls, session, account_id):
        Accountinfo = models.Accountinfo
        realname = (
            session.query(Accountinfo.realname)
    ...
    def get_account_name(cls, session, account_id):
        Accountinfo = models.Accountinfo
        account = (
            session.query(Accountinfo.realname, Accountinfo.nickname)
            .filter_by(id=account_id)
            .first()
        )
        if account:
    ...
    def get_account_phone(cls, session, account_id):
        Accountinfo = models.Accountinfo
        phone = (
            session.query(Accountinfo.phone).filter_by(id=account_id).scalar()
            or ""
        )
        return phone
    ...
    def get_account_id_name_dict_through_phone(cls, session, phone, many=False):
        ...
        if many:
            result = (
                ...
                .filtery(Accountinfo.phone.in_(phone))
                .all()
            )
            for item in result:
            ...
            if result:
                id_name_dict[result.id] = (
                    result.realname or result.nickname or "-"
                )
        return id_name_dict
    ...
    def list_account_infos(cls, session, account_id_list):
        """ 通过账户id列表获取账户列表对象 """
        result = (
            session.query(models.Accountinfo)
            .filter(models.Accountinfo.id.in_(account_id_list))
            .all()
            if account_id_list
            else []
        )
    ...
    def get_account_name_phone_by_id(cls, session, _id):
        ...
        result = (
            session.query(
                Accountinfo.phone, Accountinfo.realname, Accountinfo.nickname
            )
            .filter_by(id=_id)
            .first()
        )
...
class RefundOrderFunc:
    ...
    def get_refund_record_time(cls, refund_order):
        time = (
            str(refund_order.create_year)
            + "-"
            + str(refund_order.create_month).zfill(2)
            + "-"
            + str(refund_order.create_day).zfill(2)
            + " "
            + str(refund_order.create_time)
        )
        return time
...
class FunctionPaidFunc:
    ...
    def get_service_paid(cls, session, shop_id, service_type=0):
        ...
        service_paid = (
            session.query(PaidServiceOrder)
...
class SupplierFunc:
    ...
    def get_supplier_goods_count(cls, session, shop_id, shop_supplier_id_list):
        ...
        Goods = models.Goods
        goods_query = (
            session.query(Goods.shop_supplier_id, func.count(Goods.id))
            .filter_by(shop_id=shop_id)
            .filter(
                Goods.active != 0,
    ...
    # 获取供货商的商品ID列表
    @classmethod
    def get_supplier_goods_id_through_id(
        cls, session, shop_id, shop_supplier_id_list
    ):
        Goods = models.Goods
        goods_query = (
            session.query(Goods.id)
    ...
    # 根据供货商id列表获取供货商
    @classmethod
    def get_shop_supplier_through_id_list(
        cls, session, shop_id, shop_supplier_id_list
    ):
        ShopSupplier = models.ShopSupplier
        shop_suppliers = (
    ...
    # 获取供货商基本信息
    @classmethod
    def get_shop_supplier_base_info(
        cls, session, shop_id, shop_supplier_id_list
    ):
        info_dict = {}
        ...
        for supplier in shop_suppliers:
            ...
            account_info = (
                AccountFunc.parse_account_info(account, get_wximgurl=True)
                if account
                else {}
            )
            info_dict[supplier.id] = {
                ...
                if supplier.active_goods_owner == 1
                else 0,
    ...
    def get_shop_supplier_names(cls, session, supplier_id_list):
        ...
        if supplier_id_list:
            ShopSupplier = models.ShopSupplier
            shop_suppliers = (
                session.query(ShopSupplier.id, ShopSupplier.realname)
    ...
    def get_borrow_status_through_id_list(cls, session, id_list):
        ...
        borrow_list = (
            session.query(
                ShopSupplierBorrowing.id, ShopSupplierBorrowing.status
            )
    ...
    # 根据商品名称查找供应商
    @classmethod
    def get_supplier_through_goods_name(
        cls, session, shop_id, name_list, filter_goods_list=None
    ):
        """
        ...
        goods_list = (
            session.query(Goods)
            .filter_by(shop_id=shop_id, active=1)
            .filter(Goods.name.in_(name_list), ~Goods.id.in_(filter_goods_list))
            .all()
        )
        ...
        for goods in goods_list:
            ...
            if shop_supplier_id:
                ...
                phone = (
                    shop_supplier.phone
                    if shop_supplier and shop_supplier.phone
                    else ""
                )
                shop_supplier_name = (
                    shop_supplier.realname
                    if shop_supplier and shop_supplier.realname
                    else ""
                )
            else:
                phone = ""
    ...
    # 根据商品ID查找供应商
    @classmethod
    def get_supplier_info_through_goods_id(
        cls, session, shop_id, goods_id, check_supply_type=False
    ):
        # 查找供应商
        shop_supplier_id = session.query(
            models.Goods.shop_supplier_id
        ).filter_by(shop_id=shop_id, id=goods_id)
        ...
        if shop_supplier_id:
            ...
            data_dict = dict(
                ...
                active_goods_owner=1
                if shop_supplier.active_goods_owner == 1
                else 0,
    ...
    # 根据商品ID查找供应商
    @classmethod
    def get_supplier_info_through_goods_id_list(
        cls, session, shop_id, goods_id_list, check_supply_type=False
    ):
        # 查找供应商
        supplier_id_list = (
            session.query(models.Goods.shop_supplier_id)
            .filter_by(shop_id=shop_id)
            .filter(models.Goods.id.in_(goods_id_list))
        )
        if check_supply_type:
    ...
    def check_supplier_unique_personal(
        ...
        supplier = (
            session.query(models.ShopSupplier)
            .filter_by(
                shop_id=shop_id, realname=username, phone=phone, status=1
            )
            .first()
        )
        # 不能与其他个体户相同
        ...
        c_suppliers = (
            ...
            .filter(
                or_(
                    models.ShopSupplier.company_short_name == username,
                    models.ShopSupplier.company_full_name == username,
                )
            )
            .all()
        )
        if c_suppliers:
            if (
                db_supplier
                and len(c_suppliers) == 1
                and c_suppliers[0].id == db_supplier.id
            ):
                return True, ""
            ...
            s_suppliers = (
                ...
                .filter(
                    or_(
                        ...
                        == company_short_name,
                        models.ShopSupplier.company_full_name
                        == company_short_name,
                    )
                )
                .all()
    ...
    def get_supplier_by_account_info(cls, session, shop_id, account_info):
        ...
        result = (
            session.query(models.ShopSupplier)
            .filter_by(shop_id=shop_id, status=1, account_id=account_info.id)
            .first()
        )
        return result
    ...
    # 获取供货商的商品数量
    @classmethod
    def get_multi_supplier_goods_id_list(
        cls, session, shop_id, supplier_id_list
    ):
        """ 通过供应商ID取出供应商对应的所有货品ID
    ...
    def check_supplier_goods(cls, session, shop_id, shop_supplier_id, goods_id):
        goods = (
            session.query(models.Goods.id)
            .filter_by(
                id=goods_id, shop_id=shop_id, shop_supplier_id=shop_supplier_id
            )
            .first()
        )
        return goods
    ...
    def get_multi_goods_bottom_price(cls, session, supplier_goods_id_list):
        bottom_price = (
            session.query(models.BottomPriceGoodsRate)
            .filter(
                models.BottomPriceGoodsRate.goods_id.in_(supplier_goods_id_list)
            )
            .all()
        )
        result = {}
    ...
    def handle_add_supplier(
        self, session, shop_id, username, phone, edit_user_id, source="salesman"
    ):
        ShopSupplier = models.ShopSupplier
...
class SupplierGoodsStockin:
    @classmethod
    def format_export_supplier_goods_stockin(
        cls, data_list, total_data_sum, weight_unit_text
    ):
        data_content = []
        data_content.append(
            [
        ...
        for data in data_list:
            data_content.append(
                [
                    ...
                    data["quantity"],
                    data["weight"],
                    (data["quantity_unit_price"] or "-")
                    if data["goods_purchase_unit"]
                    else (data["weight_unit_price"] or "-"),
                    data["total_price"],
                    data["paid_cent"]
                    if data["supply_type"] not in [1, 2]
                    else "-",
                    data["credit_cent"]
                    if data["supply_type"] not in [1, 2]
                    else "-",
        ...
        data_content.append(
            [
                ...
                "",
                "",
                total_data_sum["quantity_sum"],
                total_data_sum["weight_sum"],
                "",
                total_data_sum["total_price_sum"],
                total_data_sum["paid_cent_sum"],
...
class SupplierSettlementFunc:
    ...
    def get_clearing_data(cls, session, shop_id, goods_id):
        ...
        clearing_count, clearing_money = session.query(
            func.count(ShopSupplierClearing.id),
            func.sum(
                ShopSupplierClearing.borrow_cent
                + ShopSupplierClearing.clearing_cent
            ),
        ).filter_by(shop_id=shop_id, goods_id=goods_id).first() or (0, 0)
        clearing_count = check_int(clearing_count) if clearing_count else 0
        clearing_money = (
            check_float(clearing_money / 100) if clearing_money else 0
        )
    ...
    def get_unclearing_borrow_data(cls, session, shop_id, shop_supplier_id):
        ...
        if redis.get(redis_key):
            return 0, 0
        ShopSupplierBorrowing = models.ShopSupplierBorrowing
        borrow_count, borrow_money = session.query(
            func.count(ShopSupplierBorrowing.id),
            func.sum(ShopSupplierBorrowing.borrow_cent),
        ).filter_by(
            shop_id=shop_id, shop_supplier_id=shop_supplier_id, status=0
        ).first() or (
            0,
            0,
        )
    ...
    def get_unclearing_money(cls, session, shop_id, goods_id_list):
        ...
        unclearing_money_proxy = (
            ...
            .with_hint(
                GoodsSalesRecord, "FORCE INDEX(ix_shop_goods_status_clearing)"
            )
            .scalar()
            or 0
            if proxy_goods_id_set
            else 0
        )
        # 自营
        StockInDocsDetail = models.StockInDocsDetail
        unclearing_money_self_sell = (
            session.query(func.sum(StockInDocsDetail.total_price))
            .filter_by(clearing_record_id=0, pay_type=5, status=1)
            .filter(StockInDocsDetail.goods_id.in_(self_sell_goods_id_set))
            .scalar()
            or 0
            if self_sell_goods_id_set
            else 0
        )
    ...
    def get_unclearing_money_dict(
        ...
        if clearing_record_ids is None:
            ...
            proxy_filter_params.append(
                not_(
                    GoodsSalesRecord.clearing_record_id.in_(clearing_record_ids)
                )
            )
            proxy_filter_params.append(
                GoodsSalesRecord.create_time < end_date_time
            )
        goods_unclearing_info_proxy = (
            session.query(
                GoodsSalesRecord.goods_id,
                func.sum(
                    func.IF(
        ...
        goods_unclearing_dict = {
            g[0]: {"money": g[1], "commission_mul": g[2]}
        ...
        goods_unclearing_info_self_sell = (
            session.query(
        ...
        goods_unpay_dict = {
            g[0]: {"money": g.total_price, "commission_mul": g.quantity}
        ...
        if return_type == "supplier":
            ...
            Goods = models.Goods
            goods_list = (
                session.query(Goods.id, Goods.shop_supplier_id)
                .filter_by(shop_id=shop_id)
                .filter(Goods.id.in_(agents_goods_id_list))
                .all()
                if agents_goods_id_list
                else []
            )
    ...
    def get_clear_detail(cls, session, clearing, shop_name=None):
        ...
        # 获取费率
        clearing_goods = (
            session.query(models.ShopSupplierClearingGoods.goods_rate_id)
            .filter_by(clearing_id=clearing.id)
            .first()
        )
        ...
        data_dict["rate_type_text"] = (
            GoodsRateFunc.get_rate_type_text(
    ...
    def get_clearing_goods(cls, session, clearing_id):
        ...
        clearing_goods_id_list = (
            session.query(ShopSupplierClearingGoods.goods_id)
    ...
    @classmethod
    def list_clearing_sum_each_day(
        cls, session, sales_record_id_list, borrow_id_list, data_type
    ):
        ...
        GoodsSalesRecord = models.GoodsSalesRecord
        group_by_column = (
            models.GoodsSalesRecord.bill_date
            if data_type == 0
            else GoodsSalesRecord.account_period_date
        )
        ...
        # 销售日分组
        sales_each_day = (
            sales_query_base.filter(*params_filter)
        ...
        payback_records = (
            session.query(
                ...
                func.IF(
                    models.GoodsSalesRecordPaybackRecord.payback_cent
        ...
        # 预支日分组
        group_by_column = (
            func.date(models.ShopSupplierBorrowing.create_date)
            if data_type == 0
            else models.ShopSupplierBorrowing.account_period_date
        )
        borrow_each_day = (
            session.query(
                group_by_column,
                func.sum(models.ShopSupplierBorrowing.borrow_cent).label(
                    "borrow_cent"
                ),
            )
    ...
    def _parse_payback(cls, session, payback_records, data_type):
        ...
        if data_type == 1:
            ...
            payback_order = (
                ...
                .join(
                    models.PbOrder,
                    models.GoodsSalesRecordPaybackRecord.pb_order_id
                    == models.PbOrder.id,
                )
                .filter(
                    models.GoodsSalesRecordPaybackRecord.id.in_(
                        payback_record_ids
                    )
                )
                .all()
...
class GoodsRateFunc:
    ...
    def get_rate_type_text(cls, goods_rate, fee_text="", weight_unit_text="斤"):
        ...
        if goods_rate.piece_active == 1:
            rate_type_text = "每件%s元" % (
                check_float(goods_rate.piece_cent / 100)
            )
    ...
    def get_goods_rate_through_id(cls, session, shop_id, rate_id):
        """ 根据费率ID获取商品费率 """
        goods_rate = (
            session.query(models.ClearGoodsRate)
...
class ShopCustomerFunc:
    ...
    def get_shop_customer_phone_through_id(cls, session, shop_id, customer_id):
        ...
        ShopCustomer = models.ShopCustomer
        shop_customer_phone = (
            session.query(ShopCustomer.phone)
            .filter_by(id=customer_id, shop_id=shop_id)
            .filter(ShopCustomer.status != -1)
            .first()
        )
    ...
    def get_shop_customer_name_through_id(cls, session, customer_id):
        ...
        ShopCustomer = models.ShopCustomer
        customer_name = (
            session.query(ShopCustomer.name)
    ...
    def update_purchase_info(
        cls, session, customer_id, shop_id, purchase_money=0, purchase_id=0
    ):
        """
            更新用户采购金额/采购次数
        """
    ...
    def get_shop_customer_name_through_id_list(cls, session, customer_id_list):
        ...
        if customer_id_list:
            ShopCustomer = models.ShopCustomer
            shop_customer = (
                session.query(ShopCustomer.id, ShopCustomer.name)
    ...
    @classmethod
    def purchase_point(
        cls, session, shop_id, shop_customer, order, operator_id
    ):
        """
            采购操作增加积分入口
        """
        rule = (
            session.query(models.PointRule)
            .filter(
                models.PointRule.shop_id == shop_id,
                models.PointRule.active == 1,
            )
            .first()
        )
        if not rule:
        ...
        if rule.weekly_purchase or rule.monthly_purchase:
            ...
            # 排除退款订单
            query = session.query(Order.create_time).filter(
                Order.shop_id == shop_id,
                Order.debtor_id == shop_customer.id,
                Order.status == 2,
                Order.record_type == 0,
            )
            ...
            if rule.weekly_purchase:
                ...
                # 如果本周加过就不再加
                point_history = (
                    session.query(ShopCustomerPointHistory)
                    .filter(ShopCustomerPointHistory.operate_type == 3)
                    .filter(
                        ShopCustomerPointHistory.create_time
                        > first_day_this_week
                    )
                    .filter(
                        ShopCustomerPointHistory.shop_id == shop_id,
                        ShopCustomerPointHistory.customer_id
                        == shop_customer.id,
                    )
                    .first()
                )
                if not point_history:
                    ...
                    order_dates_this_week = query.filter(
                        Order.create_time > first_day_this_week
                    ).all()
                    # 本周产生了订单的天数
            ...
            if rule.monthly_purchase:
                ...
                if not point_history:
                    ...
                    order_dates_this_month = query.filter(
                        Order.create_time > first_day_this_month
                    ).all()
                    # 本月产生了订单的天数
    ...
    def get_shop_customer_history_debt_rest(cls, session, customer_id):
        ...
        ShopCustomer = models.ShopCustomer
        history_debt_rest = (
            session.query(ShopCustomer.history_debt_rest)
            .filter_by(id=customer_id)
            .scalar()
            or 0
        )
    ...
    def format_customer_info(cls, session, customer_list, type="list"):
        ...
        all_purchase_number = (
            session.query(
                CustomerPurchase.shop_customer_id,
        ...
        # 查询客户的拉黑信息
        black_list = (
            session.query(CustomerBlackList)
            .filter(
                CustomerBlackList.customer_id.in_(black_customer_ids),
        ...
        # 客户是否有历史欠款
        debt_historys = (
            session.query(DebtHistory.debtor_id)
            .filter(
                DebtHistory.debt_type == 3,
        ...
        for i in customer_list:
            ...
            creator_name = (
                AccountFunc.get_account_name(session, creator_id)
                if creator_id
                else ""
            )
            sponsor_name = (
                AccountFunc.get_account_name(session, sponsor_id)
                if sponsor_id
                else ""
            )
            ...
            if black_info:
                ...
                defriend_operator_name = (
                    AccountFunc.get_account_name(session, defriend_operator_id)
                    if defriend_operator_id
                    else ""
                )
    ...
    # 获取赊账客户信息
    @classmethod
    def get_customer_info_through_id_list(
        self, session, shop_id, customer_id_list
    ):
        ShopCustomer = models.ShopCustomer
        debtor_list = (
            session.query(ShopCustomer)
    ...
    def get_debtor_order_info(self, session, shop_id, history):
        ...
        if debt_type == 0:
            ...
            if pay_type == 10:
                ...
                pb_order_money = (
                    session.query(models.PbOrderMoney)
                    .filter_by(id=fk_id)
                    .first()
                )
                if pb_order_money and (
                    pb_order_money.barcode_wx_cent > 0
    ...
    def get_customers_under_group(cls, session, group_id, page=-1):
        ...
        page_size = 20
        shop_customers = (
            query_base.offset(page * page_size).limit(page_size).all()
        )
        nomore = 0
    ...
    def get_customer_by_id_list(cls, session, customer_id_list):
        ...
        customers = (
            session.query(ShopCustomer)
            .filter(
                ShopCustomer.id.in_(customer_id_list), ShopCustomer.status != -1
            )
            .all()
        )
        return customers
    ...
    def get_purchase_by_id(cls, session, purchase_id):
        purchase = (
            session.query(models.CustomerPurchase)
...
class UrlShorten:
    ...
    def get_md5(self, longurl):
        longurl = (
            longurl.encode("utf8") if isinstance(longurl, str) else longurl
        )
        m = hashlib.md5()
        m.update(longurl)
        return m.hexdigest()
    ...
    def get_short_url(self, long_url):
        url = (
            self.session.query(models.ShortUrl)
            .filter_by(long_url=long_url)
            .first()
        )
        if url:
...
class AccountingPeriodFunc:
    ...
    def get_accounting_info(cls, session, shop_id, date):
        AccountingPeriodRecord = models.AccountingPeriodRecord
        accounting_record = (
            session.query(AccountingPeriodRecord)
            .filter_by(shop_id=shop_id, record_date=date)
            .first()
        )
        accounting_info = {}
        if accounting_record:
            accounting_info["username"] = (
                AccountFunc.get_account_realname(
                    session, accounting_record.user_id
                )
                if accounting_record.user_id
                else ""
            )
            accounting_info["time"] = TimeFunc.time_to_str(
                accounting_record.create_date, "time"
            )
            accounting_info["accounting_model"] = (
                1 if accounting_record.user_id else 2
            )
    ...
    def get_accounting_time(cls, session, shop_id, choose_date=""):
        ...
        AccountingPeriodRecord = models.AccountingPeriodRecord
        accounting_time = (
            session.query(AccountingPeriodRecord.create_date)
            .filter_by(shop_id=shop_id, record_date=choose_date)
            .scalar()
            or None
        )
    ...
    def get_auto_accounting_time(cls, session, shop_id):
        ...
        auto_time = (
            session.query(AutoAccountantTimeRecord.auto_time)
            .filter_by(shop_id=shop_id, status=1)
            .scalar()
        )
        if auto_time:
    ...
    def open_auto_accounting_task(cls, session, shop_id):
        auto_time_record = (
            session.query(models.AutoAccountantTimeRecord)
            .filter_by(shop_id=shop_id, status=1)
            .first()
        )
        if auto_time_record:
...
class StaffFunc:
    ...
    def get_hirelink_name_through_account_id(cls, session, shop_id, account_id):
        HireLink = models.HireLink
        name = (
            session.query(HireLink.name_remark)
            .filter_by(shop_id=shop_id, id=account_id)
            .scalar()
            or ""
        )
        return name
    ...
    # 获取员工备注
    @classmethod
    def get_hirelink_name_through_account_id_list(
        cls, session, shop_id, account_id_list
    ):
        HireLink = models.HireLink
        name_list = (
            session.query(HireLink.account_id, HireLink.name_remark)
            .filter_by(shop_id=shop_id)
    ...
    def is_salesman_goods_owner(cls, session, shop_id, staff_id):
        ...
        hirelink = (
            session.query(models.HireLink)
            .filter_by(shop_id=shop_id, account_id=staff_id)
            .first()
        )
    ...
    def assign_default_printers(cls, session, hire_link):
        ...
        for printer in default_printers:
            ...
            if (
                printer.is_salesman_default
                and hire_link.active_salesman == 1
                or printer.is_accountant_default
                and hire_link.active_accountant == 1
                or printer.is_admin_default
                and hire_link.active_admin == 1
            ):
    ...
    def get_shop_super_admin(cls, session, shop_id):
        ...
        super_admin = (
            session.query(models.HireLink)
            .filter_by(shop_id=shop_id, account_id=shop.boss_id)
            .first()
        )
        return super_admin
...
class GoodsOperationRecordFunc:
    ...
    def get_goods_allocate_data(cls, session, records, weight_unit_text="斤"):
        ...
        goods_allocate = (
            ...
            .join(
                StorageAllocate,
                StorageAllocateGoods.record_id == StorageAllocate.id,
            )
            .filter(StorageAllocateGoods.id.in_(fk_ids))
            .all()
        )
        goods_allocate_dict = {
            i.id: (i.quantity, i.weight) for i in goods_allocate
        }
    ...
    def get_goods_history_data(cls, session, records):
        ...
        fk_ids = {i.fk_id for i in records}
        goods_history = (
            session.query(GoodsHistory.id, GoodsHistory.goods_id)
    ...
    def get_goods_stockin_data(cls, session, records, weight_unit_text="斤"):
        ...
        goods_stockin_dict = {
            i.id: (i.quantity, i.weight) for i in goods_stockin
        }
    ...
    def get_goods_name_edit_data(cls, session, records):
        ...
        fk_ids = {i.fk_id for i in records}
        goods_name_edit = (
            session.query(GoodsNameEditHistory)
    ...
    def get_goods_group_change_data(cls, session, records):
        ...
        group_change_record = (
            session.query(
                GoodsGroupChangeRecord.id, GoodsGroupChangeRecord.group_id
            )
    ...
    def get_goods_group_edit_data(cls, session, records):
        ...
        fk_ids = {i.fk_id for i in records}
        goods_group_edit = (
            session.query(GoodsGroupEditHistory)
    ...
    def get_goods_breakage_data(cls, session, records, weight_unit_text="斤"):
        ...
        goods_breakage_dict = {
            i.id: (i.quantity, i.weight) for i in goods_breakage
        }
    ...
    @classmethod
    def get_goods_cost_price_edit_data(
        cls, session, records, weight_unit_text="斤"
    ):
        """ 查询GoodsCostPriceEditHistory, 获取商品id, 商品名, 变更前后的成本价 """
        fk_ids = {i.fk_id for i in records}
        historys = (
            session.query(models.GoodsCostPriceEditHistory)
            .filter(models.GoodsCostPriceEditHistory.id.in_(fk_ids))
            .all()
        )
    ...
    def get_goods_inventory_data(cls, session, records, weight_unit_text="斤"):
        ...
        goods_inventory_dict = {
            i.id: (i.quantity, i.weight) for i in goods_inventory
        }
...
class AuthFunc:
    ...
    def merge_passport(cls, session, passport_id, phone):
        ...
        data = dict(passport_id=passport_id, phone=phone, source="pifa")
        headers = {"Authorization": cls.gen_token()}
        resp = requests.post(url, json=data, headers=headers)
        if resp.status_code != 200:
            raise Exception(resp.text)
    ...
    @classmethod
    def update_single_accountinfo(
        cls, passport_id, data, session, force_create=False
    ):
        ...
        if not user:
            if force_create:
                user = Accountinfo(
                    passport_id=passport_id, uuid=Accountinfo.make_uuid(session)
                )
                session.add(user)
                session.flush()
            else:
    ...
    def update_merge_accountinfo(cls, session):
        ...
        for i in keys:
            ...
            auth_redis.set(this_key, 2)
            cls.update_single_accountinfo(
                source, cls.get_passportinfo(source), session
            )
            cls.update_single_accountinfo(
                target, cls.get_passportinfo(target), session
            )
            auth_redis.delete(this_key)
    ...
    def update_through_wx(cls, session, wx_userinfo, account, action="update"):
        ...
        if action in ["bind", "update_all"]:
            ...
            if not now_openid.startswith("oA") and wx_userinfo.get(
                "openid", ""
            ).startswith("oA"):
...
class SalesRecordAjustFunc:
    @classmethod
    def get_ajust_new_goods_info_through_id_list(
        cls, session, origin_record_id_list
    ):
        """ 获取串货调整新货品ID
        """
    ...
    def get_ajust_info(cls, session, record):
        ...
        if record.bill_date + datetime.timedelta(
            days=7
        ) > datetime.datetime.now().date() and record.status in (3, 5):
            could_ajust = True
        ...
        if ajust_record:
            ...
            ajust_info[
                "ajust_origin_goods_name"
            ] = GoodsFunc.get_goods_name_with_supplier(
                session, shop_id, origin_goods_id
            )  # 原始货品名
            ajust_info["ajust_new_goods_id"] = new_goods_id  # 新货品ID
            ajust_info[
                "ajust_new_goods_name"
            ] = GoodsFunc.get_goods_name_with_supplier(
                session, shop_id, new_goods_id
            )  # 新货品名
            ...
            ajust_info[
                ...
                session, ajust_record.operator_id
            )  # 调整人姓名
            ajust_info["ajust_operator_time"] = TimeFunc.time_to_str(
                ajust_record.create_time
            )  # 调整时间
            ajust_info["ajust_type"] = ajust_type  # 调整类型
            ajust_info[
                "ajust_origin_id"
            ] = ajust_record.origin_record_id  # 原始单号ID
            ajust_info[
                "ajust_refund_id"
            ] = ajust_record.refund_record_id  # 冲红单号ID
    ...
    def get_ajust_record(cls, session, record_id):
        ...
        GoodsSalesRecordAjust = models.GoodsSalesRecordAjust
        ajust_record = (
            session.query(GoodsSalesRecordAjust)
            .filter(
                or_(
                    GoodsSalesRecordAjust.origin_record_id == record_id,
    ...
    def get_multi_ajust_info(cls, session, record_id_list):
        ...
        ajust_record_id_list = (
            session.query(GoodsSalesRecordAjust.origin_record_id)
    ...
    def get_multi_ajust_all(cls, session, record_id_list):
        ...
        """
        GoodsSalesRecordAjust = models.GoodsSalesRecordAjust
        origin_record_id_list = session.query(
            GoodsSalesRecordAjust.origin_record_id
        ).filter(GoodsSalesRecordAjust.origin_record_id.in_(record_id_list))

        refund_record_id_list = session.query(
            GoodsSalesRecordAjust.refund_record_id
        ).filter(GoodsSalesRecordAjust.refund_record_id.in_(record_id_list))

        new_record_id_list = session.query(
            GoodsSalesRecordAjust.new_record_id
        ).filter(GoodsSalesRecordAjust.new_record_id.in_(record_id_list))

        record_id_list = origin_record_id_list.union(
            refund_record_id_list
        ).union(new_record_id_list)
...
class StockInFunc:
    @classmethod
    def get_doc_detail_unit_pub(
        cls, session, doc_detail, doc, goods_id_list, filter_detail_id_list=[]
    ):
        """获取最新的入库/报损记录的货品单位
        """
        detail_ids = (
            session.query(func.max(doc_detail.id))
            .join(doc, doc_detail.doc_id == doc.id)
            .filter(
                doc_detail.goods_id.in_(goods_id_list),
        ...
        detail_id_list = [d[0] for d in detail_ids]
        details = (
            session.query(doc_detail.goods_id, doc_detail.unit)
    ...
    def get_flow(cls, session, shop_id, page, **kwargs):
        ...
        flow_base = (
            flow_base.join(*param_join_first)
        ...
        if page == -1:
            ...
            count = (
                count_base.join(*param_join_first)
                .join(*param_join_second)
                .filter(*params_filter)
                .scalar()
            )
        ...
        if is_unclearing:
            ...
            sum_stockin_unpay = (
                ...
                .filter(*params_filter)
                .filter_by(pay_type=5)
                .join(*param_join_first)
                .join(*param_join_second)
                .first()
            )
            sum_dict["total_price_unpay"] = (
                check_float(sum_stockin_unpay.total_price / 100)
                if sum_stockin_unpay.total_price
                else 0
            )

            ids_stockin = (
                session.query(models.StockInDocsDetail.id)
                .filter(*params_filter)
                .join(*param_join_first)
                .join(*param_join_second)
                .all()
        ...
        # 筛选用货品信息
        total_goods_list = (
            session.query(models.StockInDocsDetail.goods_id, models.Goods)
            .join(*param_join_first)
            .join(*param_join_second)
        ...
        total_goods_list = [
            {"id": goods.id, "name": GoodsFunc.get_goods_detail_name(goods)}
    ...
    @classmethod
    def get_statistic_each_goods(
        cls, cls_doc_detail, cls_doc, shop_id, session, page, **kwargs
    ):
        """ 获取入库单的采购汇总 """
        page_size = 20
        supply_type = kwargs.get("supply_type")
        ...
        label_total_price = func.sum(cls_doc_detail.total_price).label(
            "total_price"
        )
        label_cost_price_quantity = func.sum(
            cls_doc_detail.cost_price_quantity
        ).label("cost_price_quantity")
        label_cost_price_weight = func.sum(
            cls_doc_detail.cost_price_weight
        ).label("cost_price_weight")
        query_list = [
        ...
        # 查询合并货品记录条数
        statistic_base = (
            statistic_base.join(*param_join_first)
            .join(*param_join_second)
            .filter(*params_filter)
        ...
        if page == -1:
            ...
            statistic = (
                master_statistic_base.order_by(params_order_by)
                .offset(page_size * page)
                .limit(page_size)
                .all()
            )
        ...
        branch_statistics = statistic_base.filter(
            models.Goods.master_goods_id.in_(master_goods_ids)
        ).all()
        branch_statistics_dict = defaultdict(list)
        for branch_statistics_item in branch_statistics:
            branch_statistics_dict[
                branch_statistics_item.master_goods_id
            ].append(branch_statistics_item)
        ...
        # 筛选用货品信息
        total_goods_list = (
            session.query(cls_doc_detail.goods_id, models.Goods.name)
            .join(*param_join_first)
            .join(*param_join_second)
            .filter(*params_filter_goods)
        )
        ...
        total_goods_list = total_goods_list.all()
        total_goods_list = [
            {"id": item.goods_id, "name": item.name}
        ...
        def prepare_data(doc_detail, goods, detail_unit, type):
            data_dict = dict(
                ...
                storage_unit_text="件"
                if goods["storage_unit"] == 1
                else weight_unit_text,
                purchase_unit=goods["purchase_unit"],
                purchase_unit_text="件"
                if goods["purchase_unit"] == 1
                else weight_unit_text,
                detail_unit=detail_unit,
            )
            # 入库单的价格用总价计算
            if type == models.StockInDocsDetail:
                data_dict["quantity_unit_price"] = (
                    check_float(doc_detail.total_price / doc_detail.quantity)
                    if doc_detail.quantity
                    else ""
                )
                try:
                    data_dict["weight_unit_price"] = (
                        check_float(
                            doc_detail.total_price
                            / doc_detail.get_weight(
                                weight_unit_text=weight_unit_text
                            )
                        )
                        if doc_detail.weight
...
class ConfigFunc:
    ...
    def get_statistic_gross(cls, session, shop_id):
        statistic_gross = (
            session.query(models.Config.disable_statistic_gross)
            .filter_by(id=shop_id)
            .scalar()
            or 0
        )
    ...
    def get_modify_paytype_days(cls, session, shop_id):
        modify_paytype_days = (
            session.query(models.Config.modify_paytype_days)
            .filter_by(id=shop_id)
            .scalar()
            or 0
        )
    ...
    def get_fee_text(cls, session, shop_id):
        fee_text_type = (
            session.query(models.Config.fee_text_type)
            .filter_by(id=shop_id)
            .scalar()
            or 0
        )
    ...
    def get_weight_unit_text(cls, session, shop_id):
        display_kg_instead = (
            session.query(models.Config.display_kg_instead)
            .filter_by(id=shop_id)
            .scalar()
            or 0
        )
...
class PaytypeModifyFunc:
    ...
    def get_origin_paytype_text(cls, session, order_id, source_type=1):
        ...
        modify_record = (
            session.query(OrderPaytypeModifyRecord)
...
class LcBankFunc:
    ...
    def get_bank_name_dict(cls, session, bank_nos):
        banks = (
            session.query(models.LcParentBank)
    ...
    def get_bank_name(cls, session, bank_no):
        bank_name = (
            session.query(models.LcParentBank.parent_bank_name)
...
class SalesmanFunc:
    ...
    @classmethod
    def list_goods_id_editable(
        cls, session, shop_id, account_id, permission_text_list
    ):
        """ 获取开票员可以编辑的货品id
...
class OrderFinishFunc:
    ...
    def gen_sales_record_by_origin(
        ...
        # 检查是否扎帐
        account_period_date = now.date()
        if AccountingPeriodFunc.get_accounting_time(
            session, sale_record.shop_id
        ):
            account_period_date = account_period_date + datetime.timedelta(1)
        ...
        record.bill_time = (
            sale_record.bill_time if not order else now.strftime("%H:%M:%S")
        )
    ...
    def gen_sales_record_by_frontend(
        ...
        sales_record.bill_time = (
            bill_time.time() if bill_time else now.strftime("%H:%M:%S")
        )
        sales_record.status = status
        sales_record.cost_price = goods.cost_price
        sales_record.shop_customer_id = shop_customer_id
        sales_record.shop_supplier_name = (
            shop_supplier.realname if shop_supplier else ""
        )
...
class ShopRegisterFunc:
    ...
    def get_applying_or_failed_shop_by_user_id(session, user_id):
        ...
        exist_apply_shop = (
            session.query(ShopApply)
            .filter(ShopApply.account_id == user_id)
            .order_by(ShopApply.id.desc())
            .first()
        )  # type: ShopApply
    ...
    def get_deposit_paid_record_by_id(session, record_id):
        ...
        record = (
            session.query(DepositPaidRecord).filter_by(id=record_id).first()
        )
        return record
    ...
    def get_shop_apply_pay_status_by_user_id(session, user_id):
        ...
        shop_apply = (
            session.query(ShopApply)
            .filter(
                ShopApply.account_id == user_id, ShopApply.status.in_([0, 1])
            )
            .first()
        )
        if not shop_apply:
            return None
        if (
            shop_apply.deposit_paid == shop_apply.amount_payable
            and shop_apply.status == 1
        ):
            return True
        else:
            return False
    ...
    def get_shop_apply_by_user_id(session, user_id):
        ...
        shop_apply = (
            session.query(ShopApply)
            .filter(ShopApply.account_id == user_id, ShopApply.status == 0)
            .first()
        )
        return shop_apply
    ...
    def get_recent_rejected_shop_apply_by_user_id(session, user_id):
        ...
        shop_apply = (
            session.query(ShopApply)
            .filter(ShopApply.account_id == user_id, ShopApply.status == 3)
            .order_by(ShopApply.id.desc())
            .first()
        )
        return shop_apply
...
class SuperAdminFunc:
    ...
    def get_super_admin_by_id(session, _id, status=None):
        ...
        if not status:
            super_admin = (
                session.query(SuperAdmin)
                .filter(SuperAdmin.id == _id, SuperAdmin.is_delete == 0)
                .first()
            )
        else:
            super_admin = (
                session.query(SuperAdmin).filter(SuperAdmin.id == _id).first()
            )
        return super_admin
    ...
    def get_apply_shop_by_passed_shop_id(session, _id):
        """通过正式店铺的id查找申请中的店铺"""
        shop_apply = (
            session.query(models.ShopApply).filter_by(shop_id=_id).first()
        )
        return shop_apply
    ...
    def get_shop_is_trial_close_dict_by_id_list(session, id_list):
        ...
        Shop = models.Shop
        shop_trail = (
            session.query(Shop.id, Shop.is_trial, Shop.status)
            .filter(Shop.id.in_(id_list))
            .all()
        )
        id_trial_dict = dict()
        id_status_dict = dict()
...
class FileToolFunc:
    ...
    @staticmethod
    def write_csv_file(
        file_name, content, shop_id, user_id, timestamp, file_path=EXPORT_PATH
    ):
        """本地保存csv文件"""
    ...
    def set_shop_export_status(shop_id, user_id, exporting_key, seconds=5):
        """设置店铺导出状态，默认十秒"""
        redis_export.setex(
            "exporting:%d-%d" % (shop_id, user_id), seconds, exporting_key
        )
