...
class TimeFunc():
    ...
    def get_fake_clearing_state(cls, the_date):
        ...
        for i in range(delta_day):
            temp_day = (the_date + datetime.timedelta(i + 1)
                        ).strftime('%Y-%m-%d')
    ...
    def get_assign_date(cls, days=0):
        ...
        assign_date = datetime.datetime(
            now_date.year, now_date.month, now_date.day) - datetime.timedelta(days=days)
        return assign_date
    ...
    def splice_time(cls, year, month, day, time):
        time = "{year}-{month}-{day} {time}".format(
            year=year, month=month, day=day, time=cls.time_to_str(time, "time"))
        return time
...
class ShopFunc():
    ...
    def get_lcpay_account(cls, session, shop_id):
        ...
        if redis.get(lcpay_account_key):
            ...
            if shop:
                ...
                redis.set(lcpay_account_key, lc_merchant_no + "|" +
                          lc_terminal_id + "|" + lc_access_token, 24 * 3600)
    ...
    def make_shop_code(cls, session):
        ...
        while 1:
            ...
            shop = session.query(models.Shop.id).filter_by(
                shop_code=str).scalar()
            if not shop:
                break
        return str
    ...
    def get_shop_name_through_id(cls, session, shop_id):
        shop_name = session.query(models.Shop.shop_name).filter_by(
            id=shop_id).scalar() or ""
        return shop_name
    ...
    def is_admin_have_many_shop(cls, session, shop_id):
        ...
        if result:
            shops_count = session.query(models.Shop).filter_by(
                boss_id=result.boss_id).count()
...
class StatisticFunc():
    ...
    def get_funds_info_per_shop_statitic_instance(cls, single_fund, data_info, source="accountant"):
        ...
        total_cent = fund_cash + fund_pos + lc_fund_wxbar + lc_fund_alibar + \
            lc_fund_wxtag + lc_fund_alitag + fund_cheque + fund_remittance
    ...
    def get_goods_stockin_data(cls, session, statistic_session, shop_id, goods_dict, weight_unit_text, args={}, if_total=False, only_goods=False):
        ...
        if group_id:
            ...
            filter_params.append(StatisticGoodsStockin.goods_id.in_(
        ...
        total_data_sum = dict(
            quantity_sum=check_float(
                sum_data.quantity / 100) if sum_data.quantity else 0,
            ...
            total_price_sum=check_float(
                sum_data.total_price / 100) if sum_data.total_price else 0,
            credit_cent_sum=check_float(
                sum_data.credit_cent / 100) if sum_data.credit_cent else 0,
            paid_cent_sum=check_float(
                sum_data.paid_cent / 100) if sum_data.paid_cent else 0,
            payback_cent_sum=check_float(
                sum_data.payback_cent / 100) if sum_data.payback_cent else 0,
            should_pay_cent_sum=check_float(
                sum_data.should_pay_cent / 100) if sum_data.should_pay_cent else 0
        )
        for statistic in statistic_all:
            ...
            date_range = TimeFunc.list_date_range(
                statistic_type, date=getattr(
                    statistic, "statistic_date", None),
...
class GoodsFunc():
    ...
    def get_goods_name_through_id_list(cls, session, goods_id_list):
        ...
        if goods_id_list:
            ...
            goods = session.query(Goods) \
                ...
                .all()
            goods_name_dict = {
                x.id: cls.get_goods_name_with_attr(x) for x in goods}
    ...
    def get_goods_and_supplier_names(cls, session, goods_id_list):
        ...
        if goods_id_list:
            ...
            for goods in goods_list:
                ...
                supplier_name = supplier_name_dict.get(
    ...
    def get_goods_in_shop(
        ...
        map_goods_pay_type = {
            detail.goods_id: detail.pay_type for detail in details}
...
class GoodsBatchFunc():
    ...
    def _construct_consume_list(cls, default_batch, batch_queue, quantity, weight, unit):
        ...
        while batch_queue:
            ...
            quantity -= update_quantity
            weight -= update_weight
            cls._update_batch_consume(
                current_batch, update_quantity, update_weight)
    ...
    def _cal_consume(cls, batch, quantity, weight, unit):
        ...
        if unit == 1:
            ...
            consume_quantity = cls._cal_update_digit(
                batch, quantity, batch_left)
...
class GoodsMasterBranchFunc():
    ...
    def get_goods_base_query(session, shop_id, merge_branch_goods=True, only_id=False):
        ...
        if merge_branch_goods:
            self_master = query_base.filter(
                models.Goods.supply_type == 0, models.Goods.is_master == 1)
...
class GoodsSaleFunc():
    ...
    def get_sale_record_info(
        ...
        sale_record_info["pay_status"] = cls._get_sales_record_pay_status(
            sale_record)
        sale_record_info["pay_status_text"] = cls.get_sales_record_pay_status_text(
            sale_record)
        ...
        if sale_record.shop_customer_id:
            ...
            sale_record_info["shop_customer"] = '%s %s ' % (
        ...
        if sale_record.order_id:
            ...
            combine_pay_dict = OrderBaseFunc.multi_combine_pay_type_text(session, [
                                                                         order_id])
        ...
        if sale_record.order_id:
            ...
            if order and order.erase_money > 0:
                ...
                fact_total_price = check_float(
                    (-fact_total_price if record_type == 1 else fact_total_price) / 100)
            ...
            if repeat_print and sale_record.pay_type == 9 and order.customer_signature:
                sale_record_info["signature_img"] = CustomerSignature.get_signature_image(
    ...
    def get_temp_sale_order_info(
        ...
        if source == "salesman":
            ...
            temp_order_info["signature_img"] = (
                redis.get(redis_key) or b"").decode()
            # 重复打印取签名
            if repeat_print and pay_type == 9 and tally_order_id:
                if tally_order.customer_signature:
                    temp_order_info["signature_img"] = CustomerSignature.get_signature_image(
                        tally_order_id)
    ...
    def query_sales_record_by_billing_record_new(
        ...
        # 自然日
            ...
            base_filter_list += [BillingSalesRecord.bill_date >=
                                 start_date, BillingSalesRecord.bill_date <= end_date]
        ...
        if only_paid and only_unpaid:
            ...
            filter_param_three = and_(GoodsSalesRecord.status.in_(
                [3, 4, 5]), GoodsSalesRecord.accountant_id > 0)
            if only_refund:
            ...
            base_filter_list.append(BillingSalesRecord.accountant_id == 0)
            filter_param_three = and_(GoodsSalesRecord.status.in_(
                [1, 2]), GoodsSalesRecord.accountant_id <= 0)
        ...
        if not (only_unpaid or only_refund or goods_ids or customer_id_list or export):
            ...
            if all_multi_record_id_list:
                multi_goods_customer = session \
                    ...
                    .filter(GoodsSalesRecord.multi_order_id.in_(all_multi_record_id_list)) \
                    .all()
            customer_goods_id_set = set(
                single_goods_customer + multi_goods_customer)
            ...
            if diy_filter_list:
                query_base = query_base.filter(
                    *diy_filter_list).order_by(BillingSalesRecord.id.desc())
            else:
                query_base = query_base.order_by(BillingSalesRecord.id.desc())
            page_sum = math.ceil(query_base.count() / page_size)
            query_base = query_base.offset(page * page_size).limit(page_size)
        ...
        # 要支持货品id或者操作人筛选，那么需要join
            ...
            base_query = session.query(BillingSalesRecord, GoodsSalesRecord).join(
                *first_join).filter(*base_filter_list)
            ...
            if not export:
                ...
                for item in base_result:
                    ...
                    # 一个票据中可能有多个货品，多品
                    goods_list = goods_record_dict.get(
                        billing_record_id, list())
                ...
                if is_multi == 0:
                    ...
                    data_dict = {
                        ...
                        TimeFunc.time_to_str(
                            first_goods_record.order_time, "time"),
        ...
        for r in single_record_list:
            ...
            if not simplify:
                data = dict(
                    ...
                    pay_status=GoodsSaleFunc.get_sales_record_pay_status_text(
                        r),
                    ...
                    commission_sumup=check_float(
                        r.get_commission_money() / 100),
                    ...
                    goods_id=r.goods_id,
                    full_num=r.num,
                    customer_name=customer_name_dict.get(
                        r.shop_customer_id, "散客"),
                    ajust=ajust,
    ...
    def _parse_multi_records(
        ...
        for record in multi_record_list:
            map_multi_records.setdefault(
                record.multi_order_id, []).append(record)
    ...
    def list_sales_each_goods_salesman(
        ...
        query_base = statistic_session.query(
            ...
            func.sum(sTable.sales_pay_count + 2 * \
                     sTable.refund_count).label("count"),
            *columns).filter_by(
    ...
    def query_and_parse_refunds(cls, session, sales_record_id_list):
        ...
        origin_record_ids = {
            r.origin_goods_sale_record_id for r in refund_infos}
...
class DebtFunc():
    ...
    def update_debt(cls, session, debt_type, debt_value, debtor_id, shop_id, operator_id, fk_id, remark="",
        ...
        debtor = session.query(models.ShopCustomer).filter_by(
            id=debtor_id).first()
        shop = session.query(models.Shop).filter_by(
            id=shop_id).with_lockmode('update').first()
        if debt_type_int == 0:
...
class AccoutantFunc():
    ...
    def update_work_record(cls, session, accountant_id,
        ...
        if pay_type == "pb_combine":
            ...
            remittance_cent = order_money.remittance_cent if not is_reset else - \
                order_money.remittance_cent
            if cash_cent:
    ...
    def get_word_record_data(cls, work_record):
        if work_record.offduty_time:
            delta_day = (work_record.offduty_time -
                         work_record.onduty_time).days
            delta_seconds = (work_record.offduty_time -
                             work_record.onduty_time).seconds
        else:
        ...
        data['order_cash_count'] = work_record.order_cash_count + \
            work_record.order_combine_cash_count
        data['order_pos_money'] = check_float(
            work_record.order_pos_cent / 100 + work_record.order_combine_pos_cent / 100)
        data['order_pos_count'] = work_record.order_pos_count + \
            work_record.order_combine_pos_count
        data['order_ali_money'] = check_float(
            work_record.order_ali_cent / 100 + work_record.order_combine_ali_cent / 100)
        data['order_ali_count'] = work_record.order_ali_count + \
            work_record.order_combine_ali_count
        data['order_wx_money'] = check_float(
            work_record.order_wx_cent / 100 + work_record.order_combine_wx_cent / 100)
        data['order_wx_count'] = work_record.order_wx_count + \
            work_record.order_combine_wx_count
        data['order_cheque_money'] = check_float(
            work_record.order_cheque_cent / 100 + work_record.order_combine_cheque_cent / 100)
        data['order_cheque_count'] = work_record.order_cheque_count + \
            work_record.order_combine_cheque_count
        ...
        data['order_remittance_count'] = work_record.order_remittance_count + \
            work_record.order_combine_remittance_count
        data['order_bank_money'] = check_float(
            data['order_pos_money'] + data['order_remittance_money'])
        data['order_bank_count'] = data['order_pos_count'] + \
            data['order_remittance_count']
        ...
        data['pb_cash_money'] = check_float(
            work_record.pb_cash_cent / 100 + work_record.pb_combine_cash_cent / 100)
        data['pb_cash_count'] = work_record.pb_cash_count + \
            work_record.pb_combine_cash_count
        data['pb_pos_money'] = check_float(
            work_record.pb_pos_cent / 100 + work_record.pb_combine_pos_cent / 100)
        data['pb_pos_count'] = work_record.pb_pos_count + \
            work_record.pb_combine_pos_count
        data['pb_ali_money'] = check_float(
            work_record.pb_ali_cent / 100 + work_record.pb_combine_ali_cent / 100)
        data['pb_ali_count'] = work_record.pb_ali_count + \
            work_record.pb_combine_ali_count
        data['pb_wx_money'] = check_float(
            work_record.pb_wx_cent / 100 + work_record.pb_combine_wx_cent / 100)
        data['pb_wx_count'] = work_record.pb_wx_count + \
            work_record.pb_combine_wx_count
        data['pb_cheque_money'] = check_float(
            work_record.pb_cheque_cent / 100 + work_record.pb_combine_cheque_cent / 100)
        data['pb_cheque_count'] = work_record.pb_cheque_count + \
            work_record.pb_combine_cheque_count
        data['pb_remittance_money'] = check_float(
            work_record.pb_remittance_cent / 100 + work_record.pb_combine_remittance_cent / 100)
        data['pb_remittance_count'] = work_record.pb_remittance_count + \
            work_record.pb_combine_remittance_count
        data['pb_bank_money'] = check_float(
            data['pb_pos_money'] + data['pb_remittance_money'])
        data['pb_bank_count'] = data['pb_pos_count'] + \
        ...
        data['refund_bank_count'] = data['refund_pos_count'] + \
            data['refund_remittance_count']

        # 收银台支出
        data['payout_total_money'] = check_float(
            work_record.deposit_refund_cent/100 + work_record.payout_cent/100 + work_record.supplier_borrow_cent/100)
        data['payout_total_count'] = work_record.deposit_refund_count + \
            work_record.payout_count + work_record.supplier_borrow_count
        data['payout_cash_money'] = check_float(
            work_record.deposit_refund_cash_cent/100 + work_record.payout_cash_cent/100 + work_record.supplier_borrow_cash_cent/100)
        data['payout_cash_count'] = work_record.deposit_refund_cash_count + \
            work_record.payout_cash_count + work_record.supplier_borrow_cash_count
        data['payout_pos_money'] = check_float(
            work_record.deposit_refund_pos_cent/100 + work_record.payout_pos_cent/100 + work_record.supplier_borrow_pos_cent/100)
        data['payout_pos_count'] = work_record.deposit_refund_pos_count + \
            work_record.payout_pos_count + work_record.supplier_borrow_pos_count
        data['payout_ali_money'] = check_float(
            work_record.deposit_refund_ali_cent/100 + work_record.payout_ali_cent/100 + work_record.supplier_borrow_ali_cent/100)
        data['payout_ali_count'] = work_record.deposit_refund_ali_count + \
            work_record.payout_ali_count + work_record.supplier_borrow_ali_count
        data['payout_wx_money'] = check_float(
            work_record.deposit_refund_wx_cent/100 + work_record.payout_wx_cent/100 + work_record.supplier_borrow_wx_cent/100)
        data['payout_wx_count'] = work_record.deposit_refund_wx_count + \
            work_record.payout_wx_count + work_record.supplier_borrow_wx_count
        data['payout_cheque_money'] = check_float(
            work_record.deposit_refund_cheque_cent/100 + work_record.payout_cheque_cent/100 + work_record.supplier_borrow_cheque_cent/100)
        data['payout_cheque_count'] = work_record.deposit_refund_cheque_count + \
            work_record.payout_cheque_count + work_record.supplier_borrow_cheque_count
        data['payout_remittance_money'] = check_float(
            work_record.deposit_refund_remittance_cent/100 + work_record.payout_remittance_cent/100 + work_record.supplier_borrow_remittance_cent/100)
        data['payout_remittance_count'] = work_record.deposit_refund_remittance_count + \
            work_record.payout_remittance_count + \
            work_record.supplier_borrow_remittance_count

        data['payout_bank_money'] = check_float(
            data['payout_pos_money'] + data['payout_remittance_money'])
        data['payout_bank_count'] = data['payout_pos_count'] + \
        ...
        data['income_total_count'] = work_record.other_income_count + \
            work_record.waste_selling_income_count
        data['income_cash_money'] = check_float(
            work_record.other_income_cash_cent / 100 + work_record.waste_selling_income_cash_cent / 100)
        data['income_cash_count'] = work_record.other_income_cash_count + \
            work_record.waste_selling_income_cash_count
        data['income_pos_money'] = check_float(
            work_record.other_income_pos_cent / 100 + work_record.waste_selling_income_pos_cent / 100)
        data['income_pos_count'] = work_record.other_income_pos_count + \
            work_record.waste_selling_income_pos_count
        data['income_ali_money'] = check_float(
            work_record.other_income_ali_cent / 100 + work_record.waste_selling_income_ali_cent / 100)
        data['income_ali_count'] = work_record.other_income_ali_count + \
            work_record.waste_selling_income_ali_count
        data['income_wx_money'] = check_float(
            work_record.other_income_wx_cent / 100 + work_record.waste_selling_income_wx_cent / 100)
        data['income_wx_count'] = work_record.other_income_wx_count + \
            work_record.waste_selling_income_wx_count
        ...
        data['income_bank_count'] = data['income_pos_count'] + \
        ...
        data['cash_discrepancy_money'] = check_float(
            cash_check_money - cash_expect_money)
        ...
        # 押金收支出
        data['deposit_spend'] = check_float(
            data['deposit_money'] - data['deposit_refund_money'])
        ...
        data['erase_bank_money'] = check_float(
            data['erase_pos_money'] + data['erase_remittance_money'])
        data['erase_bank_count'] = data['erase_pos_count'] + \
            data['erase_remittance_count']
        ...
        data['order_fee_bank_count'] = data['order_fee_pos_count'] + \
        ...
        data['total_remittance_count'] = data['order_remittance_count'] \
            ...
            + data['order_fee_remittance_count'] \
            - data['refund_remittance_count']
        data['total_bank_money'] = check_float(
            data['total_pos_money'] + data['total_remittance_money'])
        data['total_bank_count'] = data['total_pos_count'] + \
            data['total_remittance_count']
        return data
...
class OrderBaseFunc():
    ...
    def finish_order(cls, session, order, total_fee=0, if_pborder=False):
        ...
        if if_pborder:
            ...
            session.query(GoodsSalesRecordPaybackRecord).filter_by(
                pb_order_id=order_id).update({"status": 1})
            ...
            goods_sale_record_id_list = session.query(GoodsSalesRecordPaybackRecord.goods_sale_record_id,
                ...
                .all()
            payback_record_dict = {
                x.goods_sale_record_id: x.payback_cent for x in goods_sale_record_id_list}
            record_list = session.query(GoodsSalesRecord) \
            ...
            debt_record_id_list = session.query(HistoryPaybackRecord.debt_history_id,
                ...
                .all()
            payback_record_dict = {
                x.debt_history_id: x.payback_cent for x in debt_record_id_list}
        ...
        if if_pborder or order.pay_type != 9:
            ...
            # 组合支付需生成两条流水记录
            if fund_type == 10:
                cls.add_combine_order_fundflow(
                    session, shop_id, flow_source_type, fund_value, order.id, accountant_id)
            else:
                cls.add_order_fundflow(
                    session, shop_id, fund_type, flow_source_type, fund_value, fk_id, accountant_id)
            session.flush()
        ...
        if not if_pborder:
            ...
            if customer_id:
                if order_record_type == 0:
                    ...
                    customer = ShopCustomerFunc.get_shop_customer_through_id(
                        session, shop_id, customer_id)
                    ShopCustomerFunc.purchase_point(
                        session, shop_id, customer, order, accountant_id)
                else:
                    update_purchase_money = -order.fact_total_price
        ...
        if order.erase_money:
            ...
            AccoutantFunc.update_work_record(
                session, accountant_id, shop_id, order.erase_money, erase_pay_type_text, fk_id)
        ...
        if order.fee:
            ...
            AccoutantFunc.update_work_record(
                session, accountant_id, shop_id, order.fee, fee_pay_type_text, fk_id)

        # 收银员收银记录
        AccoutantFunc.update_work_record(
            session, accountant_id, shop_id, total_fee, pay_type_text, fk_id)
    ...
    def send_message_after_settlement(cls, session, order, if_pborder, is_refund=0):
        ...
        if customer_id:
            ...
            point_display = session.query(
                models.Config.point_display).filter_by(id=shop_id).scalar() or 0
            # 过滤特殊字符
            ...
            if customer.phone:
                if order.pay_type == 9:
                    errmsg = send_yunpian_credit_refund(
                        customer.phone, customer_name, shop_name, total_price, debt_rest, send_time, accountant_name)
                else:
                    errmsg = send_yunpian_refund(
                        customer.phone, customer_name, shop_name, total_price, send_time, accountant_name)
            else:
                errmsg = "no phone registered"

            if isinstance(errmsg, str) and errmsg:
                log_msg("refund_sms_error", str(
                    [order.debtor_id, customer.phone, errmsg, send_time]), "%Y-%m")
    ...
    def add_combine_order_fundflow(
        ...
        if cash_cent:
            ...
            cls.add_order_fundflow(
                session, shop_id, fund_type, source_type, fund_value, order_id, accountant_id)
        if pos_cent:
            fund_type = 2
            fund_value = pos_cent
            cls.add_order_fundflow(
    ...
    def add_order_fundflow(cls, session, shop_id, fund_type, source_type, fund_value, fk_id, accountant_id,
        ...
        fund_flow_new = models.FundFlow(shop_id=shop_id,
                                        ...
                                        create_day=t_day,
                                        create_week=t_week,
                                        create_time=create_date_time.strftime(
                                            '%H:%M:%S'),
    ...
    def barpay_lcsw_order(cls, session, shop_id, auth_code, order, if_pborder=False):
        ...
        log_msg('lc_bar_parameters/%s' %
                log_date, 'order_num:%s' % (order_num))
        log_msg('lc_bar_parameters/%s' % log_date, 'datetime:%s' %
        ...
        log_msg('lc_bar_res/%s' % log_date, 'datetime:%s' %
                (str(datetime.datetime.now())))
    ...
    def wappay_url_lcsw(cls, session, shop_id, shop_name, total_fee, wap_order):
        ...
        parameters = LcswPay.getWapPayParas(lc_merchant_no, lc_terminal_id, terminal_trace, total_fee, shop_name,
                                            lc_access_token)
        pay_url = LCSW_HANDLE_HOST + '/open/wap/110/pay?' + \
            urlencode(cls.encode_dict(parameters))
        return pay_url
    ...
    def multi_combine_pay_type_text(cls, session, order_id_list, source_type="order"):
        ...
        for order_id in pay_dict:
            pay_type_dict = pay_dict[order_id]
            pay_list = ["%s:%s元" % (pay_type, pay_money)
    ...
    def batch_format_order_record_info(
        ...
        if not get_wximgurl:
            ...
            staff_name_dict, staff_wximgurl_dict = AccountFunc.\
                get_account_name_wximg_through_id_list(
                    session, account_id_list)
        customer_name_dict = ShopCustomerFunc.get_shop_customer_name_through_id_list(
            session, customer_id_list)
        goods_name_unit_dict = GoodsFunc.get_goods_name_unit_through_id_list(
        ...
        for record in sale_records:
            ...
            accountant_name = staff_name_dict.get(
                accountant_id, "收银员%d" % accountant_id)
            ...
            salesman_id = record.salesman_id
            salesman_name = staff_name_dict.get(
                salesman_id, "开票员%d" % salesman_id)
            ...
            # 商品名称（带供货商名称）
            record_info["goods_name"] = goods_name_with_supplier
            ...
            # 已售重量
            record_info["sales_num"] = "-" if goods_info["unit"] == 1 else sales_num
...
class StorageRecordFunc():
    ...
    def format_export_storage_record_body(cls, session, record_list, shop_id,
        ...
        for storage_record in record_list:
            ...
            if record_info["type_id"] in (1, 4):
                foreign_record_num_info = '入库单号：' + \
                    record_info["foreign_record_num"]
            elif record_info["type_id"] in (6, 7):
                foreign_record_num_info = '报损单号：' + \
                    record_info["foreign_record_num"]
            elif record_info["type_id"] in (8, 9, 10):
                foreign_record_num_info = '调货单号：' + \
                    record_info["foreign_record_num"]
            elif record_info["type_id"] in (11,):
                foreign_record_num_info = '盘点单号：' + \
                    record_info["foreign_record_num"]
            else:
            ...
            data_content.append([
                ...
                str(record_info["mul"]) + '件/' +
                str(record_info["num"]) + weight_unit_text,
                str(record_info["now_storage"]) + '件/' +
                str(record_info["now_storage_num"]) + weight_unit_text,
    ...
    def get_storage_allocate_end_time_by_storage_allocate_id(cls, session, storage_allocate_id):
        """获取库存调度的完成时间"""
        GoodsStorageRecord = models.GoodsStorageRecord
        filter_list = [GoodsStorageRecord.fk_id ==
                       storage_allocate_id, GoodsStorageRecord.type.in_([8, 9, 10])]
...
class SupplierFlowFunc():
    ...
    def _get_clearing_procedure_fee(cls, session, clearing_id_list):
        ...
        procedure_list = session \
            ...
            .all()
        procedure_dict = {x.id: check_float(
            x.procedure_cent/100) for x in procedure_list}
    ...
    def batch_format_borrowing_flow_info(cls, session, shop_id, flows_borrowing_records, data_type=0):
        ...
        # 查询操作员信息
        operater_id_list = {flow.operater_id for flow,
                            _ in flows_borrowing_records}
        operator_name_dict, operator_wximgurl_dict = AccountFunc.\
            get_account_name_wximg_through_id_list(session, operater_id_list)
        # 查询供货商信息
        supplier_id_list = {flow.shop_supplier_id for flow,
                            _ in flows_borrowing_records}
        supplier_info_dict = SupplierFunc.get_shop_supplier_base_info(
            session, shop_id, supplier_id_list)
        # 查询商品信息
    ...
    def batch_format_stockin_history_info(cls, session, flows, weight_unit_text,
        ...
        for flow in flows:
            ...
            data = dict(
                ...
                operator_wximgurl=map_account_wximgurl.get(
                    flow.operator_id, ""),
                weight_unit=weight_unit_text,
        ...
        for flow in flows:
            ...
            if hasattr(flow, "wx_unionid"):
                data["operator_wximgurl"] = models.AddImgDomain. \
                    add_domain_headimgsmall(
                        flow.headimgurl) if flow.wx_unionid else ""
            if return_type == "dict":
...
class SalesRecordExportFunc():
    ...
    def format_export_body(cls, session, shop_id, salerecord_list):
        ...
        if distinguish_weight:
            data_content.append(['自然日期', '扎账日期', '流水号', '分组名', '货品名', '供货商', '单价', '单位',
                                 '重量/{}'.format(
                                     weight_unit_text), '毛重', '皮重', '件数', fee_text, '押金', '货品小计',
            ...
            for sale_record in salerecord_list:
                ...
                data_content.append([
                    ...
                    TimeFunc.time_to_str(
                        sale_record.account_period_date, "date"),
                    "'" + record_info["num"],
    ...
    def format_export_body_custom(cls, session, shop_id, salerecord_list,
        ...
        if have_headline:
            ...
            salesman_dict = {
                x.id: x.realname or x.nickname for x in all_salesman}

            # 获取收银员
            all_accountant = hire_link_base.filter(
                HireLink.active_accountant == 1).all()
            accountant_dict = {
                x.id: x.realname or x.nickname for x in all_accountant}
            ...
            all_group = session.query(
                GoodsGroup.id, GoodsGroup.name).filter_by(shop_id=shop_id).all()
            ...
            for _key in export_item_list:
                ...
                if _key == 12:
                    ...
                    key_name = item_cn_name_dict.get(_key, "未知")
                headline_list.append(key_name)
                sum_list.append(data_sum.get(
                    item_en_name_dict.get(_key, ""), ""))
            ...
            for sale_record in salerecord_list:
                ...
                for _key in export_item_list:
                    if _key == 1:
                        ...
                        continue
                    if _key == 2:
                        _content_list.append(TimeFunc.time_to_str(
            ...
            if not (source == "salesman_async" and multi_order_dict):
                ...
                if single_record_list:
                    ...
                    record_counts = session \
                        ...
                        .all()
                    record_count_dict = {data[0]: data[1]
...
class WirelessPrintFunc():
    ...
    def __init__(self, print_type, _paper_width=32, shop=None):
        ...
        if print_type == "smposprint":
            ...
            print_type_dict = {1: "ylyprint", 2: "feprint",
                               3: "sgprint", 4: "80localprint"}
    ...
    def color_invert(self, str_data, cancel=False):
        ...
        if self._type == "smposprint":
            ...
            _, num = re.search('style=', str_data).span()
            str_data = "<p style='background-color:#000;-webkit-print-color-adjust: exact;color:#fff;" + \
                str_data[num+1:]
            return str_data
        else:
            length = len(str_data)
    ...
    def inline_block(self, str_data):
        ...
        _, num = re.search('style=', str_data).span()
        str_data = "<p style='display:inline-block;min-width: 40%;box-sizing:border-box;" + \
            str_data[num + 1:]
        return str_data
        ...
        if source == "salesman":
            ...
            if receipt_config.salesman_customer_receipt:
                ...
                receip_temp = ReceiptTemplateFunc.get_receipt_temp_detail(
                    temp_name, shop_id)
                salesman_customer_title = '[客户联]' + \
                    self._salesman_customer_space + '提货撕毁'
            ...
            if receipt_config.cashier_goods_receipt and \
                ...
                if shop_id in [116, 105]:
                    ...
                    cashier_goods_title = '[客户提货联]' + \
                        self._cashier_goods_space + '提货撕毁'
                content_body_goods = float_middle(
                    zoom_in(accounting_title, default_size))
                content_body_goods += line_break(
    ...
    def format_receipt_temp(
        ...
        if accountant_time:
            accountant_text = "收银信息：%s  %s" % (
                accountant_name, accountant_time)
        ...
        if newline:
            if last_newline:
                ...
                if _type == '80localprint':
                    ...
                    if last_string:
                        content_len = len(
                            origin_text) + CharFunc.get_contain_chinese_number(origin_text)
                        last_content_len = len(
                            last_string) + CharFunc.get_contain_chinese_number(last_string)
                        content_box_len = self._paper_width - last_content_len - 28
                        if content_box_len <= 4:
                            text = "  " + text
                        text = line_break(
                            fill_box(text, content_box_len, content_len, float_dir="right"))
                    else:
                        text = line_break(text)
        else:
    ...
    def get_dot_body(self, sale_record_num):
        ...
        if _type in ["feprint", "sgprint"]:
            ...
            for i in range(len(first_chinese_array)):
                tmp_content1 = "" + \
                               first_chinese_array[i] + second_chinese_array[i] + \
                    third_chinese_array[i] + ""
        ...
        if source == "salesman":
            ...
            if receipt_config.cashier_goods_receipt and \
                ...
                content_body_data = float_middle(
                    zoom_in(accounting_title, default_size))
                content_body_data += line_break(
                    zoom_in('[客户提货联]' + self._cashier_goods_space + '提货撕毁', default_size))
                content_body_data += content_main_body
                content_body += content_body_data
                content_body_list.append(content_body_data)
    ...
    def format_multi_receipt_temp(
        ...
        combine_pay_type_text = temp_order_info.get(
        ...
        for sale_record_info in sale_record_info_list:
            ...
            if collect_multi_info and collect_multi_info.get(sale_record_num):
                ...
                commission_mul_total = multi_info.get(
                    "commission_mul_total", "")
    ...
    def get_work_record_print_body(
        ...
        order_cash_money = "现金：%s元  %s笔" % (
            record_data["order_cash_money"], record_data["order_cash_count"])
        order_bank_money = "银行卡：%s元  %s笔" % (
            record_data["order_bank_money"], record_data["order_bank_count"])
        order_cheque_money = "支票：%s元  %s笔" % (
            record_data["order_cheque_money"], record_data["order_cheque_count"])
        order_wx_money = "微信：%s元  %s笔" % (
            record_data["order_wx_money"], record_data["order_wx_count"])
        order_ali_money = "支付宝：%s元  %s笔" % (
            record_data["order_ali_money"], record_data["order_ali_count"])

        pb_total_money = "还款：%s元" % (record_data["pb_total_money"])
        pb_cash_money = "现金：%s元  %s笔" % (
            record_data["pb_cash_money"], record_data["pb_cash_count"])
        pb_bank_money = "银行卡：%s元  %s笔" % (
            record_data["pb_bank_money"], record_data["pb_bank_count"])
        pb_cheque_money = "支票：%s元  %s笔" % (
            record_data["pb_cheque_money"], record_data["pb_cheque_count"])
        pb_wx_money = "微信：%s元  %s笔" % (
            record_data["pb_wx_money"], record_data["pb_wx_count"])
        pb_ali_money = "支付宝：%s元  %s笔" % (
            record_data["pb_ali_money"], record_data["pb_ali_count"])

        refund_total_money = "退单：%s元" % (record_data["refund_total_money"])
        refund_cash_money = "现金：%s元  %s笔" % (
            record_data["refund_cash_money"], record_data["refund_cash_count"])
        refund_bank_money = "银行卡：%s元  %s笔" % (
            record_data['refund_bank_money'], record_data['refund_bank_count'])
        ...
        erase_cash_money = "现金：%s元  %s笔" % (
            record_data["erase_cash_money"], record_data["erase_cash_count"])
        erase_bank_money = "银行卡：%s元  %s笔" % (
            record_data["erase_bank_money"], record_data["erase_bank_count"])
        erase_cheque_money = "支票：%s元  %s笔" % (
            record_data["erase_cheque_money"], record_data["erase_cheque_count"])
        erase_wx_money = "微信：%s元  %s笔" % (
            record_data["erase_wx_money"], record_data["erase_wx_count"])
        erase_ali_money = "支付宝：%s元  %s笔" % (
            record_data["erase_ali_money"], record_data["erase_ali_count"])
        erase_combine_money = "组合支付：%s元  %s笔" % (
            record_data["erase_combine_money"], record_data["erase_combine_count"])

        payout_total_money = "收银台支出：%s元" % (record_data["payout_total_money"])
        income_total_money = "额外收入：%s元" % (record_data["income_total_money"])

        order_fee_money = "手续费：%s元 %s笔" % (
            record_data["order_fee_money"], record_data["order_fee_count"])
        order_fee_cash_money = "现金：%s元  %s笔" % (
            record_data["order_fee_cash_money"], record_data["order_fee_cash_count"])
        order_fee_bank_money = "银行卡：%s元  %s笔" % (
            record_data["order_fee_bank_money"], record_data["order_fee_bank_count"])
        order_fee_cheque_money = "支票：%s元  %s笔" % (
            record_data["order_fee_cheque_money"], record_data["order_fee_cheque_count"])
        order_fee_wx_money = "微信：%s元  %s笔" % (
            record_data["order_fee_wx_money"], record_data["order_fee_wx_count"])
        order_fee_ali_money = "支付宝：%s元  %s笔" % (
            record_data["order_fee_ali_money"], record_data["order_fee_ali_count"])

        total_money = "实收资金：%s元" % (record_data["total_money"])
        total_cash_money = "现金：%s元  %s笔" % (
            record_data["total_cash_money"], record_data["total_cash_count"])
        total_bank_money = "银行卡：%s元  %s笔" % (
            record_data["total_bank_money"], record_data["total_bank_count"])
        total_cheque_money = "支票：%s元  %s笔" % (
            record_data["total_cheque_money"], record_data["total_cheque_count"])
        total_wx_money = "微信：%s元  %s笔" % (
            record_data["total_wx_money"], record_data["total_wx_count"])
        total_ali_money = "支付宝：%s元  %s笔" % (
            record_data["total_ali_money"], record_data["total_ali_count"])
...
class ReceiptTemplateFunc():
    ...
    def get_default_receipt_temp(cls, temp_key):
        ...
        if temp_key in ["salesman_customer_receipt_temp", "salesman_async_customer_receipt_temp"]:
            default_temp = [
                {"name": "shop_name", "position": "center",
                    "size": "2", "print": 1, "newline": 1},
                {"name": "barcode", "position": "center",
                    "size": "1", "print": 1, "newline": 1},
                {"name": "goods_name", "position": "center",
                    "size": "3", "print": 1, "newline": 1},
                {"name": "piece", "position": "center",
                    "size": "3", "print": 1, "newline": 1},
                {"name": "sale_number", "position": "left",
                    "size": "2", "print": 1, "newline": 1},
                {"name": "batch_num", "position": "left",
                    "size": "2", "print": 0, "newline": 1},
                {"name": "weight", "position": "left",
                    "size": "2", "print": 1, "newline": 1},
                {"name": "price", "position": "left",
                    "size": "2", "print": 1, "newline": 1},
                {"name": "commission", "position": "left",
                    "size": "2", "print": 1, "newline": 1},
                {"name": "deposit", "position": "left",
                    "size": "2", "print": 1, "newline": 1},
                {"name": "total_goods_sale", "position": "left",
                    "size": "2", "print": 1, "newline": 1},
                {"name": "total_commission", "position": "left",
                    "size": "2", "print": 1, "newline": 1},
                {"name": "total_deposit", "position": "left",
                    "size": "2", "print": 1, "newline": 1},
                {"name": "receipt_money", "position": "left",
                    "size": "2", "print": 1, "newline": 1},
                {"name": "code", "position": "center",
                    "size": "1", "print": 0, "newline": 1},
                {"name": "shop_customer", "position": "center",
                    "size": "1", "print": 1, "newline": 1},
                {"name": "salesman_info", "position": "center",
                    "size": "1", "print": 1, "newline": 1},
                {"name": "accountant_info", "position": "center",
                    "size": "1", "print": 0, "newline": 1},
                {"name": "pay_info", "position": "center",
                    "size": "2", "print": 0, "newline": 1},
                {"name": "receipt_text", "position": "center",
                    "size": "1", "print": 1, "newline": 1},
                {"name": "receipt_img", "position": "center",
                    "size": "1", "print": 0, "newline": 1},
                {"name": "remark", "position": "left",
                    "size": "1", "print": 1, "newline": 1}
            ]
            ...
            default_temp = [
                ...
                {"name": "piece", "position": "center",
                    "size": "2", "print": 1, "newline": 1},
                {"name": "weight", "position": "center",
                    "size": "2", "print": 1, "newline": 1},
                {"name": "receipt_money", "position": "center",
                    "size": "2", "print": 1, "newline": 1},
                {"name": "sale_number", "position": "center",
                    "size": "2", "print": 0, "newline": 1},
                {"name": "batch_num", "position": "center",
                    "size": "2", "print": 0, "newline": 1},
                {"name": "price", "position": "left",
                    "size": "2", "print": 0, "newline": 1},
                {"name": "total_goods_sale", "position": "left",
            ...
            default_temp = [
                ...
                {"name": "pay_info", "position": "center",
                    "size": "2", "print": 1, "newline": 1},
                {"name": "shop_customer", "position": "left",
                    "size": "1", "print": 0, "newline": 1},
                {"name": "signature_img", "position": "center",
                    "size": "1", "print": 1, "newline": 1},
                {"name": "receipt_text", "position": "center",
                    "size": "1", "print": 1, "newline": 1},
                {"name": "receipt_img", "position": "center",
            ...
            default_temp = [
                ...
                {"name": "barcode", "position": "center",
                    "size": "1", "print": 1, "newline": 1},
                {"name": "code", "position": "center",
                    "size": "3", "print": 0, "newline": 1},
                {"name": "order_num", "position": "left",
                    "size": "2", "print": 1, "newline": 1},
                {"name": "category", "position": "left",
                    "size": "2", "print": 0, "newline": 0},
                {"name": "total_commission_mul", "position": "left",
                    "size": "2", "print": 1, "newline": 1},
                {"name": "total_money", "position": "left",
                    "size": "2", "print": 1, "newline": 1},
                {"name": "shop_customer", "position": "left",
                    "size": "2", "print": 1, "newline": 1},
                {"name": "goods_list", "style": [
                    ...
                    {"name": "batch_num", "position": "left",
                        "size": "1", "print": 0, "newline": 1},
                    {"name": "goods_name", "position": "left",
                        "size": "2", "print": 1, "newline": 1},
                    {"name": "piece", "position": "left",
                        "size": "2", "print": 1, "newline": 0},
                    {"name": "receipt_money", "position": "left",
                        "size": "2", "print": 1, "newline": 1},
                    {"name": "price", "position": "left",
                        "size": "1", "print": 1, "newline": 0},
                    {"name": "total_goods_sale", "position": "left",
                        "size": "1", "print": 1, "newline": 1},
                    {"name": "commission", "position": "left",
                        "size": "1", "print": 1, "newline": 0},
                ...
                {"name": "pay_info", "position": "center",
                    "size": "1", "print": 0, "newline": 1},
                {"name": "erase_text", "position": "center",
                    "size": "1", "print": 1, "newline": 1},
                {"name": "receipt_text", "position": "center",
                    "size": "1", "print": 1, "newline": 1},
                {"name": "remark", "position": "left",
...
class WirelessPrintInfoFunc():
    ...
    def get_printer_info(cls, session, printer_id):
        ...
        if printer_id:
            printer = session.query(models.Printer).filter_by(
                id=printer_id).first()
...
class AccountFunc():
    ...
    def get_account_wx_head_imgurl_through_id_list(cls, session, account_id_list):
        if account_id_list:
            ...
            account_dict = {
                x.id: x.head_imgurl_small if x.wx_unionid else "" for x in operator_list}
        else:
            account_dict = {}
        return account_dict
    ...
    def get_account_realname(cls, session, account_id):
        Accountinfo = models.Accountinfo
        realname = session.query(Accountinfo.realname).filter_by(
            id=account_id).scalar() or ""
        return realname
    ...
    def get_account_name(cls, session, account_id):
        Accountinfo = models.Accountinfo
        account = session.query(Accountinfo.realname, Accountinfo.nickname).filter_by(
            id=account_id).first()
        if account:
    ...
    def get_account_phone(cls, session, account_id):
        Accountinfo = models.Accountinfo
        phone = session.query(Accountinfo.phone).filter_by(
            id=account_id).scalar() or ""
        return phone
    ...
    def get_account_id_through_phone(cls, session, phone):
        Accountinfo = models.Accountinfo
        _id = session.query(Accountinfo.id).filter_by(
            phone=phone).scalar() or 0
        return _id
    ...
    def format_export_supplier_goods_stockin(cls, data_list, total_data_sum, weight_unit_text):
        ...
        for data in data_list:
            data_content.append([
                ...
                data["payback_cent"] if data["supply_type"] not in [
                    1, 2] else "-",
                data["should_pay_cent"] if data["supply_type"] not in [
                    1, 2] else "-"
            ])
            i += 1
        data_content.append([
...
class SupplierSettlementFunc():
    ...
    def get_unclearing_money_dict(cls, session, shop_id, agents_goods_id_list, return_type="",
        ...
        if clearing_record_ids is None:
            ...
            proxy_filter_params.append(
                not_(GoodsSalesRecord.clearing_record_id.in_(clearing_record_ids)))
            proxy_filter_params.append(
        ...
        if clearing_record_ids is None:
            ...
            self_filter_params.append(
                not_(StockInDocsDetail.clearing_record_id.in_(clearing_record_ids)))
    ...
    def list_clearing_sum_each_day(cls, session, sales_record_id_list, borrow_id_list, data_type):
        ...
        payback_records = session \
            ...
            .filter(
                models.GoodsSalesRecordPaybackRecord.goods_sale_record_id.in_(
                    sales_record_id_list),
...
class GoodsRateFunc():
    ...
    def get_rate_type_text(cls, goods_rate, fee_text="", weight_unit_text="斤"):
        ...
        if goods_rate.amount_active == 1:
            rate_type_text = "按总金额的%s%%" % (
                check_float(goods_rate.amount_cent/100))
            rate_list.append(rate_type_text)
...
class ShopCustomerFunc():
    ...
    def purchase_point(cls, session, shop_id, shop_customer, order, operator_id):
        ...
        if rule.purchases:
            ...
            cls.update_point(session, shop_id, shop_customer, point_delta,
                             operator_id, operate_type=1, rule_id=rule.id)
        ...
        if rule.weekly_purchase or rule.monthly_purchase:
            ...
            if rule.weekly_purchase:
                ...
                if not point_history:
                    ...
                    days_purchased_this_week = len(
                        {d[0].date(): "" for d in order_dates_this_week})
                    # 采购天数达标则增加积分
            ...
            if rule.monthly_purchase:
                ...
                first_day_this_month = today + \
                    datetime.timedelta(days=-today.day + 1)
                ...
                if not point_history:
                    ...
                    days_purchased_this_month = len(
                        {d[0].date(): "" for d in order_dates_this_month})
                    # 采购天数达标则增加积分
    ...
    def format_customer_info(cls, session, customer_list, type="list"):
        ...
        for i in customer_list:
            customer_info = i if isinstance(i, models.ShopCustomer) else i[0]
            purchase_info = None if isinstance(
                i, models.ShopCustomer) else i[1]
            # 查询担保人和创建人名字
            ...
            if black_info:
                defriend_time = TimeFunc.time_to_str(
                    black_info.edit_time, "date")
                ...
                defriend_operator_name = AccountFunc.get_account_name(
                    session, defriend_operator_id) if defriend_operator_id else ""
...
class UrlShorten():
    ...
    def get_md5(self, longurl):
        longurl = longurl.encode('utf8') if isinstance(
            longurl, str) else longurl
        m = hashlib.md5()
        m.update(longurl)
        return m.hexdigest()
    ...
    def get_short_url(self, long_url):
        url = self.session.query(models.ShortUrl).filter_by(
            long_url=long_url).first()
        if url:
...
class GoodsOperationRecordFunc:
    ...
    def get_operation_detail(cls, session, records, weight_unit_text="斤"):
        ...
        result.update(cls.get_goods_group_change_data(
            session, data[7] | data[8]))
    ...
    def get_goods_allocate_data(cls, session, records, weight_unit_text="斤"):
        ...
        goods_allocate = session.query(StorageAllocateGoods.id,
            ...
            .all()
        goods_allocate_dict = {i.id: (i.quantity, i.weight)
        ...
        receive_goods_id_dict = {
            i.id: i.receive_goods_id for i in goods_allocate}
        all_goods_ids_list = list(
            goods_id_dict.values()) + list(receive_goods_id_dict.values())
        goods_name_dict = GoodsFunc.get_goods_name_through_id_list(
    ...
    def get_goods_stockin_data(cls, session, records, weight_unit_text="斤"):
        ...
        goods_stockin = session.query(GoodsStockinRecord.id,
            ...
            .all()
        goods_stockin_dict = {i.id: (i.quantity, i.weight)
    ...
    def get_goods_name_edit_data(cls, session, records):
        ...
        for i in records:
            ...
            if t:
                data[i.id] = dict(
                    goods_id=t.goods_id, origin_name=t.origin_name, now_name=t.now_name)
            else:
                data[i.id] = dict(
        ...
        for i in records:
            ...
            if t:
                data[i.id] = dict(goods_id=t.goods_id,
                                  goods_name=goods_name_dict.get(
                                      t.goods_id, ""),
                                  origin_group_name=group_name_dict.get(
                                      t.origin_group_id, ""),
                                  now_group_name=group_name_dict.get(t.now_group_id, ""))
            else:
                data[i.id] = dict(goods_id=None, goods_name=None,
                                  origin_group_name=None, now_group_name=None)
        return data
    ...
    def get_goods_breakage_data(cls, session, records, weight_unit_text="斤"):
        ...
        goods_breakage = session.query(GoodsBreakageRecord.id,
            ...
            .all()
        goods_breakage_dict = {i.id: (i.quantity, i.weight)
    ...
    def get_goods_inventory_data(cls, session, records, weight_unit_text="斤"):
        ...
        goods_inventory = session.query(GoodsInventoryRecord.id,
            ...
            .all()
        goods_inventory_dict = {i.id: (i.quantity, i.weight)
...
class AuthFunc:
    ...
    def merge_passport(cls, session, passport_id, phone):
        ...
        merge_key = "passport_merge:{}|{}:pf".format(
            source_passport_id, target_passport_id)
    ...
    def login_by_wx(cls, session, wx_userinfo):
        ...
        data = resp["data"]
        user = session.query(models.Accountinfo).filter_by(
            passport_id=data["id"]).first()
        if not user:
...
class SalesRecordAjustFunc():
    ...
    def get_ajust_info(cls, session, record):
        ...
        if ajust_record:
            ...
            # 原始单号ID
            ajust_info["ajust_origin_id"] = ajust_record.origin_record_id
            # 冲红单号ID
            ajust_info["ajust_refund_id"] = ajust_record.refund_record_id
    ...
    def get_multi_ajust_info(cls, session, record_id_list):
        ...
        ajust_record_id_list = {
            x.origin_record_id for x in ajust_record_id_list}
...
class StockInFunc():
    ...
    def get_flow(cls, session, shop_id, page, **kwargs):
        ...
        # 排序条件 0: 按流水 ID 1: 按入库重量 2: 按入库件数 3: 按费用
        order_condition = kwargs.get("order_condition", 0)
    ...
    def get_statistic_each_goods(cls, cls_doc_detail, cls_doc, shop_id, session, page, **kwargs):
        ...
        goods_id_list = kwargs.get("goods_id_list", [])
        group_id_list = kwargs.get("group_id_list", [])
        # 排序条件 0: 按货品 ID 1: 按入库重量 2: 按入库件数 3: 按费用
        order_condition = kwargs.get("order_condition", 0)
        ...
        if cls_doc_detail == models.StockInDocsDetail:
            query_list.append(func.sum(case(
                [(cls_doc_detail.pay_type == 5, cls_doc_detail.total_price)])).label("credit_money"))
        ...
        for branch_statistics_item in branch_statistics:
            branch_statistics_dict[branch_statistics_item.master_goods_id].append(
        ...
        for master_statistic in total_master_statistic:
            total_data_sum["total_weight"] += UnitFunc.convert_weight_unit(
                master_statistic.weight, source="weight", weight_unit_text=weight_unit_text)
        ...
        g_dict = GoodsFunc.get_goods_in_shop(
            session, shop_id, weight_unit_text, goods_id_list, merge_self_goods=False)
        ...
        def prepare_data(doc_detail, goods, detail_unit, type):
            data_dict = dict(
                ...
                quantity=check_float(
                    doc_detail.quantity / 100) if doc_detail.quantity else "-",
        ...
        for doc in statistic:
            ...
            if doc.is_master:
                ...
                for branch_doc in branch_docs:
                    detail_unit = map_detail_unit.get(
                        branch_doc.goods_id, branch_doc.unit)
                    branch_goods = g_dict[branch_doc.goods_id]
                    branch_data = prepare_data(
                        branch_doc, branch_goods, detail_unit, cls_doc_detail)
...
class LcBankFunc():
    ...
    def get_bank_name_dict(cls, session, bank_nos):
        banks = session \
            ...
            .all()
        bank_name_dict = {
            bank.parent_bank_no: bank.parent_bank_name for bank in banks}
...
class SalesmanFunc():
    ...
    def list_goods_id_editable(cls, session, shop_id, account_id, permission_text_list):
        ...
        if "editgoods" in permission_text_list:
            ...
            result = cls._list_goods_id_created_by(
...
class CustomerSignature():
    ...
    def get_signature_image(cls, fileid, source="order"):
        ...
        if ret.status_code == 200:
            signature_image = str(base64.b64encode(
...
class ShopRegisterFunc():
    ...
    def get_shop_apply_pay_status_by_user_id(session, user_id):
        ...
        shop_apply = session.query(ShopApply).filter(
            ShopApply.account_id == user_id, ShopApply.status.in_([0, 1])).first()
        if not shop_apply:
            return None
...
class FileToolFunc():
    ...
    def set_shop_export_status(shop_id, user_id, exporting_key, seconds=5):
        """设置店铺导出状态，默认十秒"""
        redis_export.setex("exporting:%d-%d" %
                           (shop_id, user_id), seconds, exporting_key)
