...
class TimeFunc():
    ...
    def get_fake_clearing_state(cls, the_date):
        delta_day = (datetime.datetime.combine(datetime.datetime.now(), datetime.time.min) - datetime.datetime.combine(
            the_date, datetime.time.min)).days
    ...
    def transfer_input_range_to_date_or_default_range(cls, from_date, to_date):
        ...
        :rtype: tuple
        """
        from_date, to_date = cls.transfer_input_range_to_date(from_date, to_date)
        if not from_date:
    ...
    @classmethod
    def transfer_range_normal_to_accounting(cls, session, shop_id, from_date, to_date):
        ...
        if not choose_date_end:
            choose_date_end = datetime.datetime(
    ...
    def get_to_date_by_from_date(cls, from_date, to_date, statistic_type):
        ...
        try:
            if statistic_type == 1:
                ...
                to_date = from_date
                to_date = datetime.datetime(year=to_date.year + 1, month=1, day=1)
        except ValueError:
            raise ValueError("日期格式传入错误，请检查")
        return from_date, to_date
...
class ShopFunc():
    ...
    def get_lcpay_account(cls, session, shop_id):
        ...
        if redis.get(lcpay_account_key):
            ...
            if shop:
                ...
                redis.set(lcpay_account_key, lc_merchant_no + "|" + lc_terminal_id + "|" + lc_access_token, 24 * 3600)
    ...
    def get_paid_info(cls, session, shop_id):
        info = {}
        info["multi_weigh_paid"] = FunctionPaidFunc.get_service_paid(session, shop_id, service_type=1)
        info["multi_sale_goods_paid"] = FunctionPaidFunc.get_service_paid(session, shop_id, service_type=2)
        info["accounting_periods_paid"] = FunctionPaidFunc.get_service_paid(session, shop_id, service_type=5)
        info["courier"] = FunctionPaidFunc.get_service_paid(session, shop_id, service_type=6)
        return info

    # 小票设置
    @classmethod
    def get_receipt_config(cls, session, shop_id):
        receipt_config = session.query(models.ReceiptConfig).filter_by(id=shop_id).first()
    ...
    def make_shop_code(cls, session):
        ...
        while 1:
            ...
            shop = session.query(models.Shop.id).filter_by(shop_code=str).scalar()
            if not shop:
                break
        return str
    ...
    def get_shop_name_through_id(cls, session, shop_id):
        shop_name = session.query(models.Shop.shop_name).filter_by(id=shop_id).scalar() or ""
        return shop_name
    ...
    def list_goods_under_shop(cls, session, shop_id):
        """ 店铺内所有上架商品id """
        goods = session.query(models.Goods.id).filter_by(shop_id=shop_id, active=1).all()
    ...
    def get_supplier_ids_in_shop(cls,session,shop_id):
        """ 店鋪內所有供应商id """
        suppliers = session.query(models.ShopSupplier).filter_by(shop_id=shop_id, status=1).all()
    ...
    def is_admin_have_many_shop(cls, session, shop_id):
        ...
        shops_count = 0
        result = session.query(models.Shop.boss_id).filter_by(id=shop_id).first()
        if result:
            shops_count = session.query(models.Shop).filter_by(boss_id=result.boss_id).count()
    ...
    def is_max_staff(cls, session, shop_id, config=None):
        ...
        if not config:
            ...
            if result:
                max_accountant_count, max_admin_count, max_salesman_count = result[0], result[1], result[2]
            else:
...
class StatisticFunc():
    ...
    @classmethod
    def get_static_query_base(
            cls, session, sTable, statistic_type, extra_groups=None):
    ...
    def get_query_param(cls, sTable):
        ...
        :rtype: list
        """
        sum_param = [func.sum(getattr(sTable, column)).label(column)
                     for column in sTable.get_sum_column()]
        fix_param = [getattr(sTable, column).label(column)
    ...
    def is_after_accounting(cls, r_obj, statistic_type, today):
        ...
        elif statistic_type == 2 and (
                r_obj.year > today.year or
                r_obj.year == today.year and r_obj.week > this_week):
            is_after_accounting = 1
        elif statistic_type == 3 and (
                r_obj.year > today.year or
                r_obj.year == today.year and r_obj.month > today.month):
            is_after_accounting = 1
    ...
    @classmethod
    def update_statistic_data(self, session, statistic_session, shop_id, if_force_update=False):
        ...
        reslove_statistic.order(
            session, statistic_session, 0, shop_id, last_update_timestamp, new_timestamp)
        reslove_statistic.refund_order(
            session, statistic_session, 0, shop_id, last_update_timestamp, new_timestamp)
        reslove_statistic.goods_sale_record(
            session, statistic_session, 0, shop_id, last_update_timestamp, new_timestamp)
        reslove_statistic.pb_order(
            session, statistic_session, 0, shop_id, last_update_timestamp, new_timestamp)
        reslove_statistic.deposit_refund_order(
            session, statistic_session, 0, shop_id, last_update_timestamp, new_timestamp)
        reslove_statistic.fund_check(
            session, statistic_session, 0, shop_id, last_update_timestamp, new_timestamp)
    ...
    @classmethod
    def update_accounting_periods_statistic_data(self, session, statistic_session, shop_id, if_force_update=False):
        key_update_statistic = "accounting_periods_statistic:%d" % (int(shop_id))
        # 如果已经在进行更新中，就不进行更新，方法防抖
        if not redis.setnx(key_update_statistic, '1'):
            return
        # 设置过期时间30s,方法节流
        ...
        for time_type in update_time_types:
            reslove_statistic.order(
                session, statistic_session, time_type, shop_id, last_update_timestamp, new_timestamp)
            reslove_statistic.refund_order(
                session, statistic_session, time_type, shop_id, last_update_timestamp, new_timestamp)
            reslove_statistic.goods_sale_record(
                session, statistic_session, time_type, shop_id, last_update_timestamp, new_timestamp)
    ...
    @classmethod
    def get_funds_info_per_shop_statitic_instance(cls, single_fund, data_info, source="accountant"):
        ...
        data_info["bar_money"] = check_float((lc_fund_wxbar + lc_fund_alibar) / 100)
        data_info["wap_money"] = check_float((lc_fund_wxwap + lc_fund_aliwap) / 100)
        data_info["tag_money"] = check_float((lc_fund_wxtag + lc_fund_alitag) / 100)
        data_info["ali_money"] = check_float((lc_fund_alibar + lc_fund_alitag) / 100)
        data_info["wx_money"] = check_float((lc_fund_wxbar + lc_fund_wxtag) / 100)
        ...
        data_info["bank_money"] = check_float((fund_remittance + fund_pos) / 100)
    ...
    @classmethod
    def get_funds_info_per_accountant_statitic_instance(cls, single_accountant_fund, data_info):
    ...
    @classmethod
    def get_goods_stockin_data(cls, session, statistic_session, shop_id, goods_dict, weight_unit_text, args={}, if_total=False, only_goods=False):
        """
        货品入库记录日/月/年统计汇总数据
        :param page: 页码
        :param year: 年份
        ...
        # 基础查询
        if only_goods:
            columns_stockin = StatisticFunc.get_query_param(StatisticGoodsStockin)
            query_base_stockin = statistic_session.query(
                StatisticGoodsStockin.goods_id, *columns_stockin).group_by(
                StatisticGoodsStockin.goods_id)
        else:
            query_base_stockin = StatisticFunc.get_static_query_base(
                statistic_session, StatisticGoodsStockin, statistic_type,
                extra_groups=[StatisticGoodsStockin.goods_id,
                              StatisticGoodsStockin.statistic_date,
                              StatisticGoodsStockin.year,
                              StatisticGoodsStockin.month])

        filter_params = [
            models_statistic.StatisticGoodsStockin.shop_id == shop_id,
            models_statistic.StatisticGoodsStockin.statistic_type == filter_type
        ]


        if statistic_type == 1:
            filter_params.append(StatisticGoodsStockin.statistic_date >= start_date)
            filter_params.append(StatisticGoodsStockin.statistic_date <= end_date)
        else:
            if not if_total:
                query_base_stockin = query_base_stockin.filter_by(**filter_date)
        ...
        # 过滤分组ID
        if group_id:
            goods_in_group = session.query(models.Goods).filter_by(group_id=group_id, shop_id=shop_id).all()
            filter_params.append(StatisticGoodsStockin.goods_id.in_([x.id for x in goods_in_group]))
        # 过滤销售类型
        if supply_type in [0, 1, 2, 3]:
            supply_goods_id_list = GoodsFunc.get_goods_through_supply_type(session, shop_id, [supply_type])
            filter_params.append(StatisticGoodsStockin.goods_id.in_(supply_goods_id_list))
        # 过滤货品类型
        if goods_type in [0, 1, 2]:
            type_goods_id_list = GoodsFunc.get_goods_through_goods_type(session, shop_id, [goods_type])
            filter_params.append(StatisticGoodsStockin.goods_id.in_(type_goods_id_list))
        ...
        if month:
            filter_params.append(models_statistic.StatisticGoodsStockin.month == month)
        query_base_stockin = query_base_stockin.filter(*filter_params).order_by(
            StatisticGoodsStockin.id)
        if page == -1:
        ...
        total_data_sum = dict(
            quantity_sum=check_float(sum_data.quantity / 100) if sum_data.quantity else 0,
            weight_sum=check_float(UnitFunc.convert_weight_unit(
                sum_data.weight, source="weight", weight_unit_text=weight_unit_text) / 100) if sum_data.weight else 0,
            total_price_sum=check_float(sum_data.total_price / 100) if sum_data.total_price else 0,
            credit_cent_sum=check_float(sum_data.credit_cent / 100) if sum_data.credit_cent else 0,
            paid_cent_sum=check_float(sum_data.paid_cent / 100) if sum_data.paid_cent else 0,
            payback_cent_sum=check_float(sum_data.payback_cent / 100) if sum_data.payback_cent else 0,
            should_pay_cent_sum=check_float(sum_data.should_pay_cent / 100) if sum_data.should_pay_cent else 0
        )
        for statistic in statistic_all:
            ...
            date_range = TimeFunc.list_date_range(
                statistic_type, date=getattr(statistic, "statistic_date", None),
                year=getattr(statistic, "year", None), week=getattr(statistic, "week", None),
            ...
            data["weight"] = check_float(UnitFunc.convert_weight_unit(
                statistic.weight, source="weight", weight_unit_text=weight_unit_text) / 100)
            data["quantity_unit_price"] = check_float(statistic.total_price / statistic.quantity
                                                      if statistic.quantity else 0)
            data["weight_unit_price"] = check_float(UnitFunc.convert_weight_unit(
                statistic.total_price / statistic.weight if statistic.weight else 0,
                source="price",
            ...
            data["should_pay_cent"] = check_float(statistic.should_pay_cent / 100)
...
class StatisticGoodsSalesFunc():
    ...
    _goods_unpay_kpi_dict = {
        "unpay_commission_mul": 0
    }
    ...
    def init_goods_dicts(cls, goods_id_list, map_goods):
        ...
        """
        result = {}
        base_goods_dict = dict(collections.ChainMap(
            cls._goods_info_dict,
    ...
    def init_salesman_dicts(cls, salesman_id_list):
        ...
        """
        result = {}
        base_salesman_dict = dict(collections.ChainMap(
            cls._sales_kpi_base_dict,
    ...
    def init_goods_salesman_dicts(cls, salesman_id_goods_id_list, map_goods):
        ...
        """
        result = {}
        base_goods_salesman_dict = dict(collections.ChainMap(
            cls._goods_info_dict,
            cls._salesman_kpi_dict,
            cls._sales_kpi_base_dict,
            cls._goods_kpi_dict))
        for salesman_id_goods_id in salesman_id_goods_id_list:
            ...
            goods = map_goods[goods_id]
            goods_salesman_dicts_value = copy.deepcopy(base_goods_salesman_dict)
            cls._update_goods_info(goods_salesman_dicts_value, goods)
            result[salesman_id_goods_id] = goods_salesman_dicts_value
        return result

    @classmethod
    def init_goods_accountant_dicts(cls, accountant_id_goods_id_list, map_goods):
        ...
        """
        result = {}
        base_goods_accountant_dict = dict(collections.ChainMap(
            cls._goods_info_dict,
            cls._sales_kpi_base_dict))
        for accountant_id_goods_id in accountant_id_goods_id_list:
            goods_id = int(accountant_id_goods_id.split('-')[1])
            goods = map_goods[goods_id]
            goods_accountant_dicts_value = copy.deepcopy(base_goods_accountant_dict)
    ...
    @classmethod
    def update_goods_dict(
            cls, goods_dict_single, record, gross_cent, total_goods_fact_price):
    ...
    def update_salesman_dict(cls, salesman_dict_single, record):
        ...
        if record.status in (3, 5) and record.record_type == 1 or record.status == 4:
            salesman_dict_single["refund_count"] += 1
        salesman_dict_single["sales_count"] += 1
        return None

    @classmethod
    def update_goods_salesman_dict(cls, goods_salesman_dict_single, record, gross_cent):
    ...
    @classmethod
    def update_goods_accountant_dict(cls, goods_accountant_dict_single, record):
...
class GoodsFunc():
    ...
    @classmethod
    def get_goods_info(cls, goods, source="", multi=False, weight_unit_text="斤"):
        check_float = NumFunc.check_float
        ...
        goods_info["unit_text"] = cls.getUnit(goods.unit, weight_unit_text=weight_unit_text)
        goods_info["commission"] = check_float(goods.commission / 100)
        goods_info["price"] = check_float(goods.get_price(
            weight_unit_text=weight_unit_text) / 100)
        # 成本价先兼容两个版本
        goods_info["cost_price"] = check_float(goods.cost_price / 100)
        goods_info["cost_price_quantity"] = check_float(goods.cost_price_quantity / 100)
        goods_info["cost_price_weight"] = check_float(goods.get_cost_price_weight(
            weight_unit_text=weight_unit_text) / 100)
        goods_info["storage"] = check_float(goods.storage / 100)
        goods_info["storage_num"] = check_float(goods.get_storage_num(
            weight_unit_text=weight_unit_text) / 100)
    ...
    def get_goods_storage_info(cls, goods, stockin_unit, weight_unit_text="斤"):
        ...
        cost_total = check_float(goods.cost_price_quantity * goods.storage) if stockin_unit else check_float(
    ...
    def get_storage_text(cls, goods, weight_unit_text="斤"):
        ...
        if goods.storage_unit == 0:
            storage_text = "%s%s" % (NumFunc.check_float(goods.get_storage_num(
                weight_unit_text=weight_unit_text) / 100), weight_unit_text)
        else:
    ...
    def get_goods(cls, session, shop_id, goods_id):
        goods = session.query(models.Goods).filter_by(shop_id=shop_id, id=goods_id).first()
        return goods
    ...
    def get_active_goods_list_in_shop(cls, session, shop_id):
        goods = session.query(models.Goods).filter_by(shop_id=shop_id, active=1).all()
        return goods
    ...
    def get_supplier_goods(cls, session, shop_id, goods_id, supplier_id):
        goods = session.query(models.Goods).filter_by(
            shop_id=shop_id, id=goods_id, shop_supplier_id=supplier_id).first()
        return goods
    ...
    def get_goods_name_through_id_list(cls, session, goods_id_list):
        ...
        if goods_id_list:
            ...
            goods = session.query(Goods) \
                ...
                .all()
            goods_name_dict = {x.id: cls.get_goods_name_with_attr(x) for x in goods}
    ...
    def get_goods_name_unit_through_id_list(cls, session, goods_id_list):
        ...
        if goods_id_list:
            ...
            goods_name_dict = {
                g.id: {
                    "name": cls.get_goods_name_with_attr(g),
                    "supply_type": g.supply_type,
                    "supply_type_text": cls.get_supply_type(g.supply_type),
                    "unit_text": cls.getUnit(g.unit, weight_unit_text=weight_unit_text),
                    "unit": g.unit} for g in goods
            }
        return goods_name_dict

    @classmethod
    def get_group_id_dict_with_goods_ids(cls, session, goods_id_list):
        goods_id_group_id = session.query(models.Goods.id, models.Goods.group_id).filter(
        ...
        if goods_id_list:
            ...
            for group_id_name_count in group_id_name_count_list:
                group_list.append({
                    ...
                    "group_count": group_id_name_count.group_count,
                })
    ...
    @classmethod
    def get_goods_supplytype_goodstype_through_id_list(cls, session, goods_id_list):
        ...
        if goods_id_list:
            ...
            goods = session.query(Goods) \
                .filter(Goods.id.in_(goods_id_list)) \
                .all()
            supplytype_goodstype_dict = {x.id: {"supply_type": x.supply_type_text, "goods_type": x.goods_type_text} for
                                         x in goods}
    ...
    @classmethod
    def get_goods_info_list_through_id_list(cls, session, shop_id, goods_id_list):
        goods_info_dict = {}
        if goods_id_list:
            Goods = models.Goods
            ...
            supplier_dict = SupplierFunc.get_shop_supplier_base_info(session, shop_id, shop_supplier_id_list)
            for goods in goods_list:
                ...
                goods_info_dict[goods_id] = {"goods_id": goods_id,
                                             "name": goods_name,
    ...
    def get_goods_info_with_supplier(cls, session, shop_id, goods, source=""):
        ...
        if shop_supplier_id:
            shop_supplier = SupplierFunc.get_shop_supplier_through_id(session, shop_id, shop_supplier_id)
    ...
    def get_id_goup_by_supply_type(cls, session, shop_id):
        ...
        goods_id_list = session.query(Goods.id, Goods.supply_type) \
            .filter_by(shop_id=shop_id) \
            .all()
        self_goods_id_list = [x.id for x in goods_id_list if x.supply_type == 0]
        agent_goods_id_list = [x.id for x in goods_id_list if x.supply_type == 1]
        union_goods_id_list = [x.id for x in goods_id_list if x.supply_type == 3]
    ...
    def get_goods_detail_names(cls, session, goods_id_list, map_suppliers):
        ...
        return {g.id: cls.get_goods_detail_name(g, map_suppliers) for g in goods}

    @classmethod
    def get_goods_detail_names_dict_suppliers_type_dict(cls, session, goods_id_list, map_suppliers):
    ...
    def get_goods_and_supplier_names(cls, session, goods_id_list):
        ...
        if goods_id_list:
            ...
            supplier_name_dict = SupplierFunc.get_shop_supplier_names(session, supplier_id_list)
            # 组装货品名与供货商名
            for goods in goods_list:
                goods_id = goods.id
    ...
    def get_latest_pay_type(cls, session, goods):
        """ 获取最近入库的支付类型 """
        detail = session.query(models.StockInDocsDetail).filter_by(
            goods_id=goods.id).order_by(models.StockInDocsDetail.id.desc()).first()
    ...
    def get_date_acitve_goods_ids(cls, session, shop_id, choose_time):
        ...
        goods_historys = session.query(models.GoodsHistory).filter(
            models.GoodsHistory.id.in_(history_ids)).all() if history_ids else []
        result = []
    ...
    @classmethod
    def get_goods_in_shop(
            cls, session, shop_id, weight_unit_text, goods_id_list=None,
            merge_self_goods=True, contains_off=True):
        ...
        if contains_off:
            ...
            query_base = query_base.filter(models.Goods.active.in_([1, ]))
        if goods_id_list:
        ...
        map_goods_pay_type = {detail.goods_id: detail.pay_type for detail in details}
        map_stockin_unit = {detail.goods_id: detail.unit for detail in details}

        # 获取商品最新报损的单位
        map_breakage_unit = StockInFunc.get_doc_detail_unit_pub(
            session, models.BreakageDocsDetail, models.BreakageDocs, goods_id_list)
        result = {}
        for g in goods:
            ...
            g_dict = dict(
                ...
                cost_price_weight=check_float(g.get_cost_price_weight(
                    weight_unit_text=weight_unit_text) / 100),
                name=g.name,
    ...
    @classmethod
    def get_goods_has_supplier_id_list(cls, session, shop_id, shop_supplier_id=0):
        Goods = models.Goods
        goods_id_list = session.query(Goods.id)\
                                .filter_by(shop_id=shop_id)
        if shop_supplier_id:
            goods_id_list = goods_id_list.filter_by(shop_supplier_id=shop_supplier_id)
...
class GoodsBatchFunc():
    ...
    def get_current_batch(cls, session, goods_id):
        ...
        if not batch:
            batch = session.query(models.GoodsBatch).filter_by(
                goods_id=goods_id, consume_status=0, batch_status=1).order_by(
                models.GoodsBatch.id).first()
            if not batch:
    ...
    @classmethod
    def consume(cls, session, goods, sale_record, record_type, operator_id, storage_record_type,
                is_refund=False, is_ajust=False, batch_id=None):
        ...
        for consume in consume_list:
            ...
            rate = abs(
                consume["quantity"] / sale_record.commission_mul
                if consume["quantity"] else consume["weight"] / sale_record.sales_num / 100
            )
            num = "S" + char_list.pop() + sale_record.get_num()
            new_sale_record = OrderFinishFunc.gen_sales_record_by_origin(
                session, sale_record, 0, branch_goods, batch.id, order=None, num=num, rate=rate)
            OrderFinishFunc.gen_storage_record(
                session, branch_goods, origin_storage, origin_storage_num, new_sale_record,
                operator_id, storage_record_type)
    ...
    @classmethod
    def _construct_consume_list(cls, default_batch, batch_queue, quantity, weight, unit):
        ...
        while batch_queue:
            ...
            consume_list.insert(0, dict(
                quantity=update_quantity,
                weight=update_weight,
                batch=current_batch))
            quantity -= update_quantity
            weight -= update_weight
            cls._update_batch_consume(current_batch, update_quantity, update_weight)
    ...
    def _cal_consume(cls, batch, quantity, weight, unit):
        ...
        if unit == 1:
            ...
            consume_quantity = cls._cal_update_digit(batch, quantity, batch_left)
...
class GoodsGroupFunc():
    ...
    def get_groups_under_shop(cls, session, shop_id):
        ...
        groups = session.query(
            models.GoodsGroup.id, models.GoodsGroup.name).filter_by(
            shop_id=shop_id).filter(models.GoodsGroup.status.in_([-1, 0])).all()
...
class GoodsMasterBranchFunc():
    ...
    def list_goods_id_master(cls, session, shop_id):
        ...
        # 非自营货品
        query_unself = query_base.filter(models.Goods.supply_type.in_([1, 2, 3]))
        # 自营货品
        ...
        goods = query_unself.union(
            query_self_master).union(query_self_unmaster).all()
        result = {g.id for g in goods}
        return result
    ...
    @staticmethod
    def get_goods_base_query(session, shop_id, merge_branch_goods=True, only_id=False):
        """ 货品基本查询，自营货品只取主货品 """
        if only_id:
        ...
        if merge_branch_goods:
            self_master = query_base.filter(models.Goods.supply_type == 0, models.Goods.is_master == 1)
            self_sole_master = query_base.filter(models.Goods.supply_type == 0,
                                                 models.Goods.master_goods_id == 0,
    ...
    def apply_filter(base_query, shop_id):
        ...
        self_master = base_query.filter(models.Goods.supply_type == 0, models.Goods.is_master == 1)
    ...
    def match_branch_goods(cls, session, shop_id, master_goods_id_list):
        ...
        for branch_goods in branch_goods_list:
            branch_goods_dict[branch_goods.master_goods_id].append(branch_goods)
        return result


class PubRecordFunc:
    @classmethod
    def get_record_num_from_id_list(cls, session, RecordClass, id_list, shop_id=None, goods_id=None):
        """ 根据id列表获取对应的num
        可用于GoodsSalesRecord,
             GoodsStockinRecord,
             GoodsBreakageRecord,
    ...
    @classmethod
    def get_docs_id_from_num_list(cls, session, DocClass, num_list, shop_id=None):
        """ 根据num列表获取对应的单据id
...
class GoodsSaleFunc():
    ...
    def gen_sale_record_num(cls, session, shop_id, base_time=None):
        ...
        if redis.get(key):
            ...
            sale_record = session.query(models.GoodsSalesRecord) \
                ...
                .first()
            sale_record_num = str(int(sale_record.get_num()) + 1) if sale_record else str(shop_id) + base_time.strftime(
                "%y%m%d") + '00001'
    ...
    # 获取流水详情
    @classmethod
    def get_sale_record_info(
            cls,session, sale_record, signature_img="", weight_unit_text="斤",
            print_erasement=False, order_record_num=None, repeat_print=False):
        """ 获取流水详情
        ...
        """
        check_float = NumFunc.check_float
        goods = session.query(models.Goods).filter_by(id=sale_record.goods_id).first()
        supplier = session.query(models.ShopSupplier).filter_by(id=goods.shop_supplier_id).first()
        ...
        sale_record_info["full_bill_time"] = str(sale_record.bill_year) + '-' + str(sale_record.bill_month).zfill(
            2) + '-' + str(sale_record.bill_day).zfill(2) + ' ' + str(sale_record.bill_time)
        sale_record_info["num"] = sale_record.num
        sale_record_info["order_record_num"] = order_record_num if order_record_num else sale_record.num
        sale_record_info["fact_price"] = check_float(
            sale_record.get_fact_price(goods.unit, weight_unit_text=weight_unit_text) / 100)
        sale_record_info["sales_num"] = check_float(
            sale_record.get_sales_num(goods.unit, weight_unit_text=weight_unit_text))
        sale_record_info["goods_sumup"] = check_float(sale_record.fact_price * sale_record.sales_num / 100)
        sale_record_info["commission"] = check_float(sale_record.commission / 100)
        sale_record_info["commission_mul"] = check_float(sale_record.commission_mul / 100)
        sale_record_info["commission_sumup"] = check_float(
            sale_record.commission * sale_record.commission_mul / 10000)
        sale_record_info["receipt_money"] = check_float(sale_record.receipt_money / 100)
        sale_record_info["tare_weight"] = check_float(sale_record.get_tare_weight(
            weight_unit_text=weight_unit_text))
        sale_record_info["gross_weight"] = check_float(sale_record.get_gross_weight(
            weight_unit_text=weight_unit_text))
        sale_record_info["record_type"] = sale_record.record_type
        sale_record_info["order_id"] = sale_record.order_id

        shop_name = session.query(models.Shop.shop_name).filter_by(id=sale_record.shop_id).scalar()
        sale_record_info["shop_name"] = shop_name

        # 货品信息
        sale_record_info["goods_name"] = GoodsFunc.get_goods_name_with_attr(goods)
        ...
        sale_record_info["supply_type_text"] = GoodsFunc.get_supply_type(goods.supply_type)
        ...
        sale_record_info["salesman_name"] = AccountFunc.get_account_name(session, sale_record.salesman_id)
        sale_record_info["deposit"] = check_float(sale_record.deposit_avg / 100)
        sale_record_info["deposit_total"] = check_float(sale_record.deposit_money / 100)
        ...
        courier_record = courierrecordaccesser.get_courier_record_through_id(sale_record.courier_record_id)
        courier_id = 0
        courier_name = ""
        courier_headimgurl = ""
        courier_remark = ""
        ...
        if courier_record:
            ...
            courier_image_remark = courier_record.image_remark.split(",") if courier_record.image_remark else []
            courier_status_text = courier_record.get_status_text
            courier_finish_time = courier_record.finish_time.strftime("%Y-%m-%d %H:%M:%S")
        ...
        if sale_record.multi_order_id:
            ...
            if multi_order and multi_order.remark:
                ...
                sale_record_info["image_remark"] = multi_order.full_image_remark
        sale_record_info["pay_status"] = cls._get_sales_record_pay_status(sale_record)
        sale_record_info["pay_status_text"] = cls.get_sales_record_pay_status_text(sale_record)
        ...
        sale_record_info["avg_price"] = check_float(
            sale_record.sales_money / sale_record.commission_mul) if goods.unit == 0 and sale_record.commission_mul else 0
        ...
        if sale_record.accountant_id:
            sale_record_info["accountant_name"] = AccountFunc.get_account_name(session, sale_record.accountant_id)
            sale_record_info["accountant_time"] = TimeFunc.time_to_str(sale_record.order_time)
        ...
        if sale_record.shop_customer_id:
            debtor = session.query(models.ShopCustomer).filter_by(id=sale_record.shop_customer_id).first()
            sale_record_info["shop_customer_id"] = debtor.id
            sale_record_info["shop_customer"] = '%s %s ' % (debtor.name, debtor.company)
        else:
        ...
        if sale_record.order_id:
            ...
            combine_pay_dict = OrderBaseFunc.multi_combine_pay_type_text(session, [order_id])
            pay_type_text = OrderBaseFunc.order_pay_type_text(order_id, pay_type, combine_pay_dict)

            if pay_type == 12:
                bank_name = OrderBaseFunc.get_order_bank_name(session, order_id, source="order")
                if bank_name:
                    pay_type_text = "{0}({1})".format(pay_type_text, bank_name)

            sale_record_info["pay_type_text"] = pay_type_text
            sale_record_info["combine_pay_type_text"] = OrderBaseFunc.combine_pay_type_text(
                session, sale_record.order_id) if pay_type == 10 else ""
            sale_record_info["signature_img"] = signature_img if pay_type == 9 else ""
        else:
        ...
        if sale_record.order_id:
            ...
            if order and order.erase_money > 0:
                ...
                fact_total_price = check_float((-fact_total_price if record_type == 1 else fact_total_price) / 100)
                erase_money = check_float((-erase_money if record_type == 1 else erase_money) / 100)
                if print_erasement:
                    sale_record_info["erase_money"] = erase_money
                else:
                    sales_sum_count = session.query(func.count(models.GoodsSalesRecord.id)).filter_by(
            ...
            # 原始支付方式（仅修改过支付方式的有该值）
            sale_record_info["origin_pay_type_text"] = PaytypeModifyFunc.get_origin_paytype_text(
                session, sale_record.order_id)

            # 重复打印取签名
            if repeat_print and sale_record.pay_type == 9 and order.customer_signature:
                sale_record_info["signature_img"] = CustomerSignature.get_signature_image(sale_record.order_id)
    ...
    def get_multi_order_info(cls, session, records, order, multi_id=None):
        ...
        :rtype: tuple
        """
        weight_unit_text = ConfigFunc.get_weight_unit_text(session, order.shop_id)
        # 批量获取货品串货调整信息
        origin_record_id_list = {x.id for x in records}
        ajust_record_id_list = SalesRecordAjustFunc.get_multi_ajust_info(session, origin_record_id_list)
        ...
        order_total_dict = {}
        for key in ["accountant_name", "accountant_time", "full_bill_time", "pay_status",
                    "pay_status_text", "printer_num", "printer_remark", "salesman_name",
                    "shop_customer", "shop_customer_id", "salesman_id", "accountant_id",
                    "pay_type_text", "origin_pay_type_text", "courier_id", "courier_name",
                    "courier_headimgurl", "courier_remark", "courier_image_remark",
                    "courier_status_text", "courier_finish_time", "courier_msg_notify"]:
        ...
        order_total_dict["receipt_money"] = check_float(order.total_price / 100)
        ...
        for detail in detail_record_list:
            goods_list.append(
                {"commission_mul": detail["commission_mul"],
                 "name": detail["goods_name"],
                 "record_id": detail["id"],
                 "sales_num": detail["sales_num"] if detail["goods_unit"] == 0 else "-",
                 "total_money": detail["receipt_money"],
                 "unit_price": detail["fact_price"],
                 "goods_unit_text": detail["goods_unit_text"],
                 "goods_id": detail["goods_id"],
                 "num": detail["num"],
                 "ajust": detail["ajust"]}
            )
            order_total_dict["total_commission_mul"] += detail["commission_mul"]
    ...
    @classmethod
    def get_temp_sale_order_info(
            cls, session, sale_record_list, order,
            source="", pay_order=None, is_merge=False, repeat_print=False):
        """ 从order上获取相关信息
        ...
        customer_company = ""
        if customer_id:
            customer = ShopCustomerFunc.get_shop_customer_through_id(session, shop_id, customer_id)
        ...
        if tally_order_id:
            ...
            if tally_order and tally_order.erase_money > 0:
                ...
                erase_text = "%d个货品应收%s元，合计抹零%s元" % (sales_sum_count, fact_total_price, erase_money)

        if source == "salesman":
            temp_order_info = {
                "num": order.num,
                "time": TimeFunc.time_to_str(order.create_time),
                "salesman_name": salesman_name,
                "shop_name": shop_name,
                "total_recipt_money": NumFunc.check_float(order.total_price / 100),
                "total_goods_type_count": total_goods_type_count,
                "shop_id": shop_id,
            ...
            shop_name = session.query(models.Shop.shop_name).filter_by(id=shop_id).scalar()
            ...
            combine_pay_type_text = OrderBaseFunc.combine_pay_type_text(session, pay_order.id) if pay_type == 10 else ""

            if pay_order.pay_type == 12 and pay_order.bank_no:
                bank_name = OrderBaseFunc.get_order_bank_name(session, pay_order.id, source="order")
                pay_type_text = "{0}({1})".format(pay_type_text, bank_name)

            accountant_name = AccountFunc.get_account_name(session, pay_order.accountant_id)
            accountant_time = str(pay_order.pay_year) + '-' + str(pay_order.pay_month).zfill(2) + '-' + str(
            ...
            temp_order_info = {
                ...
                "shop_name": shop_name,
                "total_recipt_money": NumFunc.check_float(order.fact_total_price / 100),
                "total_goods_type_count": total_goods_type_count,
                "shop_id": shop_id,
                "pay_type_text": pay_type_text,
                "combine_pay_type_text": combine_pay_type_text,
                "salesman_name": salesman_name,
                "accountant_name": accountant_name,
                "customer_name": customer_name,
                "customer_company": customer_company,
                "remark": order.remark or merge_remarks,
                "total_commission_mul": total_commission_mul,
                "record_type": order.record_type,
                "erase_text": erase_text
            }
            ...
            temp_order_info["signature_img"] = (redis.get(redis_key) or b"").decode()
            # 重复打印取签名
            if repeat_print and pay_type == 9 and tally_order_id:
                if tally_order.customer_signature:
                    temp_order_info["signature_img"] = CustomerSignature.get_signature_image(tally_order_id)
    ...
    @classmethod
    def query_sales_record_by_billing_record(
            cls, start_date, end_date, session, page, shop_id, salesman_id_list,
            accountant_id_list, pay_type_list, show_type, only_refund,
            only_paid=True, only_unpaid=False, all_page=False, export=False,
            goods_ids=None, customer_id_list=None):
        ...
        if only_paid and only_unpaid:
            ...
            query_base = query_base.filter(BillingSalesRecord.accountant_id > 0)
        elif only_unpaid:
            query_base = query_base.filter(BillingSalesRecord.accountant_id == 0)
        # 过滤收银员
        if accountant_id_list:
            query_base = query_base.filter(BillingSalesRecord.accountant_id.in_(accountant_id_list))
        # 过滤开票员
        if salesman_id_list:
            query_base = query_base.filter(BillingSalesRecord.salesman_id.in_(salesman_id_list))
        # 过滤结算方式
        if pay_type_list:
            query_base = query_base.filter(BillingSalesRecord.pay_type.in_(pay_type_list))
        ...
        if only_refund or only_unpaid:
            ...
            if only_unpaid:
                query_base1 = query_base1.filter(
                    GoodsSalesRecord.status == 1)

            query_base2 = query_base.join(
                GoodsSalesRecord,
                GoodsSalesRecord.multi_order_id == BillingSalesRecord.record_id).filter(
                BillingSalesRecord.type_id == 1)
            if only_refund:
        ...
        # 获取记录中所有的客户ID/货品ID
        all_single_record_id_list = [x.record_id for x in query_base.all() if x.type_id == 0]
        ...
        all_multi_record_id_list = [x.record_id for x in query_base.all() if x.type_id == 1]
        ...
        if record:
            single_record_id_list = [x.record_id for x in record if x.type_id == 0]
            multi_record_id_list = [x.record_id for x in record if x.type_id == 1]
            ...
            if multi_record_id_list:
                ...
                if goods_ids:
                    ...
                    for _sale_record in record2.all():
                        ...
                        multi_goods_ids_dcit[_multi_order_id].append(_sale_record.goods_id)
                    for _multi_order_id in multi_goods_ids_dcit:
                        temp_goods_ids = multi_goods_ids_dcit[_multi_order_id]
                        # 判断goods_id交集
                        if list((set(goods_ids).union(set(temp_goods_ids))) ^ (set(goods_ids) ^ set(temp_goods_ids))):
            ...
            if goods_ids:
                if record1:
                    record1 = record1.filter(GoodsSalesRecord.goods_id.in_(goods_ids))
                if record2:
            ...
            if record1 and record2:
                record = record1.union(record2).order_by(GoodsSalesRecord.id.desc()).all()
            elif record1:
    ...
    @classmethod
    def query_sales_record_by_billing_record_new(
            cls, start_date, end_date, session, page, shop_id, salesman_id_list,
            accountant_id_list, pay_type_list, show_type, only_refund,
            only_paid=True, only_unpaid=False, all_page=False, export=False,
            goods_ids=None, customer_id_list=None):
        ...
        if show_type == 1:
            # 开票员今日数据需要展示轧帐以后的，由前端控制
            base_filter_list += [BillingSalesRecord.account_period_date >= start_date,
                                 BillingSalesRecord.account_period_date <= end_date]
        ...
        # 自然日
            ...
            base_filter_list += [BillingSalesRecord.bill_date >= start_date, BillingSalesRecord.bill_date <= end_date]
        ...
        if only_paid and only_unpaid:
            ...
            filter_param_three = and_(GoodsSalesRecord.status.in_([3, 4, 5]), GoodsSalesRecord.accountant_id > 0)
            if only_refund:
                filter_param_three = GoodsSalesRecord.status.in_([1, 2, 3, 4, 5])
        elif only_unpaid:
            base_filter_list.append(BillingSalesRecord.accountant_id == 0)
            filter_param_three = and_(GoodsSalesRecord.status.in_([1, 2]), GoodsSalesRecord.accountant_id <= 0)

        # 联合models.GoodsSalesRecord表，目的是筛选货品和客户, 冗余筛选条件减少GoodsSalesRecord查询量
        first_join = [
            GoodsSalesRecord,
            and_(or_(and_(BillingSalesRecord.record_id == GoodsSalesRecord.id, BillingSalesRecord.type_id == 0),
                and_(BillingSalesRecord.record_id == GoodsSalesRecord.multi_order_id, BillingSalesRecord.type_id == 1)),
                 GoodsSalesRecord.shop_id == shop_id, filter_param_one, filter_param_two, filter_param_three)
        ]

        # 过滤收银员
        if accountant_id_list:
            diy_filter_list.append(BillingSalesRecord.accountant_id.in_(accountant_id_list))
            goods_diy_filter_list.append(GoodsSalesRecord.accountant_id.in_(accountant_id_list))
        # 过滤开票员
        if salesman_id_list:
            diy_filter_list.append(BillingSalesRecord.salesman_id.in_(salesman_id_list))
            goods_diy_filter_list.append(GoodsSalesRecord.salesman_id.in_(salesman_id_list))
        # 过滤结算方式
        if pay_type_list:
            diy_filter_list.append(BillingSalesRecord.pay_type.in_(pay_type_list))
            goods_diy_filter_list.append(GoodsSalesRecord.pay_type.in_(pay_type_list))

        # 基础查询，沿用以前版本的逻辑, 这一段代码的目的是获取以billing_record的id为主键的字典，
        # 值分别为goods_sale_record和billing_record
        if not (only_unpaid or only_refund or goods_ids or customer_id_list or export):
            query_base = session.query(models.BillingSalesRecord) \
                .filter(*base_filter_list)
            temp_result = query_base.all()
            # 获取记录中所有的客户ID/货品ID
            all_single_record_id_list = [x.record_id for x in temp_result if x.type_id == 0]
            single_goods_customer = []
            if all_single_record_id_list:
                single_goods_customer = session \
                    .query(GoodsSalesRecord.goods_id,
            ...
            all_multi_record_id_list = [x.record_id for x in temp_result if x.type_id == 1]
            multi_goods_customer = []
            if all_multi_record_id_list:
                multi_goods_customer = session \
                    .query(
                    ...
                    .filter(GoodsSalesRecord.multi_order_id.in_(all_multi_record_id_list)) \
                    .all()
            customer_goods_id_set = set(single_goods_customer + multi_goods_customer)
            ...
            if diy_filter_list:
                query_base = query_base.filter(*diy_filter_list).order_by(BillingSalesRecord.id.desc())
            else:
                query_base = query_base.order_by(BillingSalesRecord.id.desc())
            page_sum = math.ceil(query_base.count() / page_size)
            query_base = query_base.offset(page * page_size).limit(page_size)
            ...
            if billing_records:
                ...
                if multi_record_id_list:
                    record2 = session.query(GoodsSalesRecord).filter(
                        ...
                        GoodsSalesRecord.shop_id == shop_id)
                if record1 and record2:
                    goods_sale_records = record1.union(record2).order_by(GoodsSalesRecord.id.desc()).all()
                elif record1:
                    goods_sale_records = record1.order_by(GoodsSalesRecord.id.desc()).all()
                elif record2:
                    goods_sale_records = record2.order_by(GoodsSalesRecord.id.desc()).all()
        ...
        # 要支持货品id或者操作人筛选，那么需要join
            ...
            base_query = session.query(BillingSalesRecord, GoodsSalesRecord).join(*first_join).filter(*base_filter_list)
            ...
            if not export:
                ...
                for item in base_result:
                    ...
                    # 一个票据中可能有多个货品，多品
                    goods_list = goods_record_dict.get(billing_record_id, list())
            ...
            # 过滤客户
            if customer_id_list:
                diy_filter_list.append(GoodsSalesRecord.shop_customer_id.in_(customer_id_list))
            # 过滤货品
            if goods_ids:
                diy_filter_list.append(GoodsSalesRecord.goods_id.in_(goods_ids))
        ...
        customer_name_dict = ShopCustomerFunc.get_shop_customer_name_through_id_list(session, customer_id_set)
        ...
        if goods_id_set:
            goods_name_unit_dict = GoodsFunc.get_goods_name_unit_through_id_list(session, goods_id_set)
        ...
        if return_billing_record_id:
            ...
            for _id in return_billing_record_id:
                ...
                billing_record = billing_record_dict.get(_id)  # type: models.BillingSalesRecord
                ...
                # 单品
                if is_multi == 0:
                    first_goods_record = goods_records[0]  # type: models.GoodsSalesRecord
                    goods_unit = goods_unit_dict.get(first_goods_record.goods_id)
                    data_dict = {
                ...
                # 多品
                    ...
                    for item in goods_records:
                        ...
                        total_commission_mul += check_float(temp_dict["commission_mul"])
                        total_commission_sumup += check_float(temp_dict["commission_sumup"])
                        deposit_total += check_float(temp_dict["deposit_total"])
                        total_goods_sumup += check_float(temp_dict["goods_sumup"])
                        total_receipt_money += check_float(temp_dict["receipt_money"])
    ...
    @classmethod
    def _parse_single_records(
            cls, single_record_list, goods_info_dict, session, simplify=False):
        """
        解析单品记录
        ...
        for r in single_record_list:
            ...
            if r.accountant_id:
                ...
                data["accountant_wximgurl"] = accountant.head_imgurl_small if accountant.wx_unionid else ""
                data["accountant_time"] = TimeFunc.time_to_str(r.order_time, "time")
            ...
            data["salesman_wximgurl"] = salesman.head_imgurl_small if salesman.wx_unionid else ""
    ...
    @classmethod
    def _parse_multi_records(
            cls, multi_record_list, goods_info_dict, map_multi_orders, session,
            shop_id=0):
        """
        解析多品订单记录
        ...
        for record in multi_record_list:
            map_multi_records.setdefault(record.multi_order_id, []).append(record)
        ...
        for multi_records in map_multi_records.values():
            ...
            for r in multi_records:
                goods_info = goods_info_dict.get(
                    r.goods_id, {"name": "unknown", "unit": "known"})
                multi_data = cls._merge_multi_data(
                    r, multi_data, map_multi_orders, goods_info, session,
                    shop_id=shop_id, weight_unit_text=weight_unit_text)
            result.append(multi_data)
        return result

    @classmethod
    def _merge_multi_data(
            cls, record, multi_data, map_multi_orders, goods_info, session,
            shop_id=0, weight_unit_text="斤"):
        ...
        multi_data = dict(
            ...
            goods_name=goods_info["name"] + " " + multi_data.get("goods_name", ""),
            commission_mul=NumFunc.check_float(record.get_commission_mul() / 100 +
                                               multi_data.get("commission_mul", 0)),
            # 多品暂不展示重量
            sales_num=NumFunc.check_float(sales_num + multi_data.get("sales_num", 0)),
            receipt_money=NumFunc.check_float(record.get_receipt_money() / 100 +
            ...
            commission_sumup=NumFunc.check_float(record.get_commission_money() / 100 +
                                                 multi_data.get("commission_sumup", 0)),
            deposit_total=NumFunc.check_float(record.get_deposit_money() / 100 +
                                              multi_data.get("deposit_total", 0)),
            remark=multi_order.remark,
            bill_time_full=(TimeFunc.time_to_str(record.bill_date, "date") + " "
        ...
        if not multi_data.get("salesman_name"):
            salesman = AccountFunc.get_account_through_id(session, record.salesman_id)
            multi_data["salesman_name"] \
                = salesman.realname or salesman.nickname if salesman else ""
            multi_data["salesman_headimg"] = salesman.head_imgurl if salesman else ""
            multi_data["salesman_wximgurl"] = salesman.head_imgurl_small if salesman.wx_unionid else ""
            if record.accountant_id:
                order_id = multi_order.tally_order_id
                combine_pay_dict = OrderBaseFunc.multi_combine_pay_type_text(session, [order_id])
                pay_type = record.pay_type
                pay_type_text = OrderBaseFunc.order_pay_type_text(order_id, pay_type, combine_pay_dict)
                accountant_object = AccountFunc.get_account_through_id(session, record.accountant_id)
                multi_data["accountant_name"] = accountant_object.realname or accountant_object.nickname if accountant_object else ""
                multi_data["accountant_wximgurl"] = accountant_object.head_imgurl_small if accountant_object.wx_unionid else ""
                multi_data["accountant_time"] = TimeFunc.time_to_str(record.order_time, "time")
    ...
    @classmethod
    def list_sales_each_goods_salesman(
            cls, session, statistic_session, shop_id, salesman_id,
            from_date, to_date, page, data_type=0):
        """ 开票员货品统计, 某段时间内 """
        weight_unit_text = ConfigFunc.get_weight_unit_text(session, shop_id)
        page_size = 20
        if data_type == 1:
        ...
        query_base = statistic_session.query(
            ...
            func.sum(sTable.sales_pay_count + 2 * sTable.refund_count).label("count"),
            *columns).filter_by(
            shop_id=shop_id, statistic_type=1, salesman_id=salesman_id).filter(
            sTable.goods_id.in_(master_goods_id_set)).having(and_(
            func.sum(sTable.total_goods_cent) != 0,
            func.sum(sTable.commission_mul) != 0,
            func.sum(sTable.sales_pay_count + 2 * sTable.refund_count) != 0))
        if from_date:
        ...
        supplier_dict = SupplierFunc.get_supplier_info(
        ...
        for q in query_result:
            ...
            data = {
                "sales_count": check_int(q.sales_count),
                # 历史遗留问题, sales_pay_count的名字被占用了
                "sales_pay_count": check_int(q.count),
                "refund_count": check_int(q.refund_count),
                "commission_mul": check_float(q.commission_mul / 100),
                "sales_weigh": check_float(UnitFunc.convert_weight_unit(
                    q.sales_weigh,
                    weight_unit_text=weight_unit_text,
                    source="weight") / 100) if goods.unit == 0 else "-",
                "sales_money": check_float(q.total_goods_cent / 100),
                "commission_total": check_float(q.total_commission / 100),
                "deposit_total": check_float(q.deposit_cent / 100),
                "gross_cent": check_float(q.gross_cent / 100),
                "goods_id": q.goods_id,
                "supplier_id": q.supperlier_id,
                "supplier_name": supplier_name,
                "goods_name": goods_name,
                "avg_price": check_float(avg_price),
            }
            result.append(data)
    ...
    @classmethod
    def list_merged_records(
            cls, session, shop_id, records, sub_multi=True, sub_single=False):
        """ 将票据流水根据单品和多品详情解析后返回
        ...
        # 单品结果列表
        single_result = cls._parse_single_records(
            single_record_list, goods_info_dict, session)
        # 多品结果列表
        multi_result = cls._parse_multi_records(
            multi_record_list, goods_info_dict, map_multi_orders, session,
            shop_id=shop_id)
        if sub_multi:
            # 对每一个多品订单加入对应的单品货品信息
    ...
    @classmethod
    def nullify_sales_record(
            cls, session, record_ids, operator_id, shop_id, is_multi=False):
        """ 作废一条或多条小票流水记录
        ...
        record_list = []
        multi_id_dict = MultiOrderFunc.get_id_dict_through_sales_record(session, record_ids)
    ...
    def query_and_parse_refunds(cls, session, sales_record_id_list):
        ...
        origin_record_ids = {r.origin_goods_sale_record_id for r in refund_infos}
        map_refund_id_to_origin_id = {
            r.goods_sale_record_id: r.origin_goods_sale_record_id for r in refund_infos
        }
        return refund_record_ids, origin_record_ids, map_refund_id_to_origin_id

    @classmethod
    def get_record_shown_status_print(cls, record, refund_record_ids, origin_record_ids):
    ...
    @classmethod
    def get_allstatus_salerecord_list(cls, session, shop_id, choose_date,
                                      choose_end_date,goods_id=0,
                                      salesman_id=0, status=1, accounting=0,
                                      debt_id=0, is_branch=0):


        GoodsSalesRecord = models.GoodsSalesRecord
        data_list = []
        # 获取所有主货ID
        master_goods_ids = GoodsMasterBranchFunc.list_goods_id_master(session, shop_id)
        ...
        if not debt_id:
            if accounting == 1:
                query_base = query_base.filter(GoodsSalesRecord.account_period_date >= choose_date,
                                                GoodsSalesRecord.account_period_date <= choose_end_date)
            else:
                query_base = query_base.filter(GoodsSalesRecord.bill_date >= choose_date,
        ...
        if debt_id:
            ...
            all_goods_sale_record = session.query(models.GoodsSalesRecordPaybackRecord.goods_sale_record_id)\
                                          ...
                                          .all()
            goods_sale_record_ids = [x.goods_sale_record_id for x in all_goods_sale_record]
            query_base = query_base.filter(GoodsSalesRecord.id.in_(goods_sale_record_ids))
        # 按商品id筛选
        if goods_id:
        ...
        if status:
            if not debt_id:
                if status == 3:
                    ...
                    status = [status]
                query_base = query_base.filter(GoodsSalesRecord.status.in_(status))

            unpay_salerecord_list = query_base.order_by(GoodsSalesRecord.id.desc()).all()
            data_list = OrderSalesRecordFunc.batch_format_order_record_info(session,unpay_salerecord_list,return_type="list")
        return data_list
    ...
    def get_cancle_info(cls, slave_session, record_id, multi=False):
        ...
        cancel_text= ""
        if cancel_info:
            operator_name = AccountFunc.get_account_name(slave_session, cancel_info.operator_id)
...
class MultiOrderFunc():
    ...
    def get_id_dict_through_sales_record(cls, session, record_id_list):
        ...
        multi_id_list = session.query(
            GoodsSalesRecord.id,
            GoodsSalesRecord.multi_order_id).filter(
...
class DebtFunc():
    @classmethod
    def update_debt(cls, session, debt_type, debt_value, debtor_id, shop_id, operator_id, fk_id, remark="",
                    account_period_date=None, create_time=None):
        dict_debt_type = {
            'pay_withcredit': 0,
            'payback': 1,
            'refund': 2,
        ...
        debtor = session.query(models.ShopCustomer).filter_by(id=debtor_id).first()
        shop = session.query(models.Shop).filter_by(id=shop_id).with_lockmode('update').first()
        if debt_type_int == 0:
...
class AccoutantFunc():
    ...
    def get_current_work_record(cls, session, accountant_id, shop_id):
        ...
        if not work_record or (work_record and work_record.offduty_time):
            work_record = models.AccountantWorkRecord(accountant_id=accountant_id, shop_id=shop_id)
            session.add(work_record)
            try:
                hr = session.query(models.HireLink).filter_by(account_id=accountant_id, shop_id=shop_id).first()
                session.commit()
    ...
    @classmethod
    def get_timebefore_newest_work_record(cls, session, accountant_id, shop_id, off_datetime):
        work_record = session\
    ...
    @classmethod
    def update_work_record(cls, session, accountant_id,
                           shop_id, pay_cent, pay_type,
                           fk_id=0, is_reset=False, work_record=None,
        ...
        if pay_type in ["order_cash", "order_pos", "order_alipay", "order_wx", "order_cheque", "order_remittance"]:
        ...
        if pay_type in ["pb_cash", "pb_pos", "pb_alipay", "pb_wx", "pb_cheque", "pb_remittance"]:
            if pay_type == "pb_cash":
        ...
        if pay_type == "pb_combine":
            order_money = OrderBaseFunc.get_order_money(session, fk_id, source_type=2)
            pb_total_pay_cent = 0
            ...
            work_record.pb_total_count += count
        if pay_type in ["refund_cash", "refund_pos", "refund_alipay", "refund_wx", "refund_cheque",
                        "refund_remittance"]:
        ...
        if pay_type == "order_combine":
            ...
            order_money = OrderBaseFunc.get_order_money(session, fk_id, source_type=1)
        ...
        if pay_type in ["deposit_refund_cash", "deposit_refund_pos", "deposit_refund_alipay", "deposit_refund_wx",
        ...
        if pay_type in ["salary_cash", "salary_pos", "salary_alipay", "salary_wx", "salary_cheque",
                        "salary_remittance"]:
        ...
        if pay_type in ["payout_cash", "payout_pos", "payout_alipay", "payout_wx", "payout_cheque",
                        "payout_remittance"]:
            ...
            work_record.payout_cent += pay_cent
            work_record.payout_count += count
        if pay_type in ["supplier_borrow_cash", "supplier_borrow_pos", "supplier_borrow_alipay", "supplier_borrow_wx", "supplier_borrow_cheque", "supplier_borrow_remittance"]:
        ...
        if pay_type in ["other_income_cash", "other_income_pos", "other_income_alipay", "other_income_wx",
                        "other_income_cheque", "other_income_remittance"]:
        ...
        if pay_type in ["waste_selling_income", "waste_selling_income_cash", "waste_selling_income_pos",
                        "waste_selling_income_alipay", "waste_selling_income_wx", "waste_selling_income_cheque",
        ...
        if pay_type in ["order_erase_cash", "order_erase_pos", "order_erase_alipay", "order_erase_wx",
                        "order_erase_combine", "order_erase_cheque", "order_erase_remittance"]:
        ...
        if pay_type in ["order_fee_cash", "order_fee_pos", "order_fee_alipay", "order_fee_wx",
                        "order_fee_cheque", "order_fee_remittance"]:
    ...
    def get_word_record_data(cls, work_record):
        if work_record.offduty_time:
            delta_day = (work_record.offduty_time - work_record.onduty_time).days
            delta_seconds = (work_record.offduty_time - work_record.onduty_time).seconds
        else:
            delta_day = (datetime.datetime.now() - work_record.onduty_time).days
            delta_seconds = (datetime.datetime.now() - work_record.onduty_time).seconds
        ...
        data = {}
        data['onduty_time'] = work_record.onduty_time.strftime("%Y-%m-%d %H:%M:%S")
        data['offduty_time'] = work_record.offduty_time.strftime(
            "%Y-%m-%d %H:%M:%S") if work_record.offduty_time else '--'
        data['work_time'] = work_time
        data['now_time'] = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        # 订单收银
        data['order_total_money'] = check_float(work_record.order_total_cent / 100)
        data['order_total_count'] = work_record.order_total_count
        data['order_cash_money'] = check_float(
            work_record.order_cash_cent / 100 + work_record.order_combine_cash_cent / 100)
        data['order_cash_count'] = work_record.order_cash_count + work_record.order_combine_cash_count
        data['order_pos_money'] = check_float(
            work_record.order_pos_cent / 100 + work_record.order_combine_pos_cent / 100)
        data['order_pos_count'] = work_record.order_pos_count + work_record.order_combine_pos_count
        data['order_ali_money'] = check_float(
            work_record.order_ali_cent / 100 + work_record.order_combine_ali_cent / 100)
        data['order_ali_count'] = work_record.order_ali_count + work_record.order_combine_ali_count
        data['order_wx_money'] = check_float(work_record.order_wx_cent / 100 + work_record.order_combine_wx_cent / 100)
        data['order_wx_count'] = work_record.order_wx_count + work_record.order_combine_wx_count
        data['order_cheque_money'] = check_float(
            work_record.order_cheque_cent / 100 + work_record.order_combine_cheque_cent / 100)
        data['order_cheque_count'] = work_record.order_cheque_count + work_record.order_combine_cheque_count
        data['order_remittance_money'] = check_float(
            work_record.order_remittance_cent / 100 + work_record.order_combine_remittance_cent / 100)
        data['order_remittance_count'] = work_record.order_remittance_count + work_record.order_combine_remittance_count
        data['order_bank_money'] = check_float(data['order_pos_money'] + data['order_remittance_money'])
        data['order_bank_count'] = data['order_pos_count'] + data['order_remittance_count']

        # 赊账
        data['order_credit_money'] = check_float(work_record.order_credit_cent / 100)
        data['order_credit_count'] = work_record.order_credit_count

        # 赊账退还
        data['refund_credit_money'] = check_float(work_record.refund_credit_cent / 100)
        ...
        data['pb_cash_money'] = check_float(work_record.pb_cash_cent / 100 + work_record.pb_combine_cash_cent / 100)
        data['pb_cash_count'] = work_record.pb_cash_count + work_record.pb_combine_cash_count
        data['pb_pos_money'] = check_float(work_record.pb_pos_cent / 100 + work_record.pb_combine_pos_cent / 100)
        data['pb_pos_count'] = work_record.pb_pos_count + work_record.pb_combine_pos_count
        data['pb_ali_money'] = check_float(work_record.pb_ali_cent / 100 + work_record.pb_combine_ali_cent / 100)
        data['pb_ali_count'] = work_record.pb_ali_count + work_record.pb_combine_ali_count
        data['pb_wx_money'] = check_float(work_record.pb_wx_cent / 100 + work_record.pb_combine_wx_cent / 100)
        data['pb_wx_count'] = work_record.pb_wx_count + work_record.pb_combine_wx_count
        data['pb_cheque_money'] = check_float(
            work_record.pb_cheque_cent / 100 + work_record.pb_combine_cheque_cent / 100)
        data['pb_cheque_count'] = work_record.pb_cheque_count + work_record.pb_combine_cheque_count
        data['pb_remittance_money'] = check_float(
            work_record.pb_remittance_cent / 100 + work_record.pb_combine_remittance_cent / 100)
        data['pb_remittance_count'] = work_record.pb_remittance_count + work_record.pb_combine_remittance_count
        data['pb_bank_money'] = check_float(data['pb_pos_money'] + data['pb_remittance_money'])
        data['pb_bank_count'] = data['pb_pos_count'] + data['pb_remittance_count']

        # 退单记录
        data['refund_total_money'] = check_float(work_record.refund_total_cent / 100)
        data['refund_total_count'] = work_record.refund_total_count
        data['refund_cash_money'] = check_float(work_record.refund_cash_cent / 100)
        data['refund_cash_count'] = work_record.refund_cash_count
        data['refund_pos_money'] = check_float(work_record.refund_pos_cent / 100)
        data['refund_pos_count'] = work_record.refund_pos_count
        data['refund_ali_money'] = check_float(work_record.refund_ali_cent / 100)
        ...
        data['refund_cheque_money'] = check_float(work_record.refund_cheque_cent / 100)
        data['refund_cheque_count'] = work_record.refund_cheque_count
        data['refund_remittance_money'] = check_float(work_record.refund_remittance_cent / 100)
        data['refund_remittance_count'] = work_record.refund_remittance_count
        data['refund_bank_money'] = check_float(data['refund_pos_money'] + data['refund_remittance_money'])
        data['refund_bank_count'] = data['refund_pos_count'] + data['refund_remittance_count']

        # 收银台支出
        data['payout_total_money'] = check_float(work_record.deposit_refund_cent/100 + work_record.payout_cent/100 + work_record.supplier_borrow_cent/100)
        data['payout_total_count'] = work_record.deposit_refund_count + work_record.payout_count + work_record.supplier_borrow_count
        data['payout_cash_money'] = check_float(work_record.deposit_refund_cash_cent/100 + work_record.payout_cash_cent/100 + work_record.supplier_borrow_cash_cent/100)
        data['payout_cash_count'] = work_record.deposit_refund_cash_count + work_record.payout_cash_count + work_record.supplier_borrow_cash_count
        data['payout_pos_money'] = check_float(work_record.deposit_refund_pos_cent/100 + work_record.payout_pos_cent/100 + work_record.supplier_borrow_pos_cent/100)
        data['payout_pos_count'] = work_record.deposit_refund_pos_count + work_record.payout_pos_count + work_record.supplier_borrow_pos_count
        data['payout_ali_money'] = check_float(work_record.deposit_refund_ali_cent/100 + work_record.payout_ali_cent/100 + work_record.supplier_borrow_ali_cent/100)
        data['payout_ali_count'] = work_record.deposit_refund_ali_count + work_record.payout_ali_count + work_record.supplier_borrow_ali_count
        data['payout_wx_money'] = check_float(work_record.deposit_refund_wx_cent/100 + work_record.payout_wx_cent/100 + work_record.supplier_borrow_wx_cent/100)
        data['payout_wx_count'] = work_record.deposit_refund_wx_count + work_record.payout_wx_count + work_record.supplier_borrow_wx_count
        data['payout_cheque_money'] = check_float(work_record.deposit_refund_cheque_cent/100 + work_record.payout_cheque_cent/100 + work_record.supplier_borrow_cheque_cent/100)
        data['payout_cheque_count'] = work_record.deposit_refund_cheque_count + work_record.payout_cheque_count + work_record.supplier_borrow_cheque_count
        data['payout_remittance_money'] = check_float(work_record.deposit_refund_remittance_cent/100 + work_record.payout_remittance_cent/100 + work_record.supplier_borrow_remittance_cent/100)
        data['payout_remittance_count'] = work_record.deposit_refund_remittance_count + work_record.payout_remittance_count + work_record.supplier_borrow_remittance_count

        data['payout_bank_money'] = check_float(data['payout_pos_money'] + data['payout_remittance_money'])
        data['payout_bank_count'] = data['payout_pos_count'] + data['payout_remittance_count']

        # 收银台收入
        data['income_total_money'] = check_float(
            work_record.other_income_cent / 100 + work_record.waste_selling_income_cent / 100)
        data['income_total_count'] = work_record.other_income_count + work_record.waste_selling_income_count
        data['income_cash_money'] = check_float(
            work_record.other_income_cash_cent / 100 + work_record.waste_selling_income_cash_cent / 100)
        data['income_cash_count'] = work_record.other_income_cash_count + work_record.waste_selling_income_cash_count
        data['income_pos_money'] = check_float(
            work_record.other_income_pos_cent / 100 + work_record.waste_selling_income_pos_cent / 100)
        data['income_pos_count'] = work_record.other_income_pos_count + work_record.waste_selling_income_pos_count
        data['income_ali_money'] = check_float(
            work_record.other_income_ali_cent / 100 + work_record.waste_selling_income_ali_cent / 100)
        data['income_ali_count'] = work_record.other_income_ali_count + work_record.waste_selling_income_ali_count
        data['income_wx_money'] = check_float(
            work_record.other_income_wx_cent / 100 + work_record.waste_selling_income_wx_cent / 100)
        data['income_wx_count'] = work_record.other_income_wx_count + work_record.waste_selling_income_wx_count
        data['income_cheque_money'] = check_float(
            work_record.other_income_cheque_cent / 100 + work_record.waste_selling_income_cheque_cent / 100)
        data[
        ...
        data['income_remittance_money'] = check_float(
            work_record.other_income_remittance_cent / 100 + work_record.waste_selling_income_remittance_cent / 100)
        data[
            'income_remittance_count'] = work_record.other_income_remittance_count + work_record.waste_selling_income_remittance_count
        data['income_bank_money'] = check_float(data['income_pos_money'] + data['income_remittance_money'])
        data['income_bank_count'] = data['income_pos_count'] + data['income_remittance_count']

        # 现金清点金额
        cash_expect_money = check_float((work_record.order_cash_cent
                                         + work_record.pb_cash_cent
                                         + work_record.order_combine_cash_cent
                                         + work_record.pb_combine_cash_cent
                                         + work_record.order_fee_cash_cent
                                         + work_record.other_income_cash_cent
                                         + work_record.waste_selling_income_cash_cent
                                         - work_record.refund_cash_cent
                                         - work_record.deposit_refund_cash_cent
                                         - work_record.payout_cash_cent
        ...
        data['cash_discrepancy_money'] = check_float(cash_check_money - cash_expect_money)

        # 微信/支付宝/POS清点金额
        data['wx_expect_money'] = check_float((work_record.order_wx_cent + work_record.pb_wx_cent) / 100)
        data['ali_expect_money'] = check_float((work_record.order_ali_cent + work_record.pb_ali_cent) / 100)
        data['pos_expect_money'] = check_float((work_record.order_pos_cent + work_record.pb_pos_cent) / 100)
        ...
        # 退押金金额
        data['deposit_refund_money'] = check_float(work_record.deposit_refund_cent / 100)
        data['deposit_refund_count'] = work_record.deposit_refund_count

        # 押金收支出
        data['deposit_spend'] = check_float(data['deposit_money'] - data['deposit_refund_money'])

        # 抹零金额
        data['erase_money'] = check_float(work_record.erase_total_cent / 100)
        data['erase_count'] = work_record.erase_total_count
        data['erase_cash_money'] = check_float(work_record.erase_cash_cent / 100)
        ...
        data['erase_combine_money'] = check_float(work_record.erase_combine_cent / 100)
        data['erase_combine_count'] = work_record.erase_combine_count
        data['erase_cheque_money'] = check_float(work_record.erase_cheque_cent / 100)
        data['erase_cheque_count'] = work_record.erase_cheque_count
        data['erase_remittance_money'] = check_float(work_record.erase_remittance_cent / 100)
        data['erase_remittance_count'] = work_record.erase_remittance_count
        data['erase_bank_money'] = check_float(data['erase_pos_money'] + data['erase_remittance_money'])
        data['erase_bank_count'] = data['erase_pos_count'] + data['erase_remittance_count']

        # 手续费
        data['order_fee_money'] = check_float(work_record.order_fee_cent / 100)
        data['order_fee_count'] = work_record.order_fee_count
        data['order_fee_cash_money'] = check_float(work_record.order_fee_cash_cent / 100)
        data['order_fee_cash_count'] = work_record.order_fee_cash_count
        data['order_fee_pos_money'] = check_float(work_record.order_fee_pos_cent / 100)
        data['order_fee_pos_count'] = work_record.order_fee_pos_count
        data['order_fee_ali_money'] = check_float(work_record.order_fee_ali_cent / 100)
        data['order_fee_ali_count'] = work_record.order_fee_ali_count
        data['order_fee_wx_money'] = check_float(work_record.order_fee_wx_cent / 100)
        data['order_fee_wx_count'] = work_record.order_fee_wx_count
        data['order_fee_cheque_money'] = check_float(work_record.order_fee_cheque_cent / 100)
        data['order_fee_cheque_count'] = work_record.order_fee_cheque_count
        data['order_fee_remittance_money'] = check_float(work_record.order_fee_remittance_cent / 100)
        data['order_fee_remittance_count'] = work_record.order_fee_remittance_count
        data['order_fee_bank_money'] = check_float(data['order_fee_pos_money'] + data['order_fee_remittance_money'])
        data['order_fee_bank_count'] = data['order_fee_pos_count'] + data['order_fee_remittance_count']

        # 实收资金
        data['total_money'] = check_float((work_record.order_total_cent
                                           + work_record.pb_total_cent
                                           + work_record.other_income_cent
                                           + work_record.waste_selling_income_cent
                                           + work_record.order_fee_cent
                                           - work_record.refund_total_cent
                                           - work_record.deposit_refund_cent
                                           - work_record.payout_cent
        ...
        data['total_cash_money'] = check_float((work_record.order_cash_cent
                                                + work_record.order_combine_cash_cent
                                                + work_record.pb_cash_cent
                                                + work_record.pb_combine_cash_cent
                                                + work_record.other_income_cash_cent
        ...
        data['total_pos_money'] = check_float((work_record.order_pos_cent
                                               + work_record.order_combine_pos_cent
                                               + work_record.pb_pos_cent
                                               + work_record.pb_combine_pos_cent
                                               + work_record.other_income_pos_cent
                                               + work_record.waste_selling_income_pos_cent
                                               + work_record.order_fee_pos_cent
                                               - work_record.refund_pos_cent
                                               - work_record.deposit_refund_pos_cent
                                               - work_record.payout_pos_cent
        ...
        data['total_ali_money'] = check_float((work_record.order_ali_cent
                                               + work_record.order_combine_ali_cent
                                               + work_record.pb_ali_cent
                                               + work_record.pb_combine_ali_cent
                                               + work_record.other_income_ali_cent
                                               + work_record.waste_selling_income_ali_cent
                                               + work_record.order_fee_ali_cent
                                               - work_record.refund_ali_cent
                                               - work_record.deposit_refund_ali_cent
                                               - work_record.payout_ali_cent
        ...
        data['total_wx_money'] = check_float((work_record.order_wx_cent
                                              + work_record.order_combine_wx_cent
                                              + work_record.pb_wx_cent
                                              + work_record.pb_combine_wx_cent
                                              + work_record.other_income_wx_cent
                                              + work_record.waste_selling_income_wx_cent
                                              + work_record.order_fee_wx_cent
                                              - work_record.refund_wx_cent
                                              - work_record.deposit_refund_wx_cent
                                              - work_record.payout_wx_cent
        ...
        data['total_cheque_money'] = check_float((work_record.order_cheque_cent
                                                  + work_record.order_combine_cheque_cent
                                                  + work_record.pb_cheque_cent
                                                  + work_record.pb_combine_cheque_cent
                                                  + work_record.other_income_cheque_cent
                                                  + work_record.waste_selling_income_cheque_cent
                                                  + work_record.order_fee_cheque_cent
                                                  - work_record.refund_cheque_cent
        ...
        data['total_remittance_money'] = check_float((work_record.order_remittance_cent
                                                      + work_record.order_combine_remittance_cent
                                                      + work_record.pb_remittance_cent
                                                      + work_record.pb_combine_remittance_cent
                                                      + work_record.other_income_remittance_cent
                                                      + work_record.waste_selling_income_remittance_cent
                                                      + work_record.order_fee_remittance_cent
                                                      - work_record.refund_remittance_cent
        ...
        data['total_bank_money'] = check_float(data['total_pos_money'] + data['total_remittance_money'])
        data['total_bank_count'] = data['total_pos_count'] + data['total_remittance_count']
        return data
...
class OrderBaseFunc():
    ...
    def gen_order_num(cls, session, shop_id, source="order"):
        ...
        if redis.get(key):
            ...
            if (source == "multi_order" and len(order_num) != 9) or (source != "multi_order" and len(order_num) != 11):
                redis.delete(key)
                return None
            # TODO END
            ...
            order_num = str(int(latest_order_num) + 1) if latest_order_num else init_order_num
    ...
    def gen_short_num(cls, session, shop_id, source="payout"):
        ...
        if redis.get(key):
            ...
            db_model = session.query(models.PayoutShortNum.num).filter_by(
                shop_id=shop_id).order_by(desc(models.PayoutShortNum.id)).first()
            num = "10001"
    ...
    @classmethod
    def get_db_order(cls, session, shop_id, front_end_pay_id, last_update_timestamp, if_pborder=False, if_lock=False):
        if if_pborder:
            order_redis_key = "pb_order_frontid_updatetime:%d:%s:%d" % (
            shop_id, front_end_pay_id, last_update_timestamp)
            models_order = models.PbOrder
        else:
            order_redis_key = "order_frontid_updatetime:%d:%s:%d" % (shop_id, front_end_pay_id, last_update_timestamp)
            models_order = models.Order
    ...
    @classmethod
    def create_db_order(cls, session, shop_id, accountant_id, debtor_id, front_end_pay_id, if_pborder=False,
                        multi=False):
        if if_pborder:
            models_order = models.PbOrder
            source = "pb_order"
    ...
    @classmethod
    def redis_save_order(cls, shop_id, front_end_pay_id, last_update_timestamp, order_id, if_pborder=False):
        """用于在redis中缓存支付结果
        """
        if if_pborder:
            order_redis_key = "pb_order_frontid_updatetime:%d:%s:%d" % (
    ...
    @classmethod
    def redis_save_order_lock(cls, shop_id, front_end_pay_id, last_update_timestamp, if_pborder=False):
        """防止并发生成订单
        """
        if if_pborder:
    ...
    def finish_order(cls, session, order, total_fee=0, if_pborder=False):
        ...
        if if_pborder:
            ...
            # 处理小票还款记录状态
            session.query(GoodsSalesRecordPaybackRecord).filter_by(pb_order_id=order_id).update({"status": 1})
            ...
            goods_sale_record_id_list = session.query(GoodsSalesRecordPaybackRecord.goods_sale_record_id,
                ...
                .all()
            payback_record_dict = {x.goods_sale_record_id: x.payback_cent for x in goods_sale_record_id_list}
            record_list = session.query(GoodsSalesRecord) \
                .filter(GoodsSalesRecord.id.in_(payback_record_dict.keys())) \
                .all()
            for record in record_list:
                goods_sale_record_id = record.id
                record.unpayback_money -= payback_record_dict.get(goods_sale_record_id, 0)

            # 处理历史欠款还款记录状态
            session.query(HistoryPaybackRecord).filter_by(pb_order_id=order_id).update({"status": 1})
            ...
            debt_record_id_list = session.query(HistoryPaybackRecord.debt_history_id,
                ...
                .filter_by(pb_order_id=order_id) \
                .all()
            payback_record_dict = {x.debt_history_id: x.payback_cent for x in debt_record_id_list}
            ...
            for record in record_list:
                storage_record = cls.update_sales_record(
                    session, shop_id, record, order, goods_dict, account_period_date)
            ...
            multi_order_id_list = set(x.multi_order_id for x in record_list if x.multi_order_id)
        ...
        if order.erase_money:
            order_erase = session.query(models.OrderErase).filter_by(order_id=order.id, order_type=order_type).first()
            if order_erase:
                order_erase.status = 2
                order_erase.account_period_date = account_period_date
        # 处理手续费记录
        if order.fee:
            order_fee = session.query(models.OrderFee).filter_by(order_id=order.id, order_type=order_type).first()
            if order_fee:
                order_fee.status = 2
                ...
                cls.add_order_fundflow(session, shop_id, order.pay_type, 10, order_fee.fee, order.id, order.accountant_id,
        ...
        if if_pborder:
            ...
            flow_source_type = 2
            deposit_record_type = ""
            pay_type_text = cls.get_paytype_through_order(order, source_type="pb")
            ...
            pay_type_text = cls.get_paytype_through_order(order, source_type="order")
        ...
        if (not if_pborder and order.pay_type == 9) or if_pborder:
            ...
            DebtFunc.update_debt(session, debt_type, debt_value, customer_id, shop_id, operator_id, fk_id,
                                 account_period_date=account_period_date)
        # 有实际入账的,资金流水表
        if if_pborder or order.pay_type != 9:
            ...
            # 组合支付需生成两条流水记录
            if fund_type == 10:
                cls.add_combine_order_fundflow(session, shop_id, flow_source_type, fund_value, order.id, accountant_id)
            else:
                cls.add_order_fundflow(session, shop_id, fund_type, flow_source_type, fund_value, fk_id, accountant_id)
            session.flush()
        ...
        if not if_pborder:
            # 单独更新一下押金
            if deposit_cent:
                AccoutantFunc.update_work_record(session, accountant_id, shop_id, deposit_cent, deposit_record_type,
                                                 fk_id)
            ...
            if customer_id:
                if order_record_type == 0:
                    ...
                    # 更新用户积分
                    customer = ShopCustomerFunc.get_shop_customer_through_id(session, shop_id, customer_id)
                    ShopCustomerFunc.purchase_point(session, shop_id, customer, order, accountant_id)
                else:
                    update_purchase_money = -order.fact_total_price
                ShopCustomerFunc.update_purchase_info(
                    session, customer_id, shop_id, purchase_money = update_purchase_money, purchase_id = purchase_id)

        # 更新收银员抹零记录
        if order.erase_money:
            erase_pay_type_text = cls.get_paytype_through_order(order, source_type="order_erase")
            AccoutantFunc.update_work_record(session, accountant_id, shop_id, order.erase_money, erase_pay_type_text, fk_id)

        # 更新手续费记录
        if order.fee:
            fee_pay_type_text = cls.get_paytype_through_order(order, source_type="order_fee")
            AccoutantFunc.update_work_record(session, accountant_id, shop_id, order.fee, fee_pay_type_text, fk_id)

        # 收银员收银记录
        AccoutantFunc.update_work_record(session, accountant_id, shop_id, total_fee, pay_type_text, fk_id)
    ...
    def update_sales_record(
            cls, session, shop_id, record, order, goods_dict, account_period_date,
            order_time=None, recover_storage=True):
    ...
    # 收银台结算完成之后发送短信
    @classmethod
    def send_message_after_settlement(cls, session, order, if_pborder, is_refund=0):
        customer_id = order.debtor_id
        shop_id = order.shop_id
        accountant_id = order.accountant_id
        salersman_settlement_send_message_key = "salersman_settlement_send_message_key:%d"%(order.id)
        errmsg, notify_phone = "not send", ""
        if customer_id:
            customer = ShopCustomerFunc.get_shop_customer_through_id(session, shop_id, customer_id)
            accountant_name = session.query(models.Accountinfo.realname, models.Accountinfo.nickname).filter_by(
            ...
            point_display = session.query(models.Config.point_display).filter_by(id=shop_id).scalar() or 0
            # 过滤特殊字符
            ...
            if touser:
                send(CustomerFlow, customer_name, shop_id, shop_name, type, time, accountant_name=accountant_name,
                     total_price=total_price, point_display=point_display, debt_rest=debt_rest,
                     purchase_times=customer.purchase_times, point=customer.point, touser=touser)
            ...
            if redis.get(salersman_settlement_send_message_key):
                ...
                if notify_phone:
                    if if_pborder:
                        # 发送还款短信
                        errmsg = send_yunpian_accountant_settlement_payback(notify_phone,
                                                                            customer_name,
                                                                            shop_name,
                                                                            total_price,
                                                                            debt_rest,
                        ...
                        # 发送赊账退单短信
                        if is_refund:
                            errmsg = send_yunpian_credit_refund(notify_phone,
                                                                customer_name,
                                                                shop_name,
                                                                total_price,
                                                                debt_rest,
                        ...
                        # 发送赊账短信
                            ...
                            errmsg = send_yunpian_accountant_settlement_credit(notify_phone,
                                                                               customer_name,
                                                                               shop_name,
                                                                               total_price,
                                                                               debt_rest,
                        ...
                        # 发送退单短信
                        if is_refund:
                            errmsg = send_yunpian_refund(notify_phone,
                                                         customer_name,
                                                         shop_name,
                                                         total_price,
                                                         time,
                        ...
                        # 查询店铺是否开启积分功能
                            ...
                            errmsg = send_yunpian_accountant_settlement_normal(notify_phone,
                                                                               customer_name,
                                                                               shop_name,
                                                                               customer.purchase_times,
                                                                               total_price,
                                                                               customer.point,
                                                                               time,
                                                                               accountant_name,
        ...
        if errmsg != True:
            # BUG-收银台短信发送失败信息跟踪
            log_msg("accountant_message_error",
                    str([customer_id, redis.get(salersman_settlement_send_message_key),
                         notify_phone, errmsg, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")]),
                    "%Y-%m")
    ...
    def send_message_after_refund(cls, session, order, send_sms):
        ...
        customer = ShopCustomerFunc.get_shop_customer_through_id(session, order.shop_id, order.debtor_id)
        ...
        if order.pay_type == 9:
            ...
            type = 4
        if touser:
            send(CustomerFlow, customer_name, order.shop_id, shop_name, type, send_time, accountant_name=accountant_name,
                 total_price=total_price, debt_rest=debt_rest, touser=touser)

        if send_sms:

            if customer.phone:
                if order.pay_type == 9:
                    errmsg = send_yunpian_credit_refund(customer.phone, customer_name, shop_name, total_price, debt_rest, send_time, accountant_name)
                else:
                    errmsg = send_yunpian_refund(customer.phone, customer_name, shop_name, total_price, send_time, accountant_name)
            else:
                errmsg = "no phone registered"

            if isinstance(errmsg, str) and errmsg:
                log_msg("refund_sms_error", str([order.debtor_id, customer.phone, errmsg, send_time]), "%Y-%m")

    #组合支付流水记录
    @classmethod
    def add_combine_order_fundflow(
            cls, session, shop_id, source_type, args_fund_value, order_id, accountant_id):
        '''
            条码支付的流水值是取自支付回调中实际取得的值需单独处处理（fund_type为3/4的情况使用的是回调值）
        '''
        order_money = cls.get_order_money(session, order_id, source_type=source_type)
        ...
        if cash_cent:
            ...
            cls.add_order_fundflow(session, shop_id, fund_type, source_type, fund_value, order_id, accountant_id)
        if pos_cent:
            fund_type = 2
            fund_value = pos_cent
            cls.add_order_fundflow(session, shop_id, fund_type, source_type, fund_value, order_id, accountant_id)
    ...
    @classmethod
    def get_order_money_through_id_list(cls, session, order_id_list, source_type="order"):
        if order_id_list:
    ...
    # 添加流水
    @classmethod
    def add_order_fundflow(cls, session, shop_id, fund_type, source_type, fund_value, fk_id, accountant_id,
                           create_date_time=None, account_period_date=None,
                           fee_order_type=0):
        if not create_date_time:
            create_date_time = datetime.datetime.now()
        t_year, t_month, t_day, t_week = TimeFunc.get_date_split(create_date_time)

        if not account_period_date:
            account_period_date = TimeFunc.time_to_str(create_date_time, 'date')
            if AccountingPeriodFunc.get_accounting_time(session, shop_id):
                account_period_date = TimeFunc.time_to_str(create_date_time + datetime.timedelta(1), 'date')
        fund_flow_new = models.FundFlow(shop_id=shop_id,
                                        create_year=t_year,
    ...
    # 获取订单条码支付类型
    @classmethod
    def get_order_online_type(self, session, order, if_front=False, if_pborder=False):
        pay_type = ''
    ...
    @classmethod
    def barpay_lcsw_order(cls, session, shop_id, auth_code, order, if_pborder=False):
        ret_dict = {}
        lc_merchant_no, lc_terminal_id, lc_access_token = ShopFunc.get_lcpay_account(session, shop_id)
        pay_type = cls.get_order_online_type(session, order, if_pborder=if_pborder)
        ...
        if if_pborder:
            ...
            order_num = order.num
        shop_name = session.query(models.Shop.shop_name).filter_by(id=shop_id).scalar()
        order_barcode_price = cls.get_order_barcode_price(session, order, if_pborder=if_pborder)
        parameters = LcswPay.getBarcodePayParas(pay_type, auth_code, order_num, create_time, str(order_barcode_price),
                                                shop_name, lc_merchant_no, lc_terminal_id, lc_access_token)
        ...
        log_msg('lc_bar_parameters/%s' % log_date, 'order_num:%s' % (order_num))
        log_msg('lc_bar_parameters/%s' % log_date, 'datetime:%s' % (str(datetime.datetime.now())))
        log_msg_dict('lc_bar_parameters/%s' % log_date, parameters)
        r = requests.post(LCSW_HANDLE_HOST + '/pay/100/barcodepay', data=json.dumps(parameters), verify=False,
                          headers=headers)
        try:
            ...
            send_servererror_msg.apply_async(
                args=["oAOOiwFYvm8judvvfLr2bteSI9oQ", None, "条码支付失败", "接口返回有误", time_now, ""], countdown=0)
            return 0, ret_dict
        log_msg('lc_bar_res/%s' % log_date, 'order_num:%s' % (order_num))
        log_msg('lc_bar_res/%s' % log_date, 'datetime:%s' % (str(datetime.datetime.now())))
    ...
    @classmethod
    def set_query_base(cls, redis_query_key, redis_query_value, redis_stop_key, redis_stop_value):
    ...
    def query_lcsw_order(cls, session, shop_id, order, if_pborder=False):
        ...
        if out_trade_no:
            parameters = LcswPay.getQueryParas(pay_type, order_query_num, out_trade_no, lc_merchant_no, lc_terminal_id,
                                               lc_access_token)
        else:
            parameters = LcswPay.getQueryParas(pay_type, order_query_num, out_trade_no, lc_merchant_no, lc_terminal_id,
                                               lc_access_token, pay_trace=pay_trace, pay_time=create_time)
        headers = {'content-type': 'application/json'}
        r = requests.post(LCSW_HANDLE_HOST + '/pay/100/query', data=json.dumps(parameters), verify=False,
                          headers=headers)
        res_dict = json.loads(r.text)
        log_date = datetime.datetime.now().strftime('%Y%m%d')
    ...
    def refund_lcsw_order(cls, session, shop_id, order, refund_fee):
        ...
        out_trade_no = order.lc_out_trade_no
        if out_trade_no:
            parameters = LcswPay.getRefundParas(pay_type, order_refund_num, out_trade_no, str(refund_fee),
                                                lc_merchant_no, lc_terminal_id, lc_access_token)
        else:
            parameters = LcswPay.getRefundParas(pay_type, order_refund_num, out_trade_no, str(refund_fee),
                                                lc_merchant_no, lc_terminal_id, lc_access_token, pay_trace=pay_trace,
                                                pay_time=create_time)
        headers = {'content-type': 'application/json'}
        r = requests.post(LCSW_HANDLE_HOST + '/pay/100/refund', data=json.dumps(parameters), verify=False,
                          headers=headers)
        res_dict = json.loads(r.text)
        ret_dict["out_trade_no"] = res_dict.get('out_trade_no', '')
    ...
    def encode_dict(cls, params):
        ...
        return {k: six.u(v).encode('utf-8') if isinstance(v, str) else v.encode('utf-8') if isinstance(v,
                                                                                                       six.string_types) else v
                for k, v in six.iteritems(params)}

    @classmethod
    def wappay_url_lcsw(cls, session, shop_id, shop_name, total_fee, wap_order):
        try:
        ...
        headers = {'content-type': 'application/json'}
        parameters = LcswPay.getWapPayParas(lc_merchant_no, lc_terminal_id, terminal_trace, total_fee, shop_name,
                                            lc_access_token)
        pay_url = LCSW_HANDLE_HOST + '/open/wap/110/pay?' + urlencode(cls.encode_dict(parameters))
        return pay_url
    ...
    @classmethod
    def multi_combine_pay_type_text(cls, session, order_id_list, source_type="order"):
        """ 批量获取组合支付类型

        :param order_id_list 结算/还款订单ID
        :param source_type order/pb_order 结算/还款
        """
        pay_dict = cls.combine_pay_type_detail_through_id_list(session, order_id_list, source_type=source_type)
        pay_type_text_list = []
        combine_pay_dict = dict()
        for order_id in pay_dict:
            pay_type_dict = pay_dict[order_id]
            pay_list = ["%s:%s元" % (pay_type, pay_money) for pay_type, pay_money in pay_type_dict.items()]
    ...
    # 批量获取组合支付详情
    @classmethod
    def combine_pay_type_detail_through_id_list(cls, session, order_id_list, return_type="text", source_type="order"):
        order_money_list = cls.get_order_money_through_id_list(session, order_id_list, source_type=source_type)
        ...
        for order_money in order_money_list:
            ...
            for column_name in pay_type_column_dict:
                ...
                if order_money_dict.get(column_name, 0):
                    ...
                    if return_type == "text":
                        pay_dict[order_id][cls.pay_type_text(column_code)] = money
                    else:
    ...
    def get_refund_pay_type_text(self, refund_pay_type):
        refund_pay_type_dict = {1: "refund_cash",
                                2: "refund_pos",
                                3: "refund_wx",
                                4: "refund_alipay",
                                7: "refund_wx",
...
class OrderSalesRecordFunc():
    ...
    @classmethod
    def batch_format_order_record_info(
            self, session, sale_records, return_type="dict",
            assign_accounting_date=None, attach_refund=False, get_wximgurl=False):
        ''' 批量处理订单和小票的数据
        ...
        order_num_dict = OrderBaseFunc.get_order_num_through_id_list(session, order_id_list)
        # 加上判断，如果需要返回wximgurl则返回，目的是不影响其它接口的返回结果
        if not get_wximgurl:
            staff_name_dict = AccountFunc.get_account_name_through_id_list(session, account_id_list)
        else:
            staff_name_dict, staff_wximgurl_dict = AccountFunc.\
                get_account_name_wximg_through_id_list(session, account_id_list)
        customer_name_dict = ShopCustomerFunc.get_shop_customer_name_through_id_list(session, customer_id_list)
        goods_name_unit_dict = GoodsFunc.get_goods_name_unit_through_id_list(session, goods_id_list)
        group_id_dict = GoodsFunc.get_group_id_dict_with_goods_ids(session, goods_id_list)
        group_name_dict = GoodsGroupFunc.get_group_name_dict(session, list(
            set([group_id_dict.get(goods_id) for goods_id in group_id_dict])))

        goods_batch_num_dict = GoodsBatchFunc.get_batch_num_through_id_list(session, batch_id_list)
        combine_pay_dict = OrderBaseFunc.multi_combine_pay_type_text(session, order_id_list)
        multi_order_id_list = {r.multi_order_id for r in sale_records}
        order_dict = OrderBaseFunc.get_order_dict_throuth_id_list(session, multi_order_id_list)
        supplytype_goodstype_dict = GoodsFunc.get_goods_supplytype_goodstype_through_id_list(session, goods_id_list)
        if return_type == "dict":
            record_data = {}
        else:
            record_data = []
        ...
        for record in sale_records:
            ...
            accountant_name = staff_name_dict.get(accountant_id, "收银员%d" % accountant_id)
            ...
            goods_unit = GoodsFunc.getUnit(
                goods_info["unit"], weight_unit_text=weight_unit_text) if goods_info else ""
            salesman_id = record.salesman_id
            salesman_name = staff_name_dict.get(salesman_id, "开票员%d" % salesman_id)
            sales_time = str(record.bill_year) + '-' + str(record.bill_month).zfill(2) + '-' + str(
            ...
            shop_supplier_name = record.shop_supplier_name if goods_info and goods_info["supply_type"] > 0 else ""
            ...
            sales_num = check_float(record.get_sales_num(
            ...
            if goods_id in supplytype_goodstype_dict:
                ...
                supply_type = supplytype_goodstype_dict[goods_id]["supply_type"]
            ...
            # 获取小票已还款金额
            payback_money = 0
            if pay_type == 9 and unpayback_money >= 0 and record.status in [3, 5]:
                payback_money = check_float((receipt_money - unpayback_money) / 100)
            ...
            if record.account_period_date:
                ...
                if assign_accounting_date and (record.account_period_date - assign_accounting_date).days == 1:
                    accounting = 1

            # 扎帐日期
            account_period_date = record.account_period_date.strftime("%Y-%m-%d")
            ...
            record_info["receipt_money"] = check_float(receipt_money / 100)  # 票据金额
            record_info["shop_supplier_name"] = shop_supplier_name  # 供货商名称
            record_info["only_goods_name"] = goods_name  # 商品名称
            record_info["goods_name"] = goods_name_with_supplier  # 商品名称（带供货商名称）
            record_info["goods_unit"] = goods_unit  # 商品单位
            record_info["group_id"] = group_id  # 商品分类id
            record_info["group_name"] = group_name  # 商品分类名称
            record_info["fact_price"] = check_float(record.get_fact_price(
                goods_info["unit"], weight_unit_text=weight_unit_text) / 100)  # 单价
            record_info["sales_num"] = "-" if goods_info["unit"] == 1 else sales_num  # 已售重量
            record_info["gross_weight"] = check_float(record.get_gross_weight(
                weight_unit_text=weight_unit_text))  # 毛重
            record_info["tare_weight"] = check_float(record.get_tare_weight(
            ...
            record_info["storage_num"] = check_float(
                record.get_storage_num(weight_unit_text=weight_unit_text) / 100)  # 剩余重量
            record_info["commission_mul"] = check_float(commission_mul / 100)  # 已售件数
            ...
            record_info["deposit_total"] = check_float(deposit_total / 100)  # 押金小计
            ...
            record_info["create_time"] = TimeFunc.time_to_str(record.create_time)
            ...
            if record_type == 1:
                ...
                if record_info["unpayback_money"] <= 0:
                    payback_status = "已还清"
                elif record_info["unpayback_money"] < record_info["receipt_money"]:
                    payback_status = "部分还款"
                elif record_info["unpayback_money"] == record_info["receipt_money"]:
                    payback_status = "未还款"
                else:
            ...
            if attach_refund:
                record_info["shown_status_print"] = GoodsSaleFunc.get_record_shown_status_print(
        ...
        if get_wximgurl:
            if not return_type == "dict":
                for data in record_data:
                    data["accountant_wximgurl"] = staff_wximgurl_dict.get(data.get("accountant_id", 0), "")
                    data["salesman_wximgurl"] = staff_wximgurl_dict.get(data.get("salesman_id", 0), "")
            else:
...
class StorageRecordFunc():
    ...
    @classmethod
    def batch_format_storage_record_info(cls, session, record_list, shop_id,
                                         goods_id, weight_unit_text, return_type="list"):
        '''
            批量处理库存变更记录的数据
        ...
        account_dict, account_wximgurl_dict = AccountFunc.get_account_name_wximg_through_id_list(session, account_ids)
        ...
        for record in record_list:
            ...
            data = dict(
                ...
                mul=check_float((record.now_storage - record.origin_storage) / 100),
                num=check_float(
                    (record.get_now_storage_num(weight_unit_text=weight_unit_text) -
                     record.get_origin_storage_num(weight_unit_text=weight_unit_text)) / 100),
                now_storage=check_float(record.now_storage / 100),
                now_storage_num=check_float(
                    record.get_now_storage_num(weight_unit_text=weight_unit_text) / 100),
                operator_id=record.operater_id,
                operator_name=account_dict.get(record.operater_id, ""),
                operator_wximgurl=account_wximgurl_dict.get(record.operater_id, ""),
    ...
    def format_export_storage_record_body(cls, session, record_list, shop_id,
        ...
        for storage_record in record_list:
            ...
            if record_info["type_id"] in (1, 4):
                foreign_record_num_info = '入库单号：' + record_info["foreign_record_num"]
            elif record_info["type_id"] in (6, 7):
                foreign_record_num_info = '报损单号：' + record_info["foreign_record_num"]
            elif record_info["type_id"] in (8, 9, 10):
                foreign_record_num_info = '调货单号：' + record_info["foreign_record_num"]
            elif record_info["type_id"] in (11,):
                foreign_record_num_info = '盘点单号：' + record_info["foreign_record_num"]
            else:
            ...
            data_content.append([
                ...
                str(record_info["mul"]) + '件/' + str(record_info["num"]) + weight_unit_text,
                str(record_info["now_storage"]) + '件/' + str(record_info["now_storage_num"]) + weight_unit_text,
    ...
    @classmethod
    def get_storage_data(cls, args, session, statistic_session, shop_id, master_goods_ids=None):
        date = args['date']
        end_date = args.get("end_date", "")
        data_type = args.get("data_type", 0)
        statistic_type = args.get("statistic_type", 1)
        ...
        storage_data_ret = storage_data_ret.filter(or_(StatisticGoodsStorage.stockin_record != 0,
        ...
        if statistic_type == 1 and start_date != end_date:
            ...
            storage_range_dict = StorageRecordFunc.get_begin_end_storage(args, session, shop_id, goods_id_list)
        ...
        if statistic_type == 4:
            goods_id_list = {data.goods_id for data in storage_data}
            goods_storage_dict = StorageRecordFunc.get_begin_end_storage(args, session, shop_id, goods_id_list)
    ...
    def get_begin_end_storage(cls,args,session,shop_id,goods_id_list):
        ...
        start_date, end_date = TimeFunc.get_to_date_by_from_date(date, end_date, statistic_type)
    ...
    @classmethod
    def get_extra_storage_data(cls, args, session, shop_id, storage_data, data_sales_ret):
        ...
        extra_storage_dict = StorageRecordFunc.get_begin_end_storage(args, session, shop_id, goods_to_query)

        return extra_storage_dict

    @classmethod
    def get_storage_allocate_end_time_by_storage_allocate_id(cls, session, storage_allocate_id):
        """获取库存调度的完成时间"""
        GoodsStorageRecord = models.GoodsStorageRecord
        filter_list = [GoodsStorageRecord.fk_id == storage_allocate_id, GoodsStorageRecord.type.in_([8, 9, 10])]
...
class SupplierFlowFunc():
    ...
    @classmethod
    def batch_format_supplier_flow_info(cls, session, shop_id, flow_list,
                                        return_type="list", data_type=0):
        '''
            批量处理贷款结算记录的数据
            :param shop_id: 店铺id
        ...
        operater_name_wximgurl_list = AccountFunc.get_account_name_wximg_through_id_list(session, operater_id_list)
        operater_name_dict, operater_wximgurl_dict = operater_name_wximgurl_list[0], operater_name_wximgurl_list[1]
        # 查询供货商信息
        supplier_id_list = {x.shop_supplier_id for x in flow_list}
        supplier_info_dict = SupplierFunc.get_shop_supplier_base_info(session, shop_id, supplier_id_list)
        # 查询商品信息，不查询预支记录的商品
        clearing_id_list = {x.fk_id for x in flow_list if x.flow_type == 0}
        clearing_goods_name_dict = cls._get_multi_clearing_goods(session, clearing_id_list)
        accounting_time = AccountingPeriodFunc.get_accounting_time(
            session, shop_id) if data_type == 1 else None
        # 查询货款结算手续费
        procedure_dict = cls._get_clearing_procedure_fee(session, clearing_id_list)
        ...
        for flow in flow_list:
            supplier_info = supplier_info_dict.get(flow.shop_supplier_id, dict(name="", phone="", wximgurl=""))
            flow_id = flow.fk_id
            data = {
                "supplier_id": flow.shop_supplier_id,
                "supplier_name": supplier_info["name"],
                "supplier_phone": supplier_info["phone"],
                "supplier_wximgurl": supplier_info["wximgurl"],
                "create_date": TimeFunc.time_to_str(flow.create_date),
                "flow_value": check_float(flow.flow_value / 100),
                "operater_id": flow.operater_id,
                "operater_name": operater_name_dict.get(flow.operater_id, ""),
                "operater_wximgurl": operater_wximgurl_dict.get(flow.operater_id, ""),
                "flow_id": flow_id,
                "goods_id": flow.goods_id or 0,
                "goods_list": clearing_goods_name_dict.get(flow_id, []),
                "sms_sent": flow.sms_sent,
                "is_after_accounting":
                    1 if accounting_time and flow.create_date > accounting_time else 0,
                "procedure_fee":procedure_dict.get(flow_id) if flow.flow_type == 0 else 0
            }
            if return_type == "dict":
    ...
    def _get_multi_clearing_goods(cls, session, clearing_id_list):
        ...
        goods_name_dict = GoodsFunc.get_goods_name_through_id_list(session, clearing_goods_id_list)
        ...
        for clearing_id, goods_id in clearing_id_list:
            ...
            clearing_goods_name_dict[clearing_id].append({"goods_id": goods_id, "goods_name": goods_name})
    ...
    def _get_clearing_procedure_fee(cls, session, clearing_id_list):
        ...
        procedure_list = session \
            ...
            .all()
        procedure_dict = {x.id:check_float(x.procedure_cent/100) for x in procedure_list}
    ...
    def format_export_supplier_flow_body(cls, session, shop_id, flow_list):
        ...
        data_content = []
        data_dict = SupplierFlowFunc.batch_format_supplier_flow_info(session, shop_id, flow_list, return_type="dict")
    ...
    @classmethod
    def batch_format_borrowing_flow_info(cls, session, shop_id, flows_borrowing_records, data_type=0):
        """ 构造预支流水的返回字典 """

        # 查询操作员信息
        operater_id_list = {flow.operater_id for flow, _ in flows_borrowing_records}
        operator_name_dict, operator_wximgurl_dict = AccountFunc.\
            get_account_name_wximg_through_id_list(session, operater_id_list)
        # 查询供货商信息
        supplier_id_list = {flow.shop_supplier_id for flow, _ in flows_borrowing_records}
        supplier_info_dict = SupplierFunc.get_shop_supplier_base_info(session, shop_id, supplier_id_list)
        # 查询商品信息
        ...
        for flow, record in flows_borrowing_records:
            supplier_info = supplier_info_dict.get(flow.shop_supplier_id, dict(name="", phone="", wximgurl=""))
            cancelable = record.create_date.date() == datetime.date.today() and record.status == 0
            data_list.append({
                ...
                "flow_type_text": flow.flow_type_text,
                "flow_value": check_float(flow.flow_value/100),
                "operater_id": record.operater_id,
                "operater_name": operator_name_dict.get(record.operater_id, ""),
                "operator_wximgurl": operator_wximgurl_dict.get(record.operater_id, ""),
                "borrow_type": record.borrow_type,
                "borrow_type_text": record.borrow_type_text,
                "borrow_pay_type": record.borrow_pay_type,
                "borrow_pay_type_text": record.borrow_pay_type_text,
                "reason": record.reason,
                "borrowing_status": record.status,
    ...
    def format_export_clearing_flow_body(cls, data_list):
        data_content = []
        data_content.append(["时间", "供货商", "结款货品", "结算金额", "手续费", "经办人", "短信通知"])
        for data in data_list:
            data_content.append([
                data["create_date"],
                data["supplier_name"],
    ...
    def format_export_supplier_money_statistic_body(cls, data_list):
        data_content = []
        data_content.append([
            "供货商ID", "供货商", "应结货款", "已结货款", "未结货款", "预支总额", "已结预支", "未结预支", "期末货款"])
        for data in data_list:
...
class StockinHistoryFunc():
    ...
    @classmethod
    def batch_format_stockin_history_info(cls, session, flows, weight_unit_text,
                                          map_groups_id_name, return_type="list"):
        '''
            批量处理采购入库记录的数据
        ...
        goods_supplier_name_dict = GoodsFunc.get_goods_and_supplier_names(session, goods_id_list)
    ...
    @classmethod
    def format_export_stockin_history_body(cls, session, flows,
                                           weight_unit_text, map_groups_id_name):
        ...
        data_content.append(
            ['序号', '入库时间', '到货时间', '单据号', '件数', '重量', '采购价/件', '采购价/%s' % weight_unit_text, '费用/元', '批次号', '操作员', '备注'])
        i = 1
        for flow in flows:
...
class BreakageHistoryFunc():
    ...
    @classmethod
    def batch_format_breakage_history_info(cls, flows, weight_unit_text, return_type="list"):
        '''
            批量处理报损记录的数据
        ...
        for flow in flows:
            ...
            data = dict(
                ...
                weight=check_float(doc_detail.get_weight(
                    weight_unit_text=weight_unit_text) / 100),
                cost_price_quantity=check_float(doc_detail.cost_price_quantity / 100),
                cost_price_weight=check_float(doc_detail.get_cost_price_weight(
                    weight_unit_text=weight_unit_text) / 100),
                total_price=check_float(doc_detail.total_price / 100),
                remark=doc_detail.remark,
                status=flow.status,
    ...
    def format_export_breakage_history_bodt(cls, flows, weight_unit_text):
        ...
        data_content.append(
            ['序号', '报损时间', '单据号', '件数', '重量', '报损价/件', '报损价/%s' % weight_unit_text, '费用/元', '操作员', '备注'])
        i = 1
        for flow in flows:
            record_info = data_dict.get(flow[0].id)
            data_content.append([
...
class SalesRecordExportFunc():
    ...
    def format_export_body(cls, session, shop_id, salerecord_list):
        ...
        data_content = []
        record_data_dict = OrderSalesRecordFunc.batch_format_order_record_info(session, salerecord_list)
        ...
        if distinguish_weight:
            ...
            for sale_record in salerecord_list:
                ...
                data_content.append([
                    ...
                    TimeFunc.time_to_str(sale_record.account_period_date, "date"),
                    "'" + record_info["num"],
                    record_info["group_name"],
                    record_info["only_goods_name"],
                    record_info["shop_supplier_name"],
                    record_info["fact_price"],
                    record_info["goods_unit"],
                    record_info["sales_num"],
                    record_info["gross_weight"],
                    record_info["tare_weight"],
                    record_info["commission_mul"],
                    record_info["commission"],
                    record_info["deposit"],
                    record_info["goods_total"],
                    record_info["deposit_total"],
                    record_info["commission_total"],
                    record_info["receipt_money"],
                    record_info["pay_type_text"],
                    record_info["salesman_name"],
                    record_info["sales_time"],
                    record_info["accountant_name"],
                    record_info["pay_time"],
                    record_info["debtor_name"],
                    record_info["order_num"],
                    record_info["remark"],
                    record_info["storage"],
    ...
    @classmethod
    def format_export_goods_sale_history(cls, session, shop_id, salerecord_list, export_item_list):
        """货品流水自定义导出指定字段"""
        # 店铺是否开启去皮功能
        config = ShopFunc.get_config(session, shop_id)
        distinguish_weight = config.distinguish_weight
    ...
    # 自定义获取流水指定条目（以及导出）
    @classmethod
    def format_export_body_custom(cls, session, shop_id, salerecord_list,
                                  export_item_list, data_sum, have_headline=False, filter_config=None):
        """
        ...
        data_content = []
        record_data_dict = OrderSalesRecordFunc.batch_format_order_record_info(session, salerecord_list, get_wximgurl=True)
        ...
        if not export_item_list:
            export_item_list = list(SaleExportFilterConfig.get_item_cn_name_dict(
        ...
        if have_headline:
            ...
            all_salesman = hire_link_base.filter(HireLink.active_salesman == 1).all()
            salesman_dict = {x.id: x.realname or x.nickname for x in all_salesman}

            # 获取收银员
            all_accountant = hire_link_base.filter(HireLink.active_accountant == 1).all()
            accountant_dict = {x.id: x.realname or x.nickname for x in all_accountant}

            # 获取付款方式
            pay_type_dict = models.GoodsSalesRecord().pay_type_dict

            # 获取商品分组
            all_group = session.query(GoodsGroup.id, GoodsGroup.name).filter_by(shop_id=shop_id).all()
            ...
            if text_contet_list:
                filter_config_content.append("货品类型：%s" % (",".join(text_contet_list)))
            ...
            if text_contet_list:
                filter_config_content.append("分组类型：%s" % (",".join(text_contet_list)))
            ...
            if text_contet_list:
                filter_config_content.append("销售类型：%s" % (",".join(text_contet_list)))
            ...
            if text_contet_list:
                filter_config_content.append("收银员：%s" % (",".join(text_contet_list)))
            ...
            if text_contet_list:
                filter_config_content.append("开票员：%s" % (",".join(text_contet_list)))
            ...
            if text_contet_list:
                filter_config_content.append("客户类型：%s" % (",".join(text_contet_list)))
            ...
            if text_contet_list:
                filter_config_content.append("支付方式：%s" % (",".join(text_contet_list)))

            filter_config_content.append(
                "票据金额：%s到%s" % (filter_config.get("receipt_min_money", 0), filter_config.get("receipt_max_money", "无")))
            ...
            if text_contet_list:
                filter_config_content.append("售卖单位：%s" % (",".join(text_contet_list)))

            filter_config_content.append(
                "收银时间：%s到%s" % (filter_config.get("cash_time_start", "无"), filter_config.get("cash_time_end", "无")))
            filter_config_content.append(
                "开票时间：%s到%s" % (filter_config.get("ticket_time_start", "无"), filter_config.get("ticket_time_end", "无")))
            ...
            for _key in export_item_list:
                ...
                if _key == 12:
                    ...
                    key_name = item_cn_name_dict.get(_key, "未知")
                headline_list.append(key_name)
                sum_list.append(data_sum.get(item_en_name_dict.get(_key, ""), ""))
            ...
            for sale_record in salerecord_list:
                ...
                for _key in export_item_list:
                    if _key == 1:
                        _content_list.append(TimeFunc.time_to_str(sale_record.order_time))
                        continue
                    if _key == 2:
                        _content_list.append(TimeFunc.time_to_str(sale_record.account_period_date))
                        continue
                    if not config.enable_deposit and _key in [13,15]:
                        continue
                    if not config.enable_commission and _key in [12, 16]:
            ...
            data_content = OrderSalesRecordFunc.batch_format_order_record_info(
                session, salerecord_list, return_type='list', get_wximgurl=True)
...
class OrderPrintBaseFunc():
    @classmethod
    def celery_order_print(cls, session, shop_id, order_id, user_id=0, receipt_config=None):
        if cls._check_salesman_settlement(order_id):
            ...
            need_print = ShopFunc.check_cashier_need_print(session, shop_id, receipt_config=receipt_config)
        if need_print:
            from handlers.celery_print import print_order
            print_order(order_id, user_id, receipt_config=receipt_config)

    @classmethod
    def order_print(cls, session, order, user_id=0, async=False, receipt_config=None, repeat_print=False):
        order_id = order.id
        shop_id = order.shop_id
        notice_txt = ""
        if not user_id:
        ...
        if async and cls._check_salesman_settlement(order_id):
            # 手机直接收款不取传过来的receipt_config，直接去数据库取
            need_print = ShopFunc.check_salesman_need_print(
                session, shop_id)
        else:
        ...
        if need_print:
            ...
            print_dict = {"wireless_print_type": wireless_print_type,
            ...
            if cls._check_order_records_single(session, order_id) or source == "salesman_async":
            ...
            if (receipt_config.cashier_accountant_receipt and
                receipt_config.merge_print_accountant == 1) or (
                    receipt_config.cashier_goods_receipt and
                    receipt_config.merge_print_goods == 1):
                cls._merge_sale_record_print(
                    session, order, receipt_config, print_dict,
                    repeat_print=repeat_print)
            # 原单打印模式
            notice_txt = cls._multi_sale_record_print(
                session, receipt_config, shop_id, user_id, print_dict,
                order=order, source=source, repeat_print=repeat_print)
            # 恢复打印模式设置
            if cls._check_order_records_single(session, order_id) or source == "salesman_async":
    ...
    def single_sale_record_print(
        ...
        record_counts = session.query(
            models.Order.id,
            func.count(
                models.GoodsSalesRecord.id)).join(
            models.GoodsSalesRecord,
    ...
    def _multi_sale_record_print(
        ...
        for multi_order_id in multi_sales_record_dict:
            ...
            sale_record_info_list, temp_order_info = GoodsSaleFunc.get_temp_sale_order_info(
                session, multi_record_list, multi_order,
                source=source, pay_order=order, repeat_print=repeat_print)
    ...
    @classmethod
    def _merge_sale_record_print(cls, session, order, receipt_config, print_dict, repeat_print=False):
        """ 收银台流水合并打印 """
        source = "accountant"
        wireless_print_type = int(print_dict["wireless_print_type"])
        wireless_print_num = print_dict["wireless_print_num"]
        ...
        sale_record_list = GoodsSaleFunc.get_sale_record_through_order(session, order.id)
        sale_record_info_list, temp_order_info = GoodsSaleFunc.get_temp_sale_order_info(
            session, sale_record_list, order,
            source=source, pay_order=order, is_merge=True, repeat_print=repeat_print)
    ...
    @classmethod
    def multi_sale_record_print_log(
            cls,
            session,
            shop_id,
            order_id,
        ...
        print_log = TempOrderPrintLog(shop_id=shop_id,
                                      order_id=order_id,
                                      user_id=user_id,
...
class OrderPrintContentFunc():
    ...
    @classmethod
    def order_print_body(
            cls, session, shop_id, order, source, local_print,
            if_pborder=False, pb_print=0, receipt_config=None, repeat_print=False):
        """
        ...
        if not if_pborder:
            ...
            # 如果订单内只有一张小票，强制走原单打印模式, 并在结束后恢复控制打印模式的状态
            if OrderPrintBaseFunc._check_order_records_single(session, order_id) or source == "salesman_async":
                origin_merge_print_accountant = receipt_config.merge_print_accountant
                origin_merge_print_goods = receipt_config.merge_print_goods
                receipt_config.merge_print_accountant = 0
                receipt_config.merge_print_goods = 0
            ...
            if not (source == "salesman_async" and multi_order_dict):
                ...
                if single_record_list:
                    ...
                    # 查询订单流水数，用于判断是否打印抹零信息
                    order_ids = [record.order_id for record in single_record_list]
            ...
            if print_body_list_content and wireless_print_type != 4:
                ...
                for print_body_content in print_body_list_content:
                    add_cut = "" if count_index == len(print_body_list_content) else wpp.cut()
        ...
        # 还款订单打印小票
            ...
            wpp = WirelessPrintFunc(wireless_print_type, shop=shop)
            pb_order_info = PbOrderPrintBaseFunc.get_pb_order_info(session, order)
            print_body=[]
...
class PbOrderPrintBaseFunc():
    ...
    def pb_order_print(cls, session, pb_order, user_id=0):
        ...
        wireless_print_type, wireless_print_num, wireless_print_key, _ = WirelessPrintFunc.get_hirelink_printer(session,
                                                                                                                shop_id,
                                                                                                                user_id,
                                                                                                                scene=2)
        ...
        content_body = wpp.get_payback_print_body(pb_order_info)
        if_print_success, error_txt = wpp.send_print_request(content_body, wireless_print_num)

        RecordPrintFunc.record_print_log(session, shop_id, pb_order.id, user_id, if_print_success, error_txt,
                                         wireless_print_num, print_type=1, source_type=1)

        if if_print_success:
            order_redis_key = "async_pb_order_print_mark:{}".format(pb_order.id)
            redis.set(order_redis_key, "1", 3600)
        return error_txt
    ...
    def get_pb_order_info(cls, session, pb_order):
        ...
        shop_name = session.query(models.Shop.shop_name).filter_by(id=pb_order.shop_id).scalar()
        ...
        pay_type_text = OrderBaseFunc.order_pay_type_text(pb_order.id, pb_order.pay_type, combine_pay_dict)
        if pb_order.pay_type == 12 and pb_order.bank_no:
            bank_name = OrderBaseFunc.get_order_bank_name(session, pb_order.id, source="pb_order")
            pay_type_text = "{0}({1})".format(pay_type_text, bank_name)

        pb_order_info = {}
        pb_order_info["shop_name"] = shop_name
        pb_order_info["money"] = NumFunc.check_float(pb_order.total_price / 100)
        pb_order_info["erase_money"] = NumFunc.check_float(pb_order.erase_money / 100)
        pb_order_info["fact_total_price"] = NumFunc.check_float(pb_order.fact_total_price / 100)
        pb_order_info["pay_type_text"] = pay_type_text
        pb_order_info["time"] = TimeFunc.splice_time(pb_order.pay_year, pb_order.pay_month, pb_order.pay_day,
                                                     pb_order.pay_time)
        ...
        pb_order_info["debtor_rest"] = NumFunc.check_float(shop_customer.debt_rest / 100)
        pb_order_info["accountant_name"] = accountant.realname or accountant.nickname
...
class WirelessPrintFunc():
    ...
    def __init__(self, print_type, _paper_width=32, shop=None):
        ...
        if print_type == "smposprint":
            ...
            _cashier_accountant_space = cashier_accountant_space_dict.get(print_type,(_paper_width - 8) * " ")
            _cashier_goods_space = cashier_goods_space_dict.get(print_type, (_paper_width - 20) * " ")
            _salesman_customer_space = salesman_customer_space_dict.get(print_type, (_paper_width - 16) * " ")
            _salesman_accountant_space = salesman_accountant_space_dict.get(print_type, (_paper_width - 8) * " ")
    ...
    def get_sale_record_print_body(
            self,
            sale_record_info,
            receipt_config,
            sales_num_total="",
        ...
        if source == "salesman":
            ...
            if receipt_config.salesman_customer_receipt:
                ...
                content_body += self.format_receipt_temp(
                    ...
                    receipt_config=receipt_config, fee_text=fee_text)
            ...
            if receipt_config.salesman_accountant_receipt:
                ...
                receip_temp = ReceiptTemplateFunc.get_receipt_temp_detail(
                    temp_name, shop_id)
                salesman_accountant_title = color_invert('[会计联]' + self._salesman_accountant_space)
                content_body += float_middle(
                    zoom_in(accounting_title, default_size))
                content_body += line_break(
                    zoom_in(
                        salesman_accountant_title,
                        default_size))
                content_body += self.format_receipt_temp(
                    sale_record_info, receip_temp, temp_name, receipt_config=receipt_config, fee_text=fee_text)
            ...
            if receipt_config.cashier_accountant_receipt and \
                ...
                content_body_accountant += self.format_receipt_temp(
                    sale_record_info, receip_temp, temp_name, receipt_config=receipt_config, fee_text=fee_text, index=index)
                content_body_list.append(content_body_accountant)
    ...
    def format_receipt_temp(
        ...
        commission_mul_total_text = "件数：%s%s件" % (
            commission_mul_total, commission_mul)
        ...
        if accountant_time:
            accountant_text = "收银信息：%s  %s" % (accountant_name, accountant_time)
        ...
        if temp_name == 'cashier_accountant_receipt_temp' and index:
            barcode = float_middle(barcode(sale_record_num, "accountant", index))
            ...
            if receipt_print:
                ...
                if name == "shop_name":
                    content_body += self.format_str_position_size(
                        shop_name, position, size, newline, last_newline=last_newline)
                ...
                # 条形码
                if name == "barcode":
                    if _type == "smposprint" and temp_name in ["salesman_goods_receipt_temp", "salesman_async_goods_receipt_temp"]:
                ...
                if name == "goods_name":
                    content_body += self.format_str_position_size(
                        goods_name_text, position, size, newline, last_newline=last_newline, action=name)
                    if shop_id in [105, 193, 224] and commodity_code:
                        content_body += self.format_str_position_size(
                            goods_code_text, position, size, newline, last_newline=last_newline, action="")
                # 件数
                if name == "piece":
                    if temp_name in ["salesman_customer_receipt_temp", "salesman_async_customer_receipt_temp"]:
                        content_body += self.format_str_position_size(
                        ...
                        content_body += self.format_str_position_size(
                            commission_mul_text, position, size, newline, last_newline=last_newline, action=name)
                # 流水号
                if name == "sale_number":
                    content_body += self.format_str_position_size(
                        stream_num_text, position, size, newline, last_newline=last_newline)

                # 批次号
                if name == "batch_num":
                    content_body += self.format_str_position_size(
                        batch_num_text, position, size, newline, last_newline=last_newline)

                # 重量
                if name == "weight" and sales_num_text:
                    if temp_name in ["salesman_customer_receipt_temp","salesman_async_customer_receipt_temp"]:
                        content_body += self.format_str_position_size(
                            sales_num_total_text, position, size, newline, last_newline=last_newline)
                    else:
                        content_body += self.format_str_position_size(
                            sales_num_text, position, size, newline, last_newline=last_newline)
                # 单价
                if name == "price":
                    content_body += self.format_str_position_size(
                        price_text, position, size, newline, last_newline=last_newline)
                # 行费
                if name == "commission" and commission_text:
                    content_body += self.format_str_position_size(
                        commission_text, position, size, newline, last_newline=last_newline)
                # 押金
                if name == "deposit" and deposit_avg_text:
                    content_body += self.format_str_position_size(
                        deposit_avg_text, position, size, newline, last_newline=last_newline)
                # 货品小计
                if name == "total_goods_sale":
                    content_body += self.format_str_position_size(
                        goods_sum_text, position, size, newline, last_newline=last_newline)
                # 行费小计
                if name == "total_commission" and commission_sum_text:
                    content_body += self.format_str_position_size(
                        commission_sum_text, position, size, newline, last_newline=last_newline)
                # 押金小计
                if name == "total_deposit" and deposit_sum_text:
                    content_body += self.format_str_position_size(
                        deposit_sum_text, position, size, newline, last_newline=last_newline)
                # 票据金额
                if name == "receipt_money":
                    if last_newline:
                        content_body += bottom_line_break()
                    content_body += self.format_str_position_size(
                        receipt_money_text, position, size, newline, last_newline=last_newline)
                    # 抹零信息和票据金额放在一起
                    if erase_money_text:
                        content_body += self.format_str_position_size(
                            erase_money_text, position, size, newline, last_newline=last_newline)
                        content_body += self.format_str_position_size(
                            actual_money_text, position, size, newline, last_newline=last_newline)
                # 开票信息
                if name == "salesman_info":
                    content_body += self.format_str_position_size(
                        salesman_text, position, size, newline, last_newline=last_newline)
                # 收银信息
                if name == "accountant_info":
                    content_body += self.format_str_position_size(
                        accountant_text, position, size, newline, last_newline=last_newline)
                # 支付方式
                if name == "pay_info":
                    # 组合支付方式
                    if combine_pay_type_text:
                        content_body += self.format_str_position_size(
                            combine_pay_type_text, position, size, newline, last_newline=last_newline)
                    else:
                        content_body += self.format_str_position_size(
                            pay_text, position, size, newline, last_newline=last_newline)
                # 货品识别码
                if name == "code":
                    content_body += line_break('')
                    content_body += code
                ...
                if name == "shop_customer" and shop_customer_text:
                    content_body += self.format_str_position_size(
                        shop_customer_text, position, size, newline, last_newline=last_newline)
                # 客户签名
                ...
                if name == "receipt_text" and receipt_text:
                    content_body += self.format_str_position_size(
                        receipt_text, position, size, newline, last_newline=last_newline)
                # 附加图片
                ...
                if name == "remark" and remark_text:
                    content_body += self.format_str_position_size(
                        remark_text, position, size, newline, last_newline=last_newline)
    ...
    def format_str_position_size(
        ...
        if action in [
            "goods_name",
            "piece"] and position == "center" and "：" in text:
            text = text.split("：")[1]
        if size == 2:
            text = zoom_in(text, 2)
        elif size == 3:
        ...
        if newline:
            if last_newline:
                ...
                if _type == '80localprint':
                    ...
                    if last_string:
                        content_len = len(origin_text) + CharFunc.get_contain_chinese_number(origin_text)
                        last_content_len = len(last_string) + CharFunc.get_contain_chinese_number(last_string)
                        content_box_len = self._paper_width - last_content_len - 28
                        if content_box_len <= 4:
                            text = "  " + text
                        text = line_break(fill_box(text, content_box_len, content_len, float_dir="right"))
                    else:
                        text = line_break(text)
        else:
            if _type in ["feprint", "sgprint"]:
                if size == 1:
                    ...
                    content_len = (len(origin_text) + CharFunc.get_contain_chinese_number(origin_text)) * 2
    ...
    def get_multi_sale_record_print_body(
            self,
            sale_record_info_list,
            temp_order_info,
            receipt_config,
        ...
        if source == "salesman":
            if receipt_config.salesman_customer_receipt:
                ...
                content_body_data += line_break(
                    zoom_in('[客户联]' + self._salesman_customer_space + '提货撕毁', default_size))
            ...
            if receipt_config.salesman_accountant_receipt:
                ...
                content_body_data = float_middle(
                    zoom_in(accounting_title, default_size))
                content_body_data += line_break(zoom_in(color_invert(
                    '[会计联]' + self._salesman_accountant_space), default_size))
                content_body_data += content_main_body
                content_body += content_body_data
                content_body_list.append(content_body_data)
                if receipt_config.salesman_goods_receipt:
            ...
            if receipt_config.cashier_accountant_receipt and \
                ...
                content_body_data += line_break(zoom_in(color_invert(
                    '[会计联]' + self._cashier_accountant_space), default_size))
                content_body_data += content_main_body
                content_body += content_body_data
                content_body_list.append(content_body_data)
            ...
            if receipt_config.cashier_goods_receipt and \
                ...
                content_body_data = float_middle(
                    zoom_in(accounting_title, default_size))
                content_body_data += line_break(zoom_in('[客户提货联]' + self._cashier_goods_space + '提货撕毁', default_size))
                content_body_data += content_main_body
                content_body += content_body_data
                content_body_list.append(content_body_data)
    ...
    def get_merge_sale_record_print_body(
            self,
            sale_record_info_list,
            temp_order_info,
            receipt_config,
    ...
    def format_multi_receipt_temp(
        ...
        if temp_name == 'multi_cashier_accountant_receipt_temp' and index:
            temp_barcode = float_middle(barcode(multi_order_num, "accountant", index))
        elif temp_name == 'multi_cashier_goods_receipt_temp' and index:
            temp_barcode = float_middle(barcode(multi_order_num, "goods", index))
        else:
        ...
        for sale_record_info in sale_record_info_list:
            ...
            # 件数显示处理
            if temp_name in ["multi_salesman_customer_receipt_temp", "multi_salesman_async_customer_receipt_temp"]:
                if commission_mul_total:
                    commission_mul_text = "%s%s件" % (
                        commission_mul_total, commission_mul)
                if sales_num_total:
                    sales_num_text = "重量：%s%s%s" % (
                        sales_num_total, sales_num, goods_unit_text)
                ...
                if receipt_print:
                    ...
                    if goods_print_text:
                        goods_content_body += self.format_str_position_size(
                            goods_print_text, position, size, newline,
                            last_newline=goods_last_newline, last_string=goods_last_string)
                        if name == "goods_name" and shop_id in [105, 193, 224] and commodity_code:
                            goods_content_body += self.format_str_position_size(
                                goods_code_text, position, size, newline,
                                last_newline=goods_last_newline, action="", last_string=goods_last_string)
        ...
        for receipt_dict in receip_temp:
            ...
            if name == "goods_list":
                ...
                if receipt_print:
                    ...
                    if main_print_text:
                        content_body += self.format_str_position_size(
                            main_print_text, position, size, newline, last_newline=last_newline)
    ...
    def get_work_record_print_body(
            self,
            config,
            record_data,
            shop_name="",
            accountant_name="",
            for_qingdao_qianyuan=False, total_cent=0, count=0):
        """交接班记录小票
        ...
        print_time = "打印时间：%s" % (TimeFunc.time_to_str(datetime.datetime.now()))
        work_time = "收银时长：%s" % (record_data["work_time"])
        order_total_money = "订单收款：%s元" % (record_data["order_total_money"])
        order_cash_money = "现金：%s元  %s笔" % (record_data["order_cash_money"], record_data["order_cash_count"])
        order_bank_money = "银行卡：%s元  %s笔" % (record_data["order_bank_money"], record_data["order_bank_count"])
        order_cheque_money = "支票：%s元  %s笔" % (record_data["order_cheque_money"], record_data["order_cheque_count"])
        order_wx_money = "微信：%s元  %s笔" % (record_data["order_wx_money"], record_data["order_wx_count"])
        order_ali_money = "支付宝：%s元  %s笔" % (record_data["order_ali_money"], record_data["order_ali_count"])

        pb_total_money = "还款：%s元" % (record_data["pb_total_money"])
        pb_cash_money = "现金：%s元  %s笔" % (record_data["pb_cash_money"], record_data["pb_cash_count"])
        pb_bank_money = "银行卡：%s元  %s笔" % (record_data["pb_bank_money"], record_data["pb_bank_count"])
        pb_cheque_money = "支票：%s元  %s笔" % (record_data["pb_cheque_money"], record_data["pb_cheque_count"])
        pb_wx_money = "微信：%s元  %s笔" % (record_data["pb_wx_money"], record_data["pb_wx_count"])
        pb_ali_money = "支付宝：%s元  %s笔" % (record_data["pb_ali_money"], record_data["pb_ali_count"])

        refund_total_money = "退单：%s元" % (record_data["refund_total_money"])
        refund_cash_money = "现金：%s元  %s笔" % (record_data["refund_cash_money"], record_data["refund_cash_count"])
        refund_bank_money = "银行卡：%s元  %s笔" % (record_data['refund_bank_money'], record_data['refund_bank_count'])
        refund_wx_money = "微信：%s元  %s笔" % (
            record_data["refund_wx_money"], record_data["refund_wx_count"])
        refund_ali_money = "支付宝：%s元  %s笔" % (
            record_data["refund_ali_money"], record_data["refund_ali_count"])
        refund_cheque_money = "支票：%s元  %s笔" % (
            record_data["refund_cheque_money"], record_data["refund_cheque_count"])

        erase_total_money = "抹零：%s元" % (record_data["erase_money"])
        erase_cash_money = "现金：%s元  %s笔" % (record_data["erase_cash_money"], record_data["erase_cash_count"])
        erase_bank_money = "银行卡：%s元  %s笔" % (record_data["erase_bank_money"], record_data["erase_bank_count"])
        erase_cheque_money = "支票：%s元  %s笔" % (record_data["erase_cheque_money"], record_data["erase_cheque_count"])
        erase_wx_money = "微信：%s元  %s笔" % (record_data["erase_wx_money"], record_data["erase_wx_count"])
        erase_ali_money = "支付宝：%s元  %s笔" % (record_data["erase_ali_money"], record_data["erase_ali_count"])
        erase_combine_money = "组合支付：%s元  %s笔" % (record_data["erase_combine_money"], record_data["erase_combine_count"])

        payout_total_money = "收银台支出：%s元" % (record_data["payout_total_money"])
        income_total_money = "额外收入：%s元" % (record_data["income_total_money"])

        order_fee_money = "手续费：%s元 %s笔" % (record_data["order_fee_money"], record_data["order_fee_count"])
        order_fee_cash_money = "现金：%s元  %s笔" % (record_data["order_fee_cash_money"], record_data["order_fee_cash_count"])
        order_fee_bank_money = "银行卡：%s元  %s笔" % (record_data["order_fee_bank_money"], record_data["order_fee_bank_count"])
        order_fee_cheque_money = "支票：%s元  %s笔" % (record_data["order_fee_cheque_money"], record_data["order_fee_cheque_count"])
        order_fee_wx_money = "微信：%s元  %s笔" % (record_data["order_fee_wx_money"], record_data["order_fee_wx_count"])
        order_fee_ali_money = "支付宝：%s元  %s笔" % (record_data["order_fee_ali_money"], record_data["order_fee_ali_count"])

        total_money = "实收资金：%s元" % (record_data["total_money"])
        total_cash_money = "现金：%s元  %s笔" % (record_data["total_cash_money"], record_data["total_cash_count"])
        total_bank_money = "银行卡：%s元  %s笔" % (record_data["total_bank_money"], record_data["total_bank_count"])
        total_cheque_money = "支票：%s元  %s笔" % (record_data["total_cheque_money"], record_data["total_cheque_count"])
        total_wx_money = "微信：%s元  %s笔" % (record_data["total_wx_money"], record_data["total_wx_count"])
        total_ali_money = "支付宝：%s元  %s笔" % (record_data["total_ali_money"], record_data["total_ali_count"])
        total_deposit_money = "押金收支：%s元" % (record_data["deposit_spend"])

        order_credit_money = "赊账：%s元  %s笔" % (
            record_data["order_credit_money"], record_data["order_credit_count"])
        refund_credit_money = "赊账退还：%s元  %s笔" % (
            record_data["refund_credit_money"], record_data["refund_credit_count"])
        ...
        if for_qingdao_qianyuan:
            content_body += line_break(zoom_in("供货商预支：{}元 {}笔".format(total_cent, count), 2))
        # =============================================
        content_body += line_break("")
    ...
    def get_borrowing_print_body(self, record_data):
        ...
        accountant_content_body = line_break(zoom_in(color_invert(
                '[会计联]' + self._cashier_accountant_space), self.default_size))
    ...
    def get_allocate_print_body(self, record_data, goods_list, weight_unit_text="斤"):
        """库存调拨记录打印"""
        _paper_width = self._paper_width
        line_break = self.line_break
        float_middle = self.float_middle
...
class ReceiptTemplateFunc():
    ...
    def get_receipt_key_list(cls):
        key_list = ["salesman_customer_receipt_temp",
    ...
    def get_default_receipt_temp(cls, temp_key):
        ...
        '''
        if temp_key in ["salesman_customer_receipt_temp", "salesman_async_customer_receipt_temp"]:
            default_temp = [
                {"name": "shop_name", "position": "center", "size": "2", "print": 1, "newline": 1},
                {"name": "barcode", "position": "center", "size": "1", "print": 1, "newline": 1},
                {"name": "goods_name", "position": "center", "size": "3", "print": 1, "newline": 1},
                {"name": "piece", "position": "center", "size": "3", "print": 1, "newline": 1},
                {"name": "sale_number", "position": "left", "size": "2", "print": 1, "newline": 1},
                {"name": "batch_num", "position": "left", "size": "2", "print": 0, "newline": 1},
                {"name": "weight", "position": "left", "size": "2", "print": 1, "newline": 1},
                {"name": "price", "position": "left", "size": "2", "print": 1, "newline": 1},
                {"name": "commission", "position": "left", "size": "2", "print": 1, "newline": 1},
                {"name": "deposit", "position": "left", "size": "2", "print": 1, "newline": 1},
                {"name": "total_goods_sale", "position": "left", "size": "2", "print": 1, "newline": 1},
                {"name": "total_commission", "position": "left", "size": "2", "print": 1, "newline": 1},
                {"name": "total_deposit", "position": "left", "size": "2", "print": 1, "newline": 1},
                {"name": "receipt_money", "position": "left", "size": "2", "print": 1, "newline": 1},
                {"name": "code", "position": "center", "size": "1", "print": 0, "newline": 1},
                {"name": "shop_customer", "position": "center", "size": "1", "print": 1, "newline": 1},
                {"name": "salesman_info", "position": "center", "size": "1", "print": 1, "newline": 1},
                {"name": "accountant_info", "position": "center", "size": "1", "print": 0, "newline": 1},
                {"name": "pay_info", "position": "center", "size": "2", "print": 0, "newline": 1},
                {"name": "receipt_text", "position": "center", "size": "1", "print": 1, "newline": 1},
                {"name": "receipt_img", "position": "center", "size": "1", "print": 0, "newline": 1},
                {"name": "remark", "position": "left", "size": "1", "print": 1, "newline": 1}
            ]
        elif temp_key in ["salesman_accountant_receipt_temp", "salesman_async_accountant_receipt_temp"]:
            default_temp = [
                {"name": "shop_name", "position": "center", "size": "2", "print": 1, "newline": 1},
                {"name": "goods_name", "position": "center", "size": "3", "print": 1, "newline": 1},
                {"name": "piece", "position": "center", "size": "3", "print": 1, "newline": 1},
                ...
                {"name": "receipt_money", "position": "left", "size": "2", "print": 1, "newline": 1},
                {"name": "code", "position": "center", "size": "1", "print": 0, "newline": 1},
                {"name": "barcode", "position": "center", "size": "1", "print": 0, "newline": 1},
                {"name": "deposit", "position": "left", "size": "2", "print": 0, "newline": 1},
                {"name": "commission", "position": "left", "size": "2", "print": 0, "newline": 1},
                {"name": "shop_customer", "position": "center", "size": "1", "print": 1, "newline": 1},
                {"name": "salesman_info", "position": "center", "size": "1", "print": 1, "newline": 1},
                {"name": "accountant_info", "position": "center", "size": "1", "print": 0, "newline": 1},
                {"name": "pay_info", "position": "center", "size": "2", "print": 0, "newline": 1},
        ...
        elif temp_key in ["salesman_goods_receipt_temp", "salesman_async_goods_receipt_temp"]:
            default_temp = [
                {"name": "shop_name", "position": "center", "size": "2", "print": 1, "newline": 1},
                {"name": "code", "position": "center", "size": "1", "print": 1, "newline": 1},
                {"name": "goods_name", "position": "center", "size": "2", "print": 1, "newline": 1},
                {"name": "piece", "position": "center", "size": "2", "print": 1, "newline": 1},
                {"name": "weight", "position": "center", "size": "2", "print": 1, "newline": 1},
                {"name": "receipt_money", "position": "center", "size": "2", "print": 1, "newline": 1},
                {"name": "sale_number", "position": "center", "size": "2", "print": 0, "newline": 1},
                {"name": "batch_num", "position": "center", "size": "2", "print": 0, "newline": 1},
                {"name": "price", "position": "left", "size": "2", "print": 0, "newline": 1},
                {"name": "total_goods_sale", "position": "left", "size": "2", "print": 0, "newline": 1},
                {"name": "total_commission", "position": "left", "size": "2", "print": 0, "newline": 1},
                {"name": "total_deposit", "position": "left", "size": "2", "print": 0, "newline": 1},
                {"name": "barcode", "position": "center", "size": "1", "print": 0, "newline": 1},
                {"name": "deposit", "position": "left", "size": "2", "print": 0, "newline": 1},
                {"name": "commission", "position": "left", "size": "2", "print": 0, "newline": 1},
                {"name": "shop_customer", "position": "center", "size": "1", "print": 1, "newline": 1},
            ...
            default_temp = [
                ...
                {"name": "commission", "position": "left", "size": "2", "print": 0, "newline": 1},
                {"name": "salesman_info", "position": "center", "size": "1", "print": 1, "newline": 1},
                {"name": "accountant_info", "position": "center", "size": "1", "print": 1, "newline": 1},
                {"name": "pay_info", "position": "center", "size": "2", "print": 1, "newline": 1},
                {"name": "shop_customer", "position": "left", "size": "1", "print": 0, "newline": 1},
                {"name": "signature_img", "position": "center", "size": "1", "print": 1, "newline": 1},
                {"name": "receipt_text", "position": "center", "size": "1", "print": 1, "newline": 1},
                {"name": "receipt_img", "position": "center", "size": "1", "print": 0, "newline": 1},
                {"name": "remark", "position": "left", "size": "1", "print": 1, "newline": 1}
            ]
        ...
        elif temp_key in ["multi_salesman_customer_receipt_temp",
                          "multi_salesman_accountant_receipt_temp",
                          "multi_salesman_goods_receipt_temp",
                          "multi_salesman_async_customer_receipt_temp",
                          "multi_salesman_async_accountant_receipt_temp",
            ...
            default_temp = [
                {"name": "shop_name", "position": "center", "size": "2", "print": 1, "newline": 1},
                {"name": "barcode", "position": "center", "size": "1", "print": 1, "newline": 1},
                {"name": "code", "position": "center", "size": "3", "print": 0, "newline": 1},
                {"name": "order_num", "position": "left", "size": "2", "print": 1, "newline": 1},
                {"name": "category", "position": "left", "size": "2", "print": 0, "newline": 0},
                {"name": "total_commission_mul", "position": "left", "size": "2", "print": 1, "newline": 1},
                {"name": "total_money", "position": "left", "size": "2", "print": 1, "newline": 1},
                {"name": "shop_customer", "position": "left", "size": "2", "print": 1, "newline": 1},
                {"name": "goods_list", "style": [
                    {"name": "sale_number", "position": "left", "size": "1", "print": 1, "newline": 1},
                    {"name": "batch_num", "position": "left", "size": "1", "print": 0, "newline": 1},
                    {"name": "goods_name", "position": "left", "size": "2", "print": 1, "newline": 1},
                    {"name": "piece", "position": "left", "size": "2", "print": 1, "newline": 0},
                    {"name": "receipt_money", "position": "left", "size": "2", "print": 1, "newline": 1},
                    {"name": "price", "position": "left", "size": "1", "print": 1, "newline": 0},
                    {"name": "total_goods_sale", "position": "left", "size": "1", "print": 1, "newline": 1},
                    {"name": "commission", "position": "left", "size": "1", "print": 1, "newline": 0},
                    {"name": "total_commission", "position": "left", "size": "1", "print": 1, "newline": 1},
                    {"name": "deposit", "position": "left", "size": "1", "print": 1, "newline": 0},
                    {"name": "total_deposit", "position": "left", "size": "1", "print": 1, "newline": 1},
                    {"name": "weight", "position": "left", "size": "1", "print": 1, "newline": 1}
                ]
                 },
                {"name": "salesman_info", "position": "center", "size": "1", "print": 1, "newline": 1},
                {"name": "accountant_info", "position": "center", "size": "1", "print": 0, "newline": 1},
                {"name": "pay_info", "position": "center", "size": "1", "print": 0, "newline": 1},
                {"name": "erase_text", "position": "center", "size": "1", "print": 1, "newline": 1},
                {"name": "receipt_text", "position": "center", "size": "1", "print": 1, "newline": 1},
                {"name": "remark", "position": "left", "size": "1", "print": 1, "newline": 1},
            ]
        elif temp_key in ["multi_cashier_accountant_receipt_temp",
                          "multi_cashier_goods_receipt_temp"]:
            default_temp = [
                {"name": "shop_name", "position": "center", "size": "2", "print": 1, "newline": 1},
                {"name": "barcode", "position": "center", "size": "1", "print": 1, "newline": 1},
                ...
                {"name": "salesman_info", "position": "center", "size": "1", "print": 1, "newline": 1},
                {"name": "accountant_info", "position": "center", "size": "1", "print": 1, "newline": 1},
                {"name": "pay_info", "position": "center", "size": "1", "print": 1, "newline": 1},
                {"name": "erase_text", "position": "center", "size": "1", "print": 1, "newline": 1},
                {"name": "receipt_text", "position": "center", "size": "1", "print": 1, "newline": 1},
                {"name": "remark", "position": "left", "size": "1", "print": 1, "newline": 1},
...
class WirelessPrintInfoFunc():
    ...
    def get_salersman_printer_info(cls, session, user_id, shop_id):
        printer_id = session.query(models.HireLink.printer_id).filter_by(account_id=user_id, shop_id=shop_id).scalar()
    ...
    def get_accountant_printer_info(cls, session, user_id, shop_id):
        printer_id = session.query(models.HireLink.accountant_printer_id).filter_by(account_id=user_id,
    ...
    def get_printer_info(cls, session, printer_id):
        ...
        if printer_id:
            printer = session.query(models.Printer).filter_by(id=printer_id).first()
...
class AccountFunc():
    ...
    @classmethod
    def get_account_wx_head_imgurl_through_id_list(cls, session, account_id_list):
        if account_id_list:
            Accountinfo = models.Accountinfo
            operator_list = session.query(Accountinfo) \
            .filter(Accountinfo.id.in_(account_id_list)).all()
            account_dict = {x.id: x.head_imgurl_small if x.wx_unionid else "" for x in operator_list}
        else:
            account_dict = {}
        return account_dict
    ...
    def get_account_name_wximg_through_id_list(cls, session, account_id_list):
        if account_id_list:
            ...
            wximg_dict = {x.id: models.AddImgDomain.add_domain_headimgsmall(x.headimgurl) if x.wx_unionid else ""
                          for x in account}
    ...
    # 根据ID列表获取用户是否是店铺的货主
    @classmethod
    def get_account_goods_owner_role_through_id_list(cls, session, shop_id, account_id_list):
        result = {}
        if account_id_list:
            hirelinks = session.query(
                models.HireLink.active_goods_owner, models.Accountinfo).join(
                models.Accountinfo,
                models.Accountinfo.id == models.HireLink.account_id).filter(
            ...
            for active_goods_owner, account_info in hirelinks:
                ...
                result[account_info.id] = dict(
                    ...
                    supplier_headimg=account_info.head_imgurl if active_goods_owner == 1 else "",
    ...
    def get_account_through_passport_id(cls, session, passport_id):
        Accountinfo = models.Accountinfo
        account = session.query(Accountinfo).filter_by(passport_id=passport_id).one_or_none()
        return account
    ...
    def get_account_through_uuid(cls, session, account_uuid):
        Accountinfo = models.Accountinfo
        account = session.query(Accountinfo).filter_by(uuid=account_uuid).first()
        return account
    ...
    def get_account_name(cls, session, account_id):
        Accountinfo = models.Accountinfo
        account = session.query(Accountinfo.realname, Accountinfo.nickname).filter_by(id=account_id).first()
        if account:
    ...
    def get_account_id_through_phone(cls, session, phone):
        Accountinfo = models.Accountinfo
        _id = session.query(Accountinfo.id).filter_by(phone=phone).scalar() or 0
        return _id

    # 通过手机号获取用户id,name字典
    @classmethod
    def get_account_id_name_dict_through_phone(cls, session, phone, many=False):
        Accountinfo = models.Accountinfo
        id_name_dict = {}
        if many:
            ...
            if result:
                id_name_dict[result.id] = result.realname or result.nickname or "-"
        return id_name_dict
    ...
    def parse_account_info(cls, account, get_wximgurl=False):
        ...
        if get_wximgurl:
            result["wximgurl"] = account.head_imgurl_small if account.wx_unionid else ""
        return result
...
class RefundOrderFunc():
    ...
    def get_refund_record_time(cls, refund_order):
        time = str(refund_order.create_year) + '-' + str(refund_order.create_month).zfill(2) + '-' + str(
            refund_order.create_day).zfill(2) + ' ' + str(refund_order.create_time)
        return time
...
class SupplierFunc():
    ...
    # 获取供货商的商品ID列表
    @classmethod
    def get_supplier_goods_id_through_id(cls, session, shop_id, shop_supplier_id_list):
        Goods = models.Goods
    ...
    # 根据供货商id列表获取供货商
    @classmethod
    def get_shop_supplier_through_id_list(cls, session, shop_id, shop_supplier_id_list):
        ShopSupplier = models.ShopSupplier
    ...
    # 获取供货商基本信息
    @classmethod
    def get_shop_supplier_base_info(cls, session, shop_id, shop_supplier_id_list):
        info_dict = {}
        ...
        for supplier in shop_suppliers:
            ...
            account_info = AccountFunc.parse_account_info(account, get_wximgurl=True) if account else {}
            info_dict[supplier.id] = {
                "supplier_id": supplier.id,
                "name": supplier.realname,
                "phone": supplier.phone or "",
                "headimgurl": account_info.get("supplier_headimg", ""),
                "wximgurl": account_info.get("wximgurl", ""),
                "active_goods_owner": 1 if supplier.active_goods_owner == 1 else 0,
    ...
    def get_supplier_info(cls, session, shop_id, data_sales_ret):
        ...
        if supplier_id_list:
            supplier_list = cls.get_shop_supplier_through_id_list(session, shop_id, supplier_id_list)
    ...
    # 根据商品名称查找供应商
    @classmethod
    def get_supplier_through_goods_name(cls, session, shop_id,
                                        name_list, filter_goods_list=None):
        """
        ...
        # 获取商品供应商信息
        shop_suppliers = cls.get_shop_supplier_through_id_list(session, shop_id, supplier_id_list)
        ...
        for goods in goods_list:
            ...
            data_dict["goods_storage_num"] = check_float(goods.storage_num / 100)
            ...
            data_dict["supply_type"] = GoodsFunc.get_supply_type(goods.supply_type)
            data_list_dict[goods_name].append(data_dict)
        return data_list_dict

    # 根据商品ID查找供应商
    @classmethod
    def get_supplier_info_through_goods_id(cls, session, shop_id, goods_id, check_supply_type=False):
        # 查找供应商
        shop_supplier_id = session.query(models.Goods.shop_supplier_id) \
            .filter_by(shop_id=shop_id, id=goods_id)
        if check_supply_type:
            shop_supplier_id = shop_supplier_id.filter(models.Goods.supply_type > 0)
        shop_supplier_id = shop_supplier_id.scalar() or 0
        data_dict = {}
        if shop_supplier_id:
            # 获取供应商信息
            shop_supplier = cls.get_shop_supplier_through_id(session, shop_id, shop_supplier_id)
            supplier_account = AccountFunc.get_account_through_id(
                session, shop_supplier.account_id)
            data_dict = dict(
                supplier_id=shop_supplier.id,
                supplier_name=shop_supplier.realname,
                active_goods_owner=1 if shop_supplier.active_goods_owner == 1 else 0,
                supplier_headimg=supplier_account.head_imgurl if supplier_account else "",
            )
        return data_dict

    # 根据商品ID查找供应商
    @classmethod
    def get_supplier_info_through_goods_id_list(cls, session, shop_id, goods_id_list, check_supply_type=False):
        # 查找供应商
        supplier_id_list = session.query(models.Goods.shop_supplier_id) \
            .filter_by(shop_id=shop_id)\
            .filter(models.Goods.id.in_(goods_id_list))
        if check_supply_type:
            supplier_id_list = supplier_id_list.filter(models.Goods.supply_type > 0)
    ...
    @classmethod
    def check_supplier_unique_personal(
            cls, session, shop_id, username, phone, db_supplier=None):
        ...
        if c_suppliers:
            if db_supplier and len(c_suppliers) == 1 and c_suppliers[0].id == db_supplier.id:
                return True, ""
            return False, "已经存在名称相同的企业供应商"
        return True, ""

    @classmethod
    def check_supplier_unique_company(
            cls, session, shop_id, company_short_name, company_full_name, db_supplier=None):
        ...
        if company_short_name:
            s_suppliers = session.query(models.ShopSupplier).filter_by(
                shop_id=shop_id, status=1).filter(
                or_(models.ShopSupplier.realname == company_short_name,
                    models.ShopSupplier.company_short_name == company_short_name,
                    models.ShopSupplier.company_full_name == company_short_name)).all()
        if company_full_name:
            c_suppliers = session.query(models.ShopSupplier).filter_by(
                shop_id=shop_id, status=1).filter(
                or_(models.ShopSupplier.realname == company_full_name,
                    models.ShopSupplier.company_short_name == company_full_name,
                    models.ShopSupplier.company_full_name == company_full_name)).all()
        if s_suppliers or c_suppliers:
            if db_supplier:
                ...
                # 自己不能算
                if len(supplier_id_set) == 1 and supplier_id_set == {db_supplier.id}:
                    return True, ""
    ...
    # 获取供货商的商品数量
    @classmethod
    def get_multi_supplier_goods_id_list(cls, session, shop_id, supplier_id_list):
        """ 通过供应商ID取出供应商对应的所有货品ID
    ...
    # 检查货品是否属于供应商
    @classmethod
    def check_supplier_goods(cls, session, shop_id, shop_supplier_id, goods_id):
    ...
    def get_multi_goods_bottom_price(cls, session, supplier_goods_id_list):
        bottom_price = session.query(models.BottomPriceGoodsRate).filter(
            models.BottomPriceGoodsRate.goods_id.in_(supplier_goods_id_list)).all()
        result = {}
    ...
    # 添加供应商
    @classmethod
    def handle_add_supplier(self, session, shop_id, username, phone, edit_user_id, source="salesman"):
        ShopSupplier = models.ShopSupplier
        shop_supplier = ShopSupplier(shop_id=shop_id, realname=username, phone=phone)
...
class SupplierGoodsStockin():
    ...
    @classmethod
    def format_export_supplier_goods_stockin(cls, data_list, total_data_sum, weight_unit_text):
        data_content = []
        data_content.append(
            ['序号','类型', '货品', '入库量/件', '入库量/%s'%weight_unit_text, '采购均价',  '总费用/元', '已付款', '赊账待付', '已结算', '应付款'])
        i=1
        for data in data_list:
            data_content.append([
                i,
            ...
            i+=1
        data_content.append([
            '累计', '', '', total_data_sum["quantity_sum"], total_data_sum["weight_sum"],
            '', total_data_sum["total_price_sum"], total_data_sum["paid_cent_sum"],
            total_data_sum["credit_cent_sum"], total_data_sum["payback_cent_sum"],
...
class SupplierSettlementFunc():
    ...
    def get_clearing_data(cls, session, shop_id, goods_id):
        ...
        clearing_money = check_float(clearing_money / 100) if clearing_money else 0
    ...
    def get_unclearing_money(cls, session, shop_id, goods_id_list):
        ...
        unclearing_money = check_float((unclearing_money_proxy + unclearing_money_self_sell) / 100)
        return unclearing_money

    @classmethod
    def get_unclearing_money_dict(cls, session, shop_id, agents_goods_id_list, return_type="",
                                  clearing_record_ids=None, end_date_time=None):
        """ 获取货品的未结算总金额和未结算的件数,
        ...
        if clearing_record_ids is None:
            proxy_filter_params.append(GoodsSalesRecord.clearing_record_id == 0)
        elif clearing_record_ids:
            proxy_filter_params.append(not_(GoodsSalesRecord.clearing_record_id.in_(clearing_record_ids)))
            proxy_filter_params.append(GoodsSalesRecord.create_time < end_date_time)
        ...
        goods_unclearing_dict = {g[0]: {"money": g[1], "commission_mul": g[2]}
        ...
        self_filter_params = [
            StockInDocs.status == 1,
            StockInDocsDetail.pay_type == 5,
            StockInDocsDetail.status == 1,
            StockInDocsDetail.goods_id.in_(self_sell_goods_id_set)
        ]
        if clearing_record_ids is None:
            self_filter_params.append(StockInDocsDetail.clearing_record_id == 0)
        elif clearing_record_ids:
            self_filter_params.append(not_(StockInDocsDetail.clearing_record_id.in_(clearing_record_ids)))
        ...
        goods_unpay_dict = {g[0]: {"money": g.total_price, "commission_mul": g.quantity}
    ...
    def get_clear_detail(cls, session, clearing, shop_name=None):
        ...
        data_dict["clear_commission_mul"] = check_float(clearing.clear_commission_mul/100)
        data_dict["clear_sales_num"] = check_float(clearing.get_clear_sales_num(
            weight_unit_text=weight_unit_text) / 100)
        data_dict["clear_sales_money"] = check_float(clearing.clear_sales_cent/100)
        data_dict["borrow_money"] = check_float((clearing.borrow_cent+clearing.other_value_cent)/100)
        data_dict["procedure_money"] = check_float(clearing.procedure_cent/100)
        ...
        data_dict["total_payout"] = check_float(
            data_dict["borrow_money"] + data_dict["procedure_money"])
    ...
    @classmethod
    def list_clearing_sum_each_day(cls, session, sales_record_id_list, borrow_id_list, data_type):
    ...
    def _parse_borrow(cls, borrow_each_day):
        ...
        for borrow in borrow_each_day:
            result[borrow[0]] = {"borrow": check_float(borrow.borrow_cent / 100)}
        return result
...
class GoodsRateFunc():
    ...
    def get_rate_type_text(cls, goods_rate, fee_text="", weight_unit_text="斤"):
        ...
        if goods_rate.piece_active == 1:
            rate_type_text = "每件%s元" % (check_float(goods_rate.piece_cent/100))
            rate_list.append(rate_type_text)
        if goods_rate.weight_active == 1:
            rate_type_text = "每%s%s元" % (weight_unit_text, check_float(
                goods_rate.get_weight_cent(weight_unit_text=weight_unit_text)/100))
            rate_list.append(rate_type_text)
        if goods_rate.amount_active == 1:
            rate_type_text = "按总金额的%s%%" % (check_float(goods_rate.amount_cent/100))
            rate_list.append(rate_type_text)
...
class ShopCustomerFunc():
    ...
    @classmethod
    def update_purchase_info(cls, session, customer_id, shop_id, purchase_money=0, purchase_id=0):
        """
            更新用户采购金额/采购次数
        """
        ...
        shop_customer = cls.get_shop_customer_through_id(session, shop_id, customer_id)
        if shop_customer:
        ...
        if purchase_id:
            customer_purchase = models.CustomerPurchase.get_by_id(session, purchase_id)
    ...
    @classmethod
    def purchase_point(cls, session, shop_id, shop_customer, order, operator_id):
        """
            采购操作增加积分入口
        """
        ...
        if rule.purchases:
            ...
            cls.update_point(session, shop_id, shop_customer, point_delta, operator_id, operate_type=1, rule_id=rule.id)
        ...
        if rule.weekly_purchase or rule.monthly_purchase:
            ...
            if rule.weekly_purchase:
                ...
                first_day_this_week = datetime.datetime.fromordinal(first_day_this_week.toordinal())
                ...
                if not point_history:
                    # 本周所有订单的日期
                    order_dates_this_week = query.filter(Order.create_time > first_day_this_week).all()
                    # 本周产生了订单的天数
                    days_purchased_this_week = len({d[0].date(): "" for d in order_dates_this_week})
                    # 采购天数达标则增加积分
                    if days_purchased_this_week >= rule.weekly_purchase_floor:
                        cls.update_point(session, shop_id, shop_customer, rule.weekly_purchase_point, operator_id,
                                         operate_type=3, rule_id=rule.id)

            # 1. 月采购积分增长
            if rule.monthly_purchase:
                today = datetime.date.today()
                # 本月第一天
                first_day_this_month = today + datetime.timedelta(days=-today.day + 1)
                first_day_this_month = datetime.datetime.fromordinal(first_day_this_month.toordinal())
                ...
                if not point_history:
                    # 本月所有订单的日期
                    order_dates_this_month = query.filter(Order.create_time > first_day_this_month).all()
                    # 本月产生了订单的天数
                    days_purchased_this_month = len({d[0].date(): "" for d in order_dates_this_month})
                    # 采购天数达标则增加积分
                    if days_purchased_this_month >= rule.monthly_purchase_floor:
                        cls.update_point(session, shop_id, shop_customer, rule.monthly_purchase_point, operator_id,
                                         operate_type=4, rule_id=rule.id)

    @classmethod
    def update_point(cls, session, shop_id, shop_customer, delta, operator_id, operate_type=0, rule_id=0):
        """
            :param delta: 积分变更量
    ...
    def format_customer_info(cls, session, customer_list, type="list"):
        ...
        for i in customer_list:
            customer_info = i if isinstance(i, models.ShopCustomer) else i[0]
            purchase_info = None if isinstance(i, models.ShopCustomer) else i[1]
            # 查询担保人和创建人名字
            creator_id = customer_info.creator_id
            sponsor_id = customer_info.sponsor_id
            creator_name = AccountFunc.get_account_name(session, creator_id) if creator_id else ""
            sponsor_name = AccountFunc.get_account_name(session, sponsor_id) if sponsor_id else ""
            ...
            if black_info:
                defriend_time = TimeFunc.time_to_str(black_info.edit_time, "date")
                defriend_reason = black_info.reason
                defriend_operator_id = black_info.edit_user_id
                defriend_operator_name = AccountFunc.get_account_name(session, defriend_operator_id) if defriend_operator_id else ""
            customer_group_orm = CustomerGroupORM(session)
            group_id = customer_info.group_id
            group_name, = customer_group_orm.get_group_name_through_id(group_id)
            ...
            data = dict(
                ...
                history_debt_rest=check_float(customer_info.history_debt_rest / 100),
                ...
                legal_representative_id_card_copy = customer_info.legal_representative_id_card_copy,
    ...
    # 获取赊账客户信息
    @classmethod
    def get_customer_info_through_id_list(self, session, shop_id, customer_id_list):
        ShopCustomer = models.ShopCustomer
        debtor_list = session \
            .query(ShopCustomer) \
            .filter_by(shop_id=shop_id) \
    ...
    def get_debtor_order_info(self, session, shop_id, history):
        ...
        if debt_type == 0:
            ...
            if pay_type == 10:
                ...
                pb_order_money = session.query(models.PbOrderMoney).filter_by(
                    id=fk_id).first()
                if pb_order_money and (
                        pb_order_money.barcode_wx_cent > 0 or
                        pb_order_money.barcode_ali_cent > 0):
                    can_be_reset = 0
                order_info = OrderBaseFunc.order_pay_type_text(fk_id, pay_type, combine_pay_dict)
            else:
    ...
    def get_customers_under_group(cls, session, group_id, page=-1):
        ...
        page_size = 20
        shop_customers = query_base.offset(page * page_size).limit(page_size).all()
        nomore = 0
    ...
    def check_export_permission(permission_type):
        def decorator(func):
            def wrapper(*args, **kw):
                ...
                log = DataExportLog(shop_id=shop_id,
...
class PermissionFunc():
    ...
    def dumps_permission_list(cls, permission_list):
        ...
        :rtype: str
        """
        permission_list = DataFormatFunc.format_int_to_str_inlist(permission_list)
...
class RecordPrintFunc():
    @classmethod
    def record_print_log(cl, session, shop_id, fk_id, user_id, print_result, error_txt, printer_num,
            print_type=0, source_type=0):
...
class AccountingPeriodFunc():
    ...
    def get_accounting_info(cls, session, shop_id, date):
        ...
        if accounting_record:
            accounting_info["username"] = AccountFunc.get_account_realname(session,
                                                                           accounting_record.user_id) if accounting_record.user_id else ""
            accounting_info["time"] = TimeFunc.time_to_str(accounting_record.create_date, "time")
            accounting_info["accounting_model"] = 1 if accounting_record.user_id else 2
    ...
    def open_auto_accounting_task(cls, session, shop_id):
        ...
        if auto_time_record:
            ...
            time_now = datetime.datetime.now()
            time_auto = datetime.datetime(time_now.year, time_now.month, time_now.day,
                                          auto_time.hour, auto_time.minute, auto_time.second)
            ...
            # 开启新的celery任务
            auto_accounting.apply_async(args=[shop_id, time_record_id], countdown=countdown)
...
class StaffFunc():
    ...
    # 获取员工备注
    @classmethod
    def get_hirelink_name_through_account_id(cls, session, shop_id, account_id):
        HireLink = models.HireLink
        name = session.query(HireLink.name_remark).filter_by(shop_id=shop_id, id=account_id).scalar() or ""
        return name

    # 获取员工备注
    @classmethod
    def get_hirelink_name_through_account_id_list(cls, session, shop_id, account_id_list):
        HireLink = models.HireLink
    ...
    def assign_default_printers(cls, session, hire_link):
        ...
        for printer in default_printers:
            ...
            if printer.is_salesman_default and hire_link.active_salesman == 1 or \
                ...
                PrinterFunc.gen_printer_assign_link(
                    session, printer.id, hire_link.account_id)
        return None
    ...
    def get_hirelink(cls,session,type="list",**params):
        ...
        if type == "list":
            ...
            return {hirelink.account_id:hirelink for hirelink in hirelink_list}

    @classmethod
    def get_shop_super_admin(cls, session, shop_id):
        shop = session.query(models.Shop).filter_by(id=shop_id).first()
        super_admin = session.query(models.HireLink).filter_by(shop_id=shop_id, account_id=shop.boss_id).first()
        return super_admin
...
class GoodsOperationRecordFunc:
    ...
    def get_operation_detail(cls, session, records, weight_unit_text="斤"):
        ...
        result = {}
        result.update(cls.get_goods_history_data(
            session, data[1] | data[2] | data[3] | data[4] | data[17] | data[18]))
        result.update(cls.get_goods_stockin_data(
            session, data[5] | data[12], weight_unit_text=weight_unit_text))
        result.update(cls.get_goods_name_edit_data(session, data[6]))
        result.update(cls.get_goods_group_change_data(session, data[7] | data[8]))
        result.update(cls.get_goods_group_edit_data(session, data[9]))
        result.update(cls.get_goods_breakage_data(
            session, data[10], weight_unit_text=weight_unit_text))
        result.update(cls.get_goods_cost_price_edit_data(
            session, data[11], weight_unit_text=weight_unit_text))
        result.update(cls.get_goods_allocate_data(
            session, data[14] | data[15] | data[16], weight_unit_text=weight_unit_text))
        result.update(cls.get_goods_inventory_data(
    ...
    def get_goods_allocate_data(cls, session, records, weight_unit_text="斤"):
        ...
        goods_allocate = session.query(StorageAllocateGoods.id,
            ...
            .all()
        goods_allocate_dict = {i.id: (i.quantity, i.weight) for i in goods_allocate}
        shop_dict = {i.id: i.delivery_shop_id for i in goods_allocate}
        goods_id_dict = {i.id: i.goods_id for i in goods_allocate}
        receive_goods_id_dict = {i.id: i.receive_goods_id for i in goods_allocate}
        all_goods_ids_list = list(goods_id_dict.values()) + list(receive_goods_id_dict.values())
        goods_name_dict = GoodsFunc.get_goods_name_through_id_list(session, all_goods_ids_list)
        ...
        for i in records:
            ...
            data[i.id] = dict(
                ...
                weight=check_float(UnitFunc.convert_weight_unit(
                    goods_allocate[1],
                    weight_unit_text=weight_unit_text,
                    source="weight") / 100),
                weight_unit=weight_unit_text)
    ...
    def get_goods_history_data(cls, session, records):
        ...
        goods_name_dict = GoodsFunc.get_goods_name_through_id_list(session, set(goods_id_dict.values()))
    ...
    def get_goods_stockin_data(cls, session, records, weight_unit_text="斤"):
        ...
        goods_stockin = session.query(GoodsStockinRecord.id,
            ...
            .all()
        goods_stockin_dict = {i.id: (i.quantity, i.weight) for i in goods_stockin}
    ...
    def get_goods_name_edit_data(cls, session, records):
        ...
        for i in records:
            ...
            if t:
                data[i.id] = dict(goods_id=t.goods_id, origin_name=t.origin_name, now_name=t.now_name)
            else:
                data[i.id] = dict(goods_id=None, origin_name=None, now_name=None)
        return data
    ...
    def get_goods_group_change_data(cls, session, records):
        ...
        group_name_dict = GoodsGroupFunc.get_group_name_dict(session, set(group_id_dict.values()))
    ...
    def get_goods_group_edit_data(cls, session, records):
        ...
        group_name_dict = GoodsGroupFunc.get_group_name_dict(session, group_ids)

        goods_ids = {i.goods_id for i in goods_group_edit}
        goods_name_dict = GoodsFunc.get_goods_name_through_id_list(session, goods_ids)

        data = {}
        for i in records:
            t = goods_group_edit_dict.get(i.fk_id)
            if t:
                data[i.id] = dict(goods_id=t.goods_id,
                                  goods_name=goods_name_dict.get(t.goods_id, ""),
                                  origin_group_name=group_name_dict.get(t.origin_group_id, ""),
                                  now_group_name=group_name_dict.get(t.now_group_id, ""))
            else:
                data[i.id] = dict(goods_id=None, goods_name=None, origin_group_name=None, now_group_name=None)
        return data
    ...
    def get_goods_breakage_data(cls, session, records, weight_unit_text="斤"):
        ...
        goods_breakage = session.query(GoodsBreakageRecord.id,
            ...
            .all()
        goods_breakage_dict = {i.id: (i.quantity, i.weight) for i in goods_breakage}
    ...
    @classmethod
    def get_goods_cost_price_edit_data(cls, session, records, weight_unit_text="斤"):
        ...
        for r in records:
            ...
            data[r.id] = dict(
                ...
                origin_cost_price_weight=check_float(UnitFunc.convert_weight_unit(
                    history.origin_cost_price_weight,
                    weight_unit_text=weight_unit_text,
                    source="price") / 100),
                new_cost_price_quantity=check_float(
                    history.new_cost_price_quantity / 100),
                new_cost_price_weight=check_float(UnitFunc.convert_weight_unit(
    ...
    def get_goods_inventory_data(cls, session, records, weight_unit_text="斤"):
        ...
        goods_inventory = session.query(GoodsInventoryRecord.id,
            ...
            .all()
        goods_inventory_dict = {i.id: (i.quantity, i.weight) for i in goods_inventory}
...
class AuthFunc:
    ...
    def merge_passport(cls, session, passport_id, phone):
        ...
        merge_key = "passport_merge:{}|{}:pf".format(source_passport_id, target_passport_id)
    ...
    def update_single_accountinfo(cls, passport_id, data, session, force_create=False):
        ...
        if not user:
            if force_create:
                user = Accountinfo(passport_id=passport_id, uuid=Accountinfo.make_uuid(session))
                session.add(user)
                session.flush()
            else:
                return None
        ...
        for k, v in data.items():
            if hasattr(user, k) and k in ("phone", "email", "wx_unionid", "qq_account"):
                setattr(user, k, v)
        session.commit()
        return user
...
class SalesRecordAjustFunc():
    ...
    @classmethod
    def get_ajust_new_goods_info_through_id_list(cls, session, origin_record_id_list):
        """ 获取串货调整新货品ID
        """
        ...
        if ajust_record_list:
            ...
            goods_info_dict = GoodsFunc.get_goods_info_list_through_id_list(session, shop_id, new_goods_id_list)
    ...
    def get_ajust_info(cls, session, record):
        ...
        if record.bill_date + datetime.timedelta(days=7) > datetime.datetime.now().date() and record.status in (3,5):
            could_ajust = True
        ...
        if ajust_record:
            ...
            ajust_info["ajust_origin_goods_name"] = GoodsFunc.get_goods_name_with_supplier(session, shop_id,
                                                                                           origin_goods_id)  # 原始货品名
            ajust_info["ajust_new_goods_id"] = new_goods_id  # 新货品ID
            ajust_info["ajust_new_goods_name"] = GoodsFunc.get_goods_name_with_supplier(session, shop_id,
                                                                                        new_goods_id)  # 新货品名
            ...
            ajust_info["ajust_operator_name"] = AccountFunc.get_account_realname(session,
                                                                                 ajust_record.operator_id)  # 调整人姓名
            ajust_info["ajust_operator_time"] = TimeFunc.time_to_str(ajust_record.create_time)  # 调整时间
            ajust_info["ajust_type"] = ajust_type  # 调整类型
            ajust_info["ajust_origin_id"] = ajust_record.origin_record_id  # 原始单号ID
            ajust_info["ajust_refund_id"] = ajust_record.refund_record_id  # 冲红单号ID
    ...
    def get_multi_ajust_info(cls, session, record_id_list):
        ...
        ajust_record_id_list = session.query(GoodsSalesRecordAjust.origin_record_id) \
            ...
            .all()
        ajust_record_id_list = {x.origin_record_id for x in ajust_record_id_list}
    ...
    def get_multi_ajust_all(cls, session, record_id_list):
        ...
        record_id_list = origin_record_id_list.union(refund_record_id_list).union(new_record_id_list)
...
class StockInFunc():
    ...
    @classmethod
    def get_doc_detail_unit_pub(cls, session, doc_detail, doc, goods_id_list, filter_detail_id_list=[]):
        """获取最新的入库/报损记录的货品单位
        """
        detail_ids = session \
    ...
    def get_flow(cls, session, shop_id, page, **kwargs):
        ...
        order_condition = kwargs.get("order_condition", 0)  # 排序条件 0: 按流水 ID 1: 按入库重量 2: 按入库件数 3: 按费用

        from_date, to_date = TimeFunc.transfer_input_range_to_date(from_date, to_date)
        weight_unit_text = ConfigFunc.get_weight_unit_text(session, shop_id)

        # 分组id到name的映射
        map_groups_id_name = GoodsGroupFunc.get_groups_under_shop(session, shop_id)
        ...
        param_join_second = [
            models.Goods,
            models.Goods.id == models.StockInDocsDetail.goods_id]
        params_filter = [
        ...
        if receive_date:
            params_filter.append(models.StockInDocs.receive_date == receive_date)
        if goods_id:
            params_filter.append(models.StockInDocsDetail.goods_id == goods_id)
        if is_unclearing:
            # 未结算不展示草稿和撤销的内容
            params_filter.append(models.StockInDocsDetail.clearing_record_id == 0)
            params_filter.append(models.StockInDocs.status == 1)
        if pay_type:
            params_filter.append(models.StockInDocsDetail.pay_type.in_(pay_type))
        if batch_id_list:
            params_filter.append(models.StockInDocsDetail.goods_batch_id.in_(batch_id_list))
        if detail_id_list:
            params_filter.append(models.StockInDocsDetail.id.in_(detail_id_list))
        if shop_supplier_id:
            params_filter.append(models.Goods.shop_supplier_id == shop_supplier_id)
        if group_id_list:
        ...
        if goods_id_list:
            # 筛选主货品时需要把分货品添加进来
            params_filter.append(or_(models.StockInDocsDetail.goods_id.in_(goods_id_list),
        ...
        flow_base = flow_base.join(*param_join_first).join(*param_join_second).filter(
            *params_filter)
        ...
        if page == -1:
            ...
            count = count_base.join(*param_join_first).join(*param_join_second).filter(
                *params_filter).scalar()
            page_sum = math.ceil(count / page_size)

        data_list = StockinHistoryFunc.batch_format_stockin_history_info(session, flows, weight_unit_text,
                                                                         map_groups_id_name, return_type="list")
        ...
        sum_stockin = session.query(
            ...
            func.sum(models.StockInDocsDetail.total_price).label("total_price")).join(
            *param_join_first).join(*param_join_second).filter(
            *params_filter).first()
        sum_dict = dict(
            count=check_int(sum_stockin.count)
            if sum_stockin.count else 0,
        ...
        if is_unclearing:
            ...
            sum_stockin_unpay = session.query(
                func.sum(models.StockInDocsDetail.total_price).label("total_price")).filter(
                *params_filter).filter_by(pay_type=5).join(
                *param_join_first).join(*param_join_second).first()
            sum_dict["total_price_unpay"] = check_float(
                sum_stockin_unpay.total_price / 100) if sum_stockin_unpay.total_price else 0

            ids_stockin = session.query(models.StockInDocsDetail.id).filter(
                *params_filter).join(*param_join_first).join(*param_join_second).all()
        ...
        total_goods_list = GoodsMasterBranchFunc.apply_filter(total_goods_list, shop_id)
        total_goods_list = total_goods_list.all()
        total_goods_list = [{"id": goods.id,
                             "name": GoodsFunc.get_goods_detail_name(goods)} for _, goods in total_goods_list]

        return page_sum, data_list, sum_dict, total_goods_list

    @classmethod
    def get_statistic_each_goods(cls, cls_doc_detail, cls_doc, shop_id, session, page, **kwargs):
        """ 获取入库单的采购汇总 """
        page_size = 20
        supply_type = kwargs.get("supply_type")
        from_date = kwargs.get("from_date")
        ...
        goods_id_list = kwargs.get("goods_id_list", [])
        group_id_list = kwargs.get("group_id_list", [])
        order_condition = kwargs.get("order_condition", 0)  # 排序条件 0: 按货品 ID 1: 按入库重量 2: 按入库件数 3: 按费用
        ...
        label_total_price = func.sum(cls_doc_detail.total_price).label("total_price")
        label_cost_price_quantity = func.sum(cls_doc_detail.cost_price_quantity).label("cost_price_quantity")
        label_cost_price_weight = func.sum(cls_doc_detail.cost_price_weight).label("cost_price_weight")
        query_list = [
        ...
        if cls_doc_detail == models.StockInDocsDetail:
            query_list.append(func.sum(case([(cls_doc_detail.pay_type == 5, cls_doc_detail.total_price)])).label("credit_money"))
        statistic_base = session.query(*query_list)
        count_base = session.query(
            func.count(cls_doc_detail.goods_id))

        param_join_first = [
            cls_doc,
            cls_doc.id == cls_doc_detail.doc_id]
        param_join_second = [
            models.Goods,
            models.Goods.id == cls_doc_detail.goods_id]
        params_group_by = [
            cls_doc_detail.goods_id,
            models.Goods.supply_type,
            models.Goods.group_id]
        params_filter = [
            ...
            cls_doc.status.in_([-1, 1, 2]),
            cls_doc_detail.status == 1]
        if supply_type is not None:
            if supply_type not in [0, 1, 2, 3]:
                raise ValueError("供应类型选择错误，只能为自营、代卖、货主、联营")
            params_filter.append(models.Goods.supply_type == supply_type)
        ...
        master_statistic_base = GoodsMasterBranchFunc.apply_filter(statistic_base, shop_id)
        if page == -1:
        ...
        # 查询主货品对应的分货品汇总记录
        master_goods_ids = [item.goods_id for item in statistic if item.is_master == 1]
        ...
        for branch_statistics_item in branch_statistics:
            branch_statistics_dict[branch_statistics_item.master_goods_id].append(branch_statistics_item)
        ...
        for master_statistic in total_master_statistic:
            total_data_sum["total_weight"] += UnitFunc.convert_weight_unit(master_statistic.weight, source="weight", weight_unit_text=weight_unit_text)
            total_data_sum["total_quantity"] += master_statistic.quantity
            total_data_sum["total_price"] += master_statistic.total_price
        total_data_sum["total_weight"] = check_float(total_data_sum["total_weight"] / 100)
        total_data_sum["total_quantity"] = check_float(total_data_sum["total_quantity"] / 100)
        total_data_sum["total_price"] = check_float(total_data_sum["total_price"] / 100)
        ...
        total_goods_list = GoodsMasterBranchFunc.apply_filter(total_goods_list, shop_id)
        total_goods_list = total_goods_list.all()
        total_goods_list = [{"id": item.goods_id,
                             "name": item.name} for item in total_goods_list]

        goods_id_list = [s.goods_id for s in statistic + branch_statistics]
        g_dict = GoodsFunc.get_goods_in_shop(session, shop_id, weight_unit_text, goods_id_list, merge_self_goods=False)

        # 获取商品最新入库的入库单位
        map_detail_unit = StockInFunc.get_doc_detail_unit_pub(session,
                                                   cls_doc_detail,
                                                   cls_doc,
                                                   goods_id_list)
        ...
        def prepare_data(doc_detail, goods, detail_unit, type):
            data_dict = dict(
                ...
                quantity=check_float(doc_detail.quantity / 100) if doc_detail.quantity else "-",
                weight=check_float(UnitFunc.convert_weight_unit(doc_detail.weight, source="weight",
                                                                weight_unit_text=weight_unit_text) / 100) if doc.weight else "-",
                total_price=check_float(doc_detail.total_price / 100),
                goods_supply_type=doc_detail.supply_type,
                storage_unit=goods["storage_unit"],
                storage_unit_text="件" if goods["storage_unit"] == 1 else weight_unit_text,
                purchase_unit=goods["purchase_unit"],
                purchase_unit_text="件" if goods["purchase_unit"] == 1 else weight_unit_text,
                detail_unit=detail_unit,
            )
            # 入库单的价格用总价计算
            if type == models.StockInDocsDetail:
                data_dict["quantity_unit_price"] = check_float(
                    doc_detail.total_price / doc_detail.quantity) if doc_detail.quantity else ""
                try:
                    data_dict["weight_unit_price"] = check_float(doc_detail.total_price / doc_detail.get_weight(
                        weight_unit_text=weight_unit_text)) if doc_detail.weight else ""
                ...
                data_dict["cost_price_quantity"] = check_float(doc_detail.cost_price_quantity / 100)
                try:
                    data_dict["cost_price_weight"] = check_float(
                        doc_detail.get_cost_price_weight(weight_unit_text=weight_unit_text) / 100)
                except AttributeError:
                    data_dict["cost_price_weight"] = check_float(
        ...
        for doc in statistic:
            ...
            if doc.is_master:
                ...
                for branch_doc in branch_docs:
                    detail_unit = map_detail_unit.get(branch_doc.goods_id, branch_doc.unit)
                    branch_goods = g_dict[branch_doc.goods_id]
                    branch_data = prepare_data(branch_doc, branch_goods, detail_unit, cls_doc_detail)
        ...
        if pay_type == 10:
            ...
            for key in key_name_dict:
                ...
                if combine_money:
                    ...
                    combine_pay_type_text = OrderBaseFunc.pay_type_dict[combine_pay_type]
                    combine_pay_type_text = "{0}:{1}元".format(combine_pay_type_text, combine_money)
...
class LcBankFunc():
    ...
    def get_bank_name_dict(cls, session, bank_nos):
        banks = session \
            ...
            .all()
        bank_name_dict = {bank.parent_bank_no: bank.parent_bank_name for bank in banks}
...
class SalesmanFunc():
    ...
    @classmethod
    def list_goods_id_editable(cls, session, shop_id, account_id, permission_text_list):
        """ 获取开票员可以编辑的货品id
        ...
        if "editgoods" in permission_text_list:
            ...
            result = cls._list_goods_id_created_by(session, shop_id, account_id)
        return result
...
class GoodsOwnerFunc():
    ...
    def list_goods_id_editable(cls, session, shop_id, account_id):
        ...
        :rtype: list
        """
        permission_text_list = cls.permission_text_list(session, shop_id, account_id)
    ...
    def permission_text_list(cls, session, shop_id, account_id):
        ...
        if supplier:
            result = PermissionFunc.permission_text_list(
                "goods_owner",
                PermissionFunc.loads_permission_list(supplier.goods_owner_permission))
        return result
...
class OrderFinishFunc():
    ...
    @classmethod
    def gen_sales_record_by_origin(
            cls, session, sale_record, record_type, goods, batch_id,
            order=None, num=None, rate=1):
        """ 通过原始小票记录生成新的小票记录 """
        now = datetime.datetime.now()
        t_year, t_month, t_day, t_week = TimeFunc.get_date_split(now)
        # 检查是否扎帐
        account_period_date = now.date()
        if AccountingPeriodFunc.get_accounting_time(session, sale_record.shop_id):
            account_period_date = account_period_date+datetime.timedelta(1)
        ...
        record.bill_time = sale_record.bill_time if not order else now.strftime('%H:%M:%S')
    ...
    @classmethod
    def gen_sales_record_by_frontend(
            cls, session, operater_id, goods, batch_id, status, fact_price,
            sales_num, commission, commission_mul, commission_sum, deposit_avg,
            deposit_sum, receipt_money, shop_customer_id=0, remark="", wireless_print_num="",
            printer_remark="", tare_weight=0, is_refund=False, purchase_id=0, num=None,
            bill_time=None, accountant_id=0, order_id=None, pay_type=None, image_remark=""):
        """ 通过前端传递的参数生成小票 """
        shop_id = goods.shop_id
        user_id = operater_id
        now = datetime.datetime.now()
        ...
        sales_record = models.GoodsSalesRecord(
            ...
            goods_id=goods.id)
        if not num:
            num=GoodsSaleFunc.gen_sale_record_num(session, shop_id, base_time=bill_time)
        sales_record.num = num

        sales_record.set_fact_price(
            round(fact_price * 100), goods.unit, weight_unit_text=weight_unit_text)
        ...
        sales_record.bill_time = bill_time.time() if bill_time else now.strftime('%H:%M:%S')
    ...
    @classmethod
    def gen_storage_record(
            cls, session, goods, origin_storage, origin_storage_num, sale_record,
            operator_id, record_type):
        """ 生成库存变更记录 """
        storage_record = models.GoodsStorageRecord(
            shop_id=goods.shop_id,
            goods_id=goods.id,
...
class CustomerSignature():
    ...
    def get_signature_image(cls, fileid, source="order"):
        ...
        if ret.status_code == 200:
            signature_image = str(base64.b64encode(ret.content).decode("utf-8"))
...
class CommonToolsFunc():
    ...
    def transform_str_to_list(list_or_csv_str):
        ...
        if list_or_csv_str.startswith("["):
            python_list = CommonToolsFunc.transform_json_list_to_python_list(list_or_csv_str)
        else:
            python_list = CommonToolsFunc.transform_csv_to_list(list_or_csv_str)
        return python_list
...
class ShopRegisterFunc():
    ...
    def get_deposit_paid_record_by_id(session, record_id):
        ...
        record = session.query(DepositPaidRecord).filter_by(id=record_id).first()
        return record
    ...
    def get_shop_apply_pay_status_by_user_id(session, user_id):
        """通过用户id获取店铺押金支付状态"""
        ShopApply = models.ShopApply
        shop_apply = session.query(ShopApply).filter(ShopApply.account_id == user_id, ShopApply.status.in_([0, 1])).first()
        if not shop_apply:
            return None
    ...
    def get_shop_apply_by_user_id(session, user_id):
        ...
        ShopApply = models.ShopApply
        shop_apply = session.query(ShopApply).filter(ShopApply.account_id == user_id, ShopApply.status == 0).first()
        return shop_apply
...
class SuperAdminFunc():
    ...
    def get_super_admin_by_id(session, _id, status=None):
        ...
        if not status:
            super_admin = session.query(SuperAdmin).filter(SuperAdmin.id == _id, SuperAdmin.is_delete == 0).first()
        else:
            super_admin = session.query(SuperAdmin).filter(SuperAdmin.id == _id).first()
        return super_admin

    @staticmethod
    def get_apply_shop_by_passed_shop_id(session, _id):
        """通过正式店铺的id查找申请中的店铺"""
        shop_apply = session.query(models.ShopApply).filter_by(shop_id=_id).first()
        return shop_apply
    ...
    def get_shop_is_trial_close_dict_by_id_list(session, id_list):
        """获取店铺id列表对应的店铺的试用状态"""
        Shop = models.Shop
        shop_trail = session.query(Shop.id, Shop.is_trial, Shop.status).filter(Shop.id.in_(id_list)).all()
        id_trial_dict = dict()
        id_status_dict = dict()
...
class FileToolFunc():
    ...
    @staticmethod
    def write_csv_file(file_name, content, shop_id, user_id, timestamp, file_path=EXPORT_PATH):
        """本地保存csv文件"""
        file_name += "|{}:{}:{}".format(shop_id, user_id, timestamp)
        file = FileToolFunc.get_file_path_name(file_name, file_path=file_path)
        name_key = FileToolFunc.get_file_name_redis_key(shop_id, user_id, timestamp)
        status_key = FileToolFunc.get_file_status_redis_key(shop_id, user_id, timestamp)
        try:
    ...
    def set_shop_export_status(shop_id, user_id, exporting_key, seconds=5):
        """设置店铺导出状态，默认十秒"""
        redis_export.setex("exporting:%d-%d" % (shop_id, user_id), seconds, exporting_key)
